...
| check-context-architecture.sh ✅（已执行） | 无（随退役免录） | R1–R12：上下文类型禁字段、能力调用限定点 | 已完成（判据修正①→③/④）：R1/R7/R9 字段集断言编译期恒真（struct 改动全构造点暴露）；R2/R4/R5/R6/R10 落四条引擎规则（workspace-control/git-owner/persistence/live-resources）；R3/R8/R12 黑名单符号已物理删除④，复活归 review |
| check-unified-authorization.sh ✅（已执行） | 无 | 授权统一（混合①②④） | 已完成：Allow(AuthorizationContext) 变体签名由类型系统锁定；project/tools 读取禁式落 pattern.project.no-permission-state-read 与 pattern.tools-adapters.no-allow-all；patch 顺序断言（精确字面量序列）与 legacy 防复活（settings.rs/PolicyDecision 已物理删除，引用即编译失败）④ 退役，顺序语义归测试 |
...
| 编排器内嵌 run_tui_single_source_structure_guard ✅（已执行） | scope.tui.arch.inline-exclusions（entry 已删） | retired widget adapter 仅 cfg(test)、render 禁镜像存储/生产读写 API | 已完成：retired adapter 已是 #[cfg(test)] pub mod 模块级锁定（生产树编译不可达，恒真④）；镜像禁式落 pattern.tui.render-no-mirror-storage（pub(super) 精确形态，view_state 派生投影合法）；CompactProgress 等黑名单符号已物理删除④；内嵌函数与调用行删除，编排器瘦身 80 行 |
...
- 已完成 27/30：F-1~13、F-15~17、F-19~22、F-24~30（除 F-14、F-18 半段、F-23）。
- defer 清单（正则/计数/正向存在类特征，随引擎 regex 断言器批次评估）：
  F-14 行数预算与必需文件断言（需 line_budget 断言器）；F-18 层边界 use 分区半段；
  F-23 unsafe-text 正则豁免；logging static 声明白名单；config per-var env 细分；
  F-13 run_loop 计数恰一断言。
- 引擎断言器 8 种，registry rules 88 条；`.agents/hooks/` 业务脚本 63→22。
