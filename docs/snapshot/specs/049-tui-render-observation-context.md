...
- **rev.3（本版）**：修正两处 review 发现的设计错误——
  1. §10.1.2 `tool_display` 归属写反：它是 render 端绘制元数据（render 是唯一实质消费者：`tool_result.rs` 绘制时调 `format_result_lines` / `result_max_lines` / `result_render_kind`），**应留在 render**；消除反向依赖靠删掉 view_assembler 唯一一处 `lookup_display` 调用、由 render 在绘制时格式化，而非上移 presenter。仅 nesting 上移（§5.3、§7.3、§8.3、§10.1.2、§11）。
  2. §4.5 与 §7.3 自相矛盾：「单一 owner」被误表述成「timeline 零 payload」。修正为 hybrid——tool 数据（chats 为 owner）在 timeline 存引用；非 tool 文本（timeline 独占）自持 payload；二者皆满足无重复（§2、§4.5、§7.3、§7.4、§13、§14）。
- **rev.2**：基于对源码的逐条 fact-check，整合架构 review 意见——
...
4. 拆分 `ConversationModel.blocks` 的职责，引入 `OutputTimelineModel` 作为输出区 read model：**对已被 `chats` 拥有的数据（tool call/result）只存引用、不复制**；timeline 独占的非 tool 文本自持 payload——核心是同一份显示数据单一 owner、无重复（DRY，宪法 #4）。
...
任一可显示数据**有且仅有一个所有者**——「单一 owner」的实质是**无重复存储**，而非「timeline 必须零 payload」。是否需要以引用持有，取决于该数据**有没有第二个 owner**：

1. tool call / tool result 的内容真相只存于 `ConversationModel.chats`（`ChatTurn.tool_calls`）。它**已被 chats 拥有**，故在 timeline 中**必须以引用（`tool_call_id`）出现、严禁复制**——内联即重蹈 §5.5 的双写。
2. 非 tool 文本 / thinking / 系统 / 错误 / askuser 在 `chats` 中**没有第二份**（chats 不存这些交织显示块），timeline（或其内容 store）**就是它们的唯一 owner**，故**可自持 payload**——这不违反单一 owner，因为不存在重复。
3. Presenter（ViewAssembler）在 assemble 时，对引用项按 id **join** `chats`、对自持项直接读取，组合出 ViewModel。

禁止的是「同一字段在两个 model / 两个 struct 里各存一份、靠手工同步」。当前 `ConversationBlock::ToolCall { name, summary, args_preview }` 与 `chats` 双写，即违反本条（见 §5.5），重构后必须消除；而 `AssistantText { text }` 这类**单一持有**的文本不在此列。
...
> **注（rev.3）**：「反向依赖存在」的诊断成立，但**修法因项而异**（详见 §10.1.2）。`nesting` 治理 ViewModel 树形、应上移 `view_model`；而 `tool_display` 是 render 端绘制元数据（render 是其唯一实质消费者），**应留在 render**——反向依赖靠删掉 view_assembler 唯一一处 `lookup_display` 调用（`summarize_non_embedded_result` 的 orphan 摘要）消除，**而非上移 `tool_display`**。
...
这违反宪法 #4（DRY：数据访问逻辑 MUST 定义一次）。注意：`blocks` 并非纯冗余——跨异构条目（用户/assistant/thinking/工具/系统/错误/askuser）的**交织顺序只有 `blocks` 持有**，`chats` 无法重建。所以正解不是删 `blocks`，而是让它退化为「顺序索引 + tool 引用 + 自持非 tool 文本」（§4.5、§7.3）。
...
### 7.3 OutputTimelineModel（tool 存引用 + 非 tool 自持 payload）
...
OutputTimelineItem  // 混合：被别处拥有的存引用；timeline 独占的自持 payload
  - ToolCall   { context, tool_call_id }    // 引用：status/args/summary/result 唯一 owner 在 chats
  - ToolResult { context, tool_call_id }    // 引用：同上；仅记录「结果应渲染在此调用之后」
  - AssistantText { id, text }              // 自持：timeline 即唯一 owner，无第二份
  - Thinking      { id, text }
  - UserMessage   { id, text }
  - System { id, text } / Error { id, text } / HookNotice {..} / AgentProgress {..} / AskUser {..}
```

要点：

1. **tool 条目只引用 `tool_call_id`**，不复制 `name/summary/args_preview/result`——直接消除 §5.5 的死字段与双写。
2. **非 tool 文本（text/thinking/系统/错误/askuser）自持 payload**：它们在 `chats` 中没有第二份，timeline item 即唯一 owner，内联存储已满足单一 owner（§4.5）。这些变体可直接沿用现有 `ConversationBlock` 的形状（本就带 payload）；**唯独 tool 变体必须降为引用**。是否再抽进独立 keyed store 只是后续可选归一，与单一 owner 无关。
...
4. **orphan / 非嵌入 tool 结果的摘要格式化落到 render**：assembler 不再调 `tool_display::lookup_display`，改为 ViewModel 携带 `(tool_name, raw_output, is_error)`、由 render 绘制时做 `format_result_summary`（与它已在做的 `format_result_lines` 截断同层）——这正是消除 §5.3 反向依赖的方式。
...
**原子性约束（rev.2 新增）**：projector 对 `ConversationModel` 与 `OutputTimelineModel` 的两份 patch **必须在同一次 reduce 内原子应用**，不得出现「chats 已更新、timeline 未更新」的半态。由于 §7.3 让 **tool 数据在 timeline 只存引用、内容唯一 owner 在 chats**，二者天然不会内容不一致；本约束进一步保证「引用存在性」与「内容存在性」同帧一致（例如：append 了 ToolCall 引用，则该 `tool_call_id` 必已在 chats 中存在 placeholder）。
...
Presenter 负责展示语义：标题、状态文案、层级、diagnostic block 映射、按引用 join 出 tool 视图。**tool 特定的摘要/截断格式化（含 orphan/非嵌入 result）由 render 持 `tool_display` 完成**，不在 Presenter（见 §7.3 第 4 点、§10.1.2）。
...
2. **nesting 规则**（`allowed_child` / `MAX_BLOCK_DEPTH`）治理 ViewModel 树形结构，移到 `view_model`（或 `view_assembler`）；render 消费已成形的树、不需要它。
3. **`tool_display` lookup 留在 render**（绘制元数据，render 是唯一实质消费者：`tool_result.rs` 绘制时调 `format_result_lines` / `result_max_lines` / `result_render_kind`）。消除 `view_assembler → render` 反向依赖**不是上移 `tool_display`，而是删掉 view_assembler 唯一一处 `lookup_display` 调用**（`summarize_non_embedded_result` 的 orphan/非嵌入摘要）：改为 ViewModel 只携带 `(tool_name, raw_output, is_error)`，render 绘制时做 `format_result_summary`——与 §7.3 第 2/4 点一致。
4. 增加依赖方向检查清单/门禁：`adapter/**`、`view_assembler/**` 不得 `use crate::tui::render::*`。
...
落地 §10.1：safe_text 迁移、nesting 上移、删掉 view_assembler 的 `lookup_display` 调用（`tool_display` 留 render）、依赖方向门禁。
...
2. nesting、block semantic title 收敛到 Presenter；`tool_display` lookup 留在 render（assembler 不再调用）。
...
缓解：以 §7.3 的「tool 数据在 timeline 只存引用、内容单一 owner 在 chats」从结构上消除内容不一致；再以 §7.4 的同帧原子应用保证引用与内容存在性一致；§12.4.7 增设半态断言测试。
...
7. `OutputTimelineModel` 成为输出 timeline（顺序 + tool 引用 + 自持非 tool 文本）的所有者；**tool 数据无双写、无死字段**（chats 为唯一 owner）。
