...
| 113 | AskUserQuestion 回答后 LLM 新输出渲染到 AskUser 块上方；AskUser 块本身固定在初始位置，其他内容在上方不断刷新 | 中 | 待修复 | 未确认 | 2026-06 | **现象 1**：用户回答 AskUserQuestion 后，LLM 继续生成的文本出现在 AskUser 交互块的上方而非下方。**分析 1**：`append_assistant_text` 通过 `append_or_extend_text_block` 追加文本，如果 `active_text_block_id` 仍指向 AskUser 之前的 AssistantText 块，新文本会被 extend 到该旧块（位于 AskUser 块之前）。关键：`active_text_block_id` 只在 `TextBlockComplete`（`model.rs:262`）和 `complete_chat`（`model.rs:227`）时清除。AskUserQuestion 工具调用前如果没有收到 `TextBlockComplete` 事件，`active_text_block_id` 就不会为 None，后续文本就会 extend 到旧块。**现象 2**（追加观察）：AskUser 块本身不会随着新输出推进而向下滚动，而是固定在初始渲染位置；后续 LLM 文本、工具调用结果等增量内容都在 AskUser 块上方区域不断刷新，视觉上 AskUser 像被"钉"在屏幕某一位置。**待确认**：是否同根因（旧 block 序列错位）或独立的 view model 行为（如某种 pinned/锚定渲染逻辑保持 AskUser 块位置）。**关键代码**：`apps/cli/src/tui/model/conversation/model.rs:243,360-393`（`append_assistant_text` → `append_or_extend_text_block`），`model.rs:260-269`（`complete_text_block`） |
