...
| 65 | 工具结果 fenced code block 后续内容继续显示为 code 颜色 | 中 | 修复中（#58 主管线已切换，工具结果 fence 处理待 assistant/tool 组件接线后确认） | 未确认 | 2026-05 | Edit/Write 等工具结果中包含 fenced code block 时，例如 `✓ replaced 1 occurrence(s) in ...` 后展示文件路径并以 ``` 收尾，但后续普通内容仍呈现 code 颜色；疑似 Markdown fence 状态未在工具结果块结束后复位，或 tool result 渲染缓存/样式 span 泄漏到后续行 |
...
**状态**：修复中（#58 主管线已切换）。Assistant 消息的 fence 已在 `blocks/assistant_message.rs` 正确处理（`test_assistant_fence_does_not_leak_style_after_close`），但**工具结果**的 fence/markdown 尚未接线——`blocks/tool_call.rs::format_result_lines` 仅按行套用语义色、不解析 markdown fence，故工具结果内 fence 后续行的样式问题待 tool 结果 markdown 接线后确认。
