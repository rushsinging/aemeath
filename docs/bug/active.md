...
| 112 | spinner phase_text 先于实际内容数秒出现在 TUI | 中 | 待确认 | 待用户确认 | 2026-06 | `UiEvent::Text` / `UiEvent::Thinking` 只追加 conversation 内容，未同步推进 spinner phase；而其它路径会提前设置 Thinking/Generating，导致 phase_text 先出现、内容稍后才显现。修复：在 agent event adapter 中将 Text/Thinking 映射为“追加内容 + 同步设置对应 spinner phase”的同一批 intent，保证内容更新与 phase 切换同 tick 生效。验证：新增并通过 `test_map_agent_event_text_sets_generating_phase_with_text_update`、`test_map_agent_event_thinking_sets_thinking_phase_with_text_update`。 |
