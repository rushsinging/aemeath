...
| 51 | Output area 复制时复制出 Markdown 源码而非渲染后纯文本 | 中 | 待确认 | 未确认 | 2026-05 | 根因：get_selected_text 直接从 OutputLine.content 读取原始 Markdown 文本（含 **bold**、*italic*、`code` 等格式标记），而 TUI 渲染时通过 inline_markdown_spans 剥离了这些标记。修复：新增 strip_inline_formatting 函数剥离内联格式标记，在 get_selected_text 返回前应用；补充选择复制回归测试覆盖内联 Markdown 剥离与未闭合标记保留 |
