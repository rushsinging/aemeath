...
| 112 | spinner phase_text 先于实际内容数秒出现在 TUI | 中 | 待确认 | 未确认 | 2026-06 | spinner 中显示的 phase_text（如 Thinking/Generating）总是先出现，几秒后 TUI 输出区才出现对应的 assistant 内容，两者之间存在明显的视觉时序差 |
