...
| 116 | TaskListCreate 工具返回未带 task list ID，LLM 无法继续维护任务状态 | 中 | 待修复 | 未确认 | 2026-06 | 现象：LLM 在执行 `TaskListCreate` 工具创建任务列表后，工具结果只显示成功，但未返回任务列表 ID 或后续可用于 `TaskCreate` 的 listId/sessionId 字段，LLM 报告“无法获取任务 ID 继续维护状态”，必须等用户回复“继续”才能用 `TaskList` 重新查询。期望：`TaskListCreate` 工具结果中明确返回 task list ID、并在工具描述/result 中告知 LLM 下一步可直接调用 `TaskCreate` 而不需重新查询。 |
