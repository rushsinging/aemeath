...
| 112 | spinner phase_text 先于实际内容数秒出现在 TUI | 中 | 待确认 | 未确认 | 2026-06 | spinner 中显示的 phase_text（如 Thinking/Generating）总是先出现，几秒后 TUI 输出区才出现对应的 assistant 内容，两者之间存在明显的视觉时序差。用户希望 TUI 渲染更实时。**分析**：`UiEvent::Text` 到达时（`ui_event.rs:19-24`）spinner 立即设为 `Generating`，但 `Text` 事件的内容没有同时追加到 TUI 输出区——需要确认 `Text` 事件的处理中是否缺少对 conversation model / output 的即时更新。同理 `Thinking` 事件（`ui_event.rs:26-33`）只设 spinner phase，thinking 内容可能也未实时渲染到输出区。**关键代码**：`apps/cli/src/tui/app/update/ui_event.rs:19-34`（Text/Thinking 事件处理），`apps/cli/src/tui/app/run_loop.rs:119`（Enter 后设 Thinking phase） |
...
**复现路径**：
1. 在 TUI 中发送一条消息，等待 agent 开始处理
2. 处理期间继续输入文本并发送（消息进入 input queue）
