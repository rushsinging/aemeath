...
> 对应 Issue: #454（统一 ReasoningLevel）、#502（effort 越界 + Anthropic budget_tokens 废弃）
>
...
| **Volcengine** | Medium | `thinking=disabled` | `reasoning.effort=low` | `reasoning.effort=medium` | ⚠️ 越界 | ⚠️ 越界 | ⚠️ 越界 | 3 档（off/low/medium） |
| **MiniMax** | Medium | `thinking=disabled` | `thinking=adaptive` | `thinking=adaptive` | ⚠️ 越界 | ⚠️ 越界 | ⚠️ 越界 | 2 档（off/adaptive） |
| **Mimo** | Medium | `thinking=disabled` | `thinking=enabled` | `thinking=enabled` | ⚠️ 越界 | ⚠️ 越界 | ⚠️ 越界 | 2 档（off/enabled） |
| **Agnes** | High | `enable_thinking=false` | `=true` | `=true` | `=true` | ⚠️ 越界 | ⚠️ 越界 | 2 档（off/enabled） |
| **Ollama** | Medium | `reasoning=false` | `reasoning=true` | `reasoning=true` | ⚠️ 越界 | ⚠️ 越界 | ⚠️ 越界 | 2 档（off/enabled） |

> **⚠️ 越界**：表示该档位超出 provider 声明的 `max_reasoning_level()`。标记 #502 的缺陷——`loop_runner.rs:506` 在 graph 动态调节时**未做** `max_reasoning_level()` clamp，会原样发送越界值。bootstrap 初始化时已正确 clamp，但运行时路径缺失。详见 §3.3。
...
if let Some(ref graph) = reasoning_graph {
    if graph.enabled() && client.is_reasoning() {
        // ⚠️ 已知缺陷 #502：此处缺少 provider max_reasoning_level() clamp
        // 设计上应做三层 clamp（见 §2.4 三层 clamp），但实际实现为：
        //   client.set_reasoning_level(graph.current_effort());
        // 缺少 .clamped_to(client.max_reasoning_level())
        // 修复后应为：
        let clamped = graph.current_effort()
            .clamped_to(client.max_reasoning_level());
        client.set_reasoning_level(clamped);
    }
}
...
|---|---|---|---|
| Anthropic | 6 档（budget tokens 线性映射） | `thinking.budget_tokens`（⚠️ 废弃中，#502） | 迁移到 `output_config.effort` 后恢复 5 档 |
| OpenAI | 4 档（off/low/medium/high） | `reasoning.effort` | max_level=High |
...
| Volcengine | 3 档（off/low/medium） | `reasoning.effort` 或 `thinking.type` | max_level=Medium |
