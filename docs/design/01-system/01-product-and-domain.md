...
> 本文定义 aemeath 的产品目标、要解决的核心问题，以及领域的子域划分（问题空间）与 Bounded Context 清单（解决方案空间）。
...
| **编排（Workflow / Orchestration）** | 用图/状态机调节与编排 agent 行为。当前已落地 reasoning graph（effort 阶段调节）；多-agent 图编排是 **v0.2.0（Workflow Graph MVP）** 的目标。 |
...
Bounded Context 是**解决方案空间**的边界，一个 BC 内部维持一套一致的模型与统一语言。本项目共 **15 个 BC**（2 核心 + 7 支撑 + 6 通用），与上述子域基本 1:1 映射。

| # | Bounded Context | 子域类型 | 当前代码落点 | 备注 |
|---|---|---|---|---|
| 1 | **Agent Execution** | 核心 | `runtime/business/{agent,chat/looping}` | 含 Loop Engine 模块、唯一状态机 AgentRun、tool/model/interaction 编排 |
| 2 | **Workflow / Orchestration** | 核心 | `runtime/business/reasoning_graph` | 已落地 reasoning graph；多-agent 编排属 v0.2.0 |
| 3 | **Context Management** | 支撑 | `runtime/business/{compact,memory_inject,session,prompt/build}` + `prompt` crate | 含 Session 对话历史聚合、compact 家族、token 预算、记忆注入、prompt/guidance、会话身份 |
| 4 | **Memory** | 支撑 | `storage/business/memory` + `runtime/business/reflection` | 含 Reflection |
| 5 | **Task Management** | 支撑 | `shared/task` + `storage/business/task` + `tools/task_*` | 独立聚合 + 状态机 + 依赖图不变量 |
| 6 | **Project / Workspace** | 支撑 | `project/**` | worktree、git 上下文 |
| 7 | **Policy** | 支撑 | `policy/**` | 权限评估 |
| 8 | **Audit** | 支撑 | `audit/**` + `runtime/business/cost` | 含 Cost / Usage / Pricing |
| 9 | **Tool & Skill & Command** | 支撑 | `tools/**` | 含 MCP / skill / slash command |
| 10 | **Provider** | 通用 | `provider/**` | 内部 ACL |
| 11 | **Hook** | 通用 | `hook/**` | — |
| 12 | **Storage** | 通用 | `storage/**`（gateway/api 层） | 持久化机制 |
| 13 | **Config** | 通用 | `shared/config` + `runtime/core/config_*` | 含 reasoning 静态阈值 |
| 14 | **Application Version Control** | 通用 | `update/**` | 原 update，扩展版本渠道 / 升级策略 |
| 15 | **Logging** | 通用 | `packages/global/logging` | — |

> **交付层（非 BC）**：CLI / TUI / REPL（退役中）/ Server（设计中）是**入站适配器**，不占 BC 名额。`packages/sdk` 是核心域的**入站端口 + Published Language**，所有权归 Agent Execution。详见 `03-context-map.md` 与 `04-system-architecture.md`。
...
5. **Workflow 为 v0.2.0 预留**：reasoning graph 是 Workflow BC 的已落地雏形；多-agent 图编排在 v0.2.0 扩展。设计上 AgentRun 的触发源必须抽象（用户输入 / 父 AgentRun / 未来编排器），以便 v0.2.0 加编排层时 Agent Execution 不改。

## 6. 相关文档

- 统一语言：`02-ubiquitous-language.md`
- 上下文地图与集成关系：`03-context-map.md`
- 系统架构与六边形：`04-system-architecture.md`
