...
| 子域 | 说明 |
|---|---|
| **Agent 执行（Agent Runtime）** | 驱动"推理 → 工具 → 观察"循环、维护单次执行的状态机、编排模型调用与工具执行。这是产品的心脏。 |
...
|---|---|---|---|
| 1 | **Agent Runtime** | 核心 | Loop Engine、唯一状态机 Run、tool / model / interaction 编排 |
...
1. **单状态机**：整个系统只有一个领域状态机——`Run`（Agent Runtime 内），且**内存态、不持久化、崩溃从头开始**。Session 不是状态机，是数据聚合。
...
5. **Workflow 为 v0.2.0 预留**：reasoning graph 是 Workflow BC 的雏形；多-agent 图编排在 v0.2.0 扩展。设计上 Run 的触发源必须抽象（用户输入 / 父 Run / 未来编排器），以便 v0.2.0 加编排层时 Agent Runtime 不改。
