...
> **aemeath 采用 Hexagonal + Clean 作为 crate 内部默认组织方式；仓库外层按 BC / feature crate 组织。**

这项决策包含三层含义：

1. 仓库外层继续按稳定业务能力（Bounded Context）组织 feature crate，让顶层名称直接表达系统做什么；
2. **crate 内部默认采用 Hexagonal 依赖方向**（`domain ← application ← ports ← adapters`）：领域策略 NEVER 依赖技术细节，出站 port 隔离易变外部 detail，入站 façade / OHS 发布稳定语言。同一用例沿变化轴共置的代码 **MUST** 留在各自 Hexagonal 层内，**NEVER** 为共置跨越层边界。
3. 小模块 MAY 只使用部分层（如仅 `domain + adapters`），**NEVER** 为对称预建空层。技术外部 seam 出现时按证据定义消费方-owned outbound port；BC boundary seam 出现时按 Context Map 定义供应方-owned façade/OHS。只有 §3.8 至少一个强边界收益成立时才拆 crate。
...
| Hexagonal | 哪些交互跨越应用内外，如何隔离外部参与者 | **MUST** 作为 crate 内部默认依赖方向（`domain ← application ← ports ← adapters`）；易变 detail 的出站 seam **MUST** 由消费策略拥有目的明确的 port；对其他能力开放的入站 façade / OHS **MUST** 由供应能力拥有 | 每个模块四层全建；每函数一个接口；左右或上下分层图 |
...
4. 一次用例变化需要共同修改的策略、校验、局部类型和测试 **SHOULD** 共置；仅因代码"类型相同"而分散到全局技术目录是 **NEVER** 允许的组织依据。共置 **MUST** 在各自 Hexagonal 层内发生——同一用例的 domain 逻辑共置于 `domain/`，adapter 实现共置于 `adapters/`，**NEVER** 跨越层边界为共置而共置。
...
```text
crate 内部默认分层（Hexagonal 依赖方向）：
  domain/          ← 领域策略、不变量、Published Language（NEVER 依赖外层）
  application/     ← 用例编排（依赖 domain，经 ports 消费外部）
  ports/           ← 出站 port 定义（依赖 domain）
  adapters/        ← 技术实现、外部 detail（依赖 ports + domain）

可选升格（按证据触发）：
  ├── 可选：跨用例共享不变量的 model（domain 内部）
  ├── 可选：按 provider / protocol 命名的技术目录（adapters 内部）
  ├── 可选：`capabilities/` 递归竖切（仅在 §3.1 证据成立时）
...
5. 判定需要递归竖切时，切片 **MAY** 位于模块私有 `capabilities/` 下，但 **MUST** 同步提供 capability 间依赖矩阵和守卫；每个切片内部仍 **MUST** 遵守 Hexagonal 依赖方向。模块根 façade 只受控 re-export 稳定 Published Language，切片之间 **NEVER** 穿透内部实现。

### 3.2 叶子：Hexagonal 最简形态

单一职责、少量文件、一个主要用例或一组紧密行为 **MUST** 先使用 Hexagonal 最简形态：

```text
src/
├── lib.rs             # 窄 façade + composition-only wiring
├── domain.rs          # 领域策略、不变量、Published Language
├── domain/            # 仅在多文件共同消费时展开
├── adapters.rs        # 技术实现（外部 detail 终止于此）
└── adapters/          # 仅在多 provider / protocol 时展开
```

- **MUST** 把测试放在被测行为附近。
- **NEVER** 为对称美观预建空层（如仅有一个 adapter 时 **NEVER** 预建 `ports/`）。
- 当一次修改经常跨越三个以上互不相关的职责，或不同用例拥有独立词汇、状态与测试夹具时，**SHOULD** 回到 §3.1 递归划分能力。

### 3.3 可选 `capabilities/` 递归竖切

> **本节降级为可选结构。** 大多数单能力 crate **NEVER** 需要 `capabilities/`。仅在 §3.1 判定存在多个拥有独立词汇、变化轴、状态所有权和测试夹具的真实能力时才 **MAY** 引入。

切片名称 **MUST** 表达用例或稳定能力，例如 `session`、`compact`、`atomic_blob`，**NEVER** 只表达代码的技术形态。引入 `capabilities/` 时 **MUST**：

```text
src/
├── lib.rs                         # 模块窄 façade
├── domain.rs                      # 跨 capability 共享的 PL / 不变量
├── capabilities.rs                # 私有切片注册，不承载业务逻辑
└── capabilities/
    ├── capability_a.rs             # 切片 façade + 该 capability 的 domain/application
    ├── capability_a/
    │   └── adapter.rs              # 该 capability 的技术 adapter
    └── capability_b.rs
```

- **MUST** 为每个 capability 声明其 Hexagonal 层（domain / application / ports / adapters），哪怕只用了两层。
- **MUST** 提供 capability 间依赖矩阵（谁可 import 谁）并配套机械守卫。
...
### 3.9 COLA → Hexagonal 层名映射

现有 COLA 层（`api / business / contract / core / gateway`）逐步退役，按以下映射迁移到 Hexagonal 层名：

| COLA 层 | Hexagonal 层 | 语义 |
|---|---|---|
| `contract` | `domain`（部分） | 领域类型 / Published Language 定义 |
| `business` | `domain` + `application` | 领域逻辑（→ `domain`）+ 用例编排（→ `application`） |
| `core` | `adapters` | 基础设施 / 技术实现 |
| `gateway` | `ports` + `application` | 对外 façade（→ `ports`）+ 用例入口（→ `application`） |
| `api` | `application` | 用例路由 |

迁移 **MUST** 按上述映射重命名目录并调整 `use` 语句；**NEVER** 仅改目录名而不更新 import。迁移完成后 COLA 层名 **NEVER** 再出现。具体迁移顺序和进度见 [Migration Governance](../03-engineering/03-migration-governance.md)。
...
以下 Rust 树统一使用 Hexagonal 模块布局（`domain / application / ports / adapters`），小模块 MAY 只使用其中部分层，**NEVER** 表示每个同类模块都必须复制全部四层。

### 4.1 小型 Policy：Hexagonal 最简形态

```text
src/
├── lib.rs             # 窄 façade + composition-only wiring
├── domain.rs          # PolicyRequest、PolicyDecision（PL）
└── adapters.rs        # AllowAll adapter 及就近单元测试
```

此投影对应 [Policy 模块设计](../02-modules/policy/README.md) 的小型评估能力：`PolicyRequest` 与 `PolicyDecision` 由 `domain.rs` 发布，`AllowAll` 行为在 `adapters.rs` 就近组织。小型 Policy **SHOULD** 只用 `domain + adapters` 两层，**NEVER** 为尚不存在的规则引擎预建 `ports/` 或 `application/`；未来多个决策用例共享真实规则不变量时才 **MAY** 提取 model 到 `domain/` 展开子模块。

### 4.2 Provider：Hexagonal + 按 provider / protocol 命名技术目录

```text
src/
├── lib.rs                     # 窄 façade + composition-only wiring
├── domain.rs                  # 领域策略：driver capability、model 能力解析
├── domain/
│   ├── capability.rs           # driver + model 能力解析
│   ├── invoke.rs               # 单次调用与统一流语义
│   └── error.rs                # wire / HTTP 错误 → ProviderErrorKind
├── ports.rs                   # 对外 port（Runtime-owned ProviderPort 实现签名）
└── adapters/
...
此投影与 [Provider 模块设计](../02-modules/provider/README.md) 对齐。`domain` 持有 capability 解析、invoke 语义和错误映射；`adapters` 按 provider / protocol 命名技术集成，把 wire type 与错误转换收在边界内。Runtime 消费方 **MUST** 拥有目的性 provider port，Provider adapter 实现该 port；Provider **NEVER** 把 HTTP / SSE 类型作为统一 façade 的 Published Language。若某集成只有一个文件，**MUST** 使用同名 `.rs` 文件而非空壳目录。

### 4.3 复杂 Runtime：Hexagonal 四层 + 内部按能力组织

```text
src/
├── lib.rs                          # 窄 façade
├── domain/                         # 领域策略、Run 状态机、Published Language
│   ├── agent_run.rs                #   Run 生命周期不变量
│   ├── agent_run/
│   │   ├── state.rs
│   │   └── step.rs
│   └── event.rs                    #   DomainEvent PL
├── application/                    # 用例编排
│   ├── agent_client.rs             #   入站 OHS façade（不是通用 api 层）
│   ├── loop_engine.rs              #   驱动单个 Run
│   ├── loop_engine/
│   │   ├── drive.rs
│   │   └── stuck_guard.rs
│   ├── model_invocation.rs         #   模型调用编排
│   ├── model_invocation/
│   │   └── retry.rs
│   ├── tool_coordination.rs        #   工具协调
│   ├── tool_coordination/
│   │   └── approval.rs
│   ├── context_coordination.rs     #   上下文协调
│   ├── context_coordination/
│   │   └── window.rs
│   └── interaction.rs              #   用户交互
├── ports/                          # 出站 port 定义
│   ├── provider.rs                 #   ProviderPort 签名
│   └── event_sink.rs               #   事件 sink
...
2. 单一用例或紧密行为 **MUST** 选择 Hexagonal 最简形态（`domain + adapters`）；已有独立词汇、共同变化与测试边界时，才 **SHOULD** 按 §3.1 评估是否需要 `capabilities/` 竖切或 `application` / `ports` 升格。大包若出现三个或更多候选能力，**SHOULD** 按 §3.1 递归复查，但最终仍以能力证据而非数量决定。
...
| 递归能力拆分，`capabilities/` 降格为可选结构（v2 修订）；Hexagonal 依赖方向成为 crate 内部默认 | [Vertical Slice Architecture](https://www.jimmybogard.com/vertical-slice-architecture/)；[Hexagonal Architecture](https://alistair.cockburn.us/hexagonal-architecture) | 能力识别仍按独立词汇、变化原因、状态与测试边界判定；统一依赖方向约束降低守卫成本并防止长期劣化 | 以文件数或“三个”阈值机械拆分；把 `capabilities/` 当成边界证据或默认容器 |
...
| **拒绝：每模块复制相同数量子目录**（v2 精确化：拒绝的是形式对称而非统一依赖方向） | [Clean Architecture](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html)；[Microsoft DDD guidance](https://learn.microsoft.com/en-us/dotnet/architecture/microservices/microservice-ddd-cqrs-patterns/ddd-oriented-microservice) | 保留依赖方向约束；小模块 MAY 只用部分层 | 拒绝原因：为对称预建空层给小模块增加仪式；但**统一依赖方向约束**不在此列——它是防偏移的最低成本守卫 |
