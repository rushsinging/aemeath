...
- **MUST** 用 DDD 战略设计识别子域、统一语言、Bounded Context、聚合与 Context Map（见 [01](01-product-and-domain.md)~[03](03-context-map.md)）。
...
- **MUST** 先在单 crate 内用 Rust 可见性稳定能力边界；只有编译器隔离或稳定 Published Language 确有价值时才 **MAY** 拆 crate。
...
- **判据**：先在单 crate 内用 Rust module privacy 稳定边界；只有确需编译器隔离、稳定 Published Language、独立构建目标或其他已说明的强边界收益时才 **MAY** 升格为独立 crate。
...
| 2026-07-14 | 架构总决策改为 capability-first + 用例共置 + 按需 port，以 Context Map 约束跨 BC 集成、明确共享 owner 与已确认外部 seam，并将 Runtime 具体拆分收敛到模块级真相源 | [#972](https://github.com/rushsinging/aemeath/issues/972) |
