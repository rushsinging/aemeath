...
- 为保持模块私有性，能力 **MAY** 暴露 composition-only 的 opaque production factory，并在 factory 内构造不跨模块公开的私有 detail；Composition Root **MUST** 独占“选择并调用哪个 factory”的权力。若实现选择需要配置或存在多个候选，选择逻辑 **MUST** 回到 Composition Root，或由它以稳定 factory 参数显式下发。
