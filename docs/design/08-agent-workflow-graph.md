...
pub struct WorkflowStep {
    /// 节点类型：explore / plan / execute / verify
...
fn execute(&self, input: Value, ctx: &ToolExecutionContext) -> ToolResult {
    let steps: Vec<WorkflowStep> = parse_and_validate(input)?;
...
```rust
fn execute(&self, input: Value, ctx: &ToolExecutionContext) -> ToolResult {
    let new_steps = parse_and_validate(input)?;

    if let Some(graph) = ctx.session.workflow_mut() {
        graph.replan(new_steps);
        ToolResult::success(json!({
            "replanned": true,
        ...
        // 首次建图
        let graph = WorkflowGraph::new(new_steps);
        ctx.session.set_workflow(graph);
        ToolResult::success(json!({
            "registered": true,
            "steps": graph.steps_summary(),
...
```
Idle → (LLM 调用 PlanWorkflow) → Explore → Plan → Execute → Verify → Idle
                                                              ↑
                                                     (replan 插入新步骤)
```
...
```
原始图: [explore, plan, execute, verify]
                    ↑ cursor=2, 在 execute 节点
...
replan 后:
  ...
  cursor: 0  ← 重置到新图起点
  replan_count: 1
  original_steps: [explore, plan, execute, verify]  ← 保留
```
