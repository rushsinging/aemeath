...
pub struct WorkflowGraph {
    ...
    /// delivering 中间态：cursor 进入 deliver / 走完末节点时设为当时的 cursor 值，
    /// 等待 Stop hook 仲裁。Stop hook 放行 → 真正销毁图；阻止 → 回滚 cursor 到此值。
    /// None 表示不在 delivering 态。
...
- **`delivering: Option<usize>`**：deliver / 走完末节点时的中间态。进入此态后图**不立即销毁**，而是等待 Stop hook 仲裁。详见 §2.11。
...
fn execute(&self, input: Value, ctx: &ToolExecutionContext) -> ToolResult {
    ...
    if graph.current_node() == ReasoningNode::Deliver || graph.cursor >= graph.steps.len() {
        // 不立即销毁图——进入 delivering 中间态，等待 Stop hook 仲裁
        graph.begin_delivering();
        let next_node = graph.current_node();
        return ToolResult::success(json!({
            "delivering": true,
            "finished_node": finished_node,
            "next_node": next_node,
            "message": "工作流已到达交付点，等待 Stop hook 仲裁",
        }));
    }
...
Turn 开始:
  ① 检查 session.workflow
     ├─ 有图且 delivering → 使用 deliver 节点 effort（Medium）
  ...
  ③ LLM 返回响应，执行 tool calls
     ├─ 含 WorkflowPlan tool call → 执行（注册/replan 图，清除 delivering 态）
     ├─ 含 WorkflowNext tool call → 执行（推进 cursor）
     │   ├─ 推进到 deliver 或走完所有步骤 → 图进入 delivering 态（不销毁）
     │   └─ 推进到其他节点 → cursor++，下一轮使用新节点的 effort
     ├─ 含其他 tool call → 执行 → 记录到当前 cursor 节点的 NodeRecord
     └─ 纯文本回复（无 tool call）→ 图状态不变，cursor 不动

  ④ Agent loop 终止判定（LLM 输出纯文本，无 tool call）
     ├─ 无图 或 非 delivering → 正常触发 Stop hook
     │   ├─ Stop hook 放行 → agent loop 终止
     │   └─ Stop hook 阻止 → 注入 continue 消息，回到 ①
     └─ 有图且 delivering → 触发 Stop hook
         ├─ Stop hook 放行 → 真正销毁图（session.workflow = None），agent loop 终止
         └─ Stop hook 阻止 → 回滚 cursor 到 delivering 前的位置，清除 delivering 态，
             注入"Stop hook 阻止交付：<原因>"消息，回到 ①
...
```
Idle → Start → Explore → Plan → Execute → Verify → Delivering → Idle
       ↑                ↑                       ↑          ↑
       (cursor 初始)    (replan 可插入)         (进入中间态) (Stop hook 放行)
                                                 或走完末节点
                                                 ↓ Stop hook 阻止
                                              回滚 cursor，继续工作
```
...
- **图的推进**：LLM 调 `WorkflowNext`，cursor++ 到下一个节点
- **图的 delivering 态**：cursor 进入 `deliver` 节点，或 cursor 走完最后一个工作节点（未声明 deliver）。图**不立即销毁**，等待 Stop hook 仲裁
- **图的终止（销毁）**：delivering 态下 Stop hook 放行 → 真正销毁图
- **图的回滚**：delivering 态下 Stop hook 阻止 → cursor 回滚到 delivering 前的位置，清除 delivering 态，LLM 继续工作
- **图的销毁（非正常）**：session 结束。图持久化到 session，resume 时恢复。
...
### 2.11 Stop Hook 与 Delivering 交互

#### 问题

Graph 的交付终止（cursor 进入 deliver / 走完末节点）和 agent loop 的终止（Stop hook）是两个独立决策点。如果 Graph 直接销毁、Stop hook 随后阻止，LLM 会处于"图没了但还要继续"的矛盾状态。

#### 方案：delivering 中间态

引入 `delivering: Option<usize>` 字段，解耦 **图标记完成** 和 **图真正销毁**：

```
LLM 调 WorkflowNext → cursor 进入 deliver / 走完末节点
  → graph 进入 delivering 态（delivering = Some(deliver_cursor)）
  → WorkflowNext tool_result 返回 { "delivering": true }
  → 图仍然存在，cursor 不再前进

LLM 输出纯文本（无 tool call）→ agent loop 尝试终止
  → 触发 Stop hook
  ├─ Stop hook 放行 → 销毁图（session.workflow = None）→ agent loop 正常终止
  └─ Stop hook 阻止 → 回滚 cursor 到 delivering 前的位置，清除 delivering 态
      → 注入"Stop hook 阻止交付：<原因>"消息
      → LLM 继续工作（可修复问题后再次推进到 deliver）
```
...
impl WorkflowGraph {
    /// 进入 delivering 中间态
    pub fn begin_delivering(&mut self) {
        self.delivering = Some(self.cursor);
    }

    /// Stop hook 放行：调用方负责销毁图
    pub fn is_delivering(&self) -> bool {
        self.delivering.is_some()
    }

    /// Stop hook 阻止：回滚 cursor 到 deliver 前的位置，清除 delivering 态
    pub fn rollback_delivering(&mut self) {
        if let Some(prev_cursor) = self.delivering {
            self.cursor = prev_cursor;  // 回到 deliver / 末节点
            // 回退到前一个工作节点（deliver 本身不算工作节点）
            if self.cursor > 0 {
                self.cursor -= 1;
            }
...
#### replan 清除 delivering

如果 delivering 态下 LLM 主动调 `WorkflowPlan` replan（而非等 Stop hook），replan 逻辑 **MUST** 清除 delivering 态：

```rust
pub fn replan(&mut self, new_steps: Vec<WorkflowStep>) {
    self.steps = new_steps;
    self.cursor = 0;
    self.delivering = None;  // 清除 delivering 态
    self.replan_count += 1;
    // original_steps 不变
}
```
...
| 场景 | 处理 |
|---|---|
| delivering 态下 LLM 调 WorkflowNext | 忽略——已在 delivering 态，cursor 不动 |
| delivering 态下 LLM 调其他 tool（如 Edit） | 正常执行——delivering 不阻塞 tool |
| delivering 态下 session 被 compact | 图保留，delivering 态保留 |
| delivering 态下 session 结束 | 图持久化，resume 时恢复 delivering 态 |
| 无图时 Stop hook 阻止 | 正常 agent loop 行为，与 graph 无关 |
...
| `agent/features/runtime/src/business/chat/looping/loop_runner.rs` | 执行循环改造：cursor effort 联动 + Workflow tool 优先执行 + delivering 中间态与 Stop hook 交互 |
...
### 5.7 Stop Hook 与 Graph Delivering 的反复震荡

**风险**：LLM 到达 deliver → Stop hook 阻止 → 回滚 → LLM 修复后再次 deliver → Stop hook 再次阻止 → 循环。

**缓解**：
- Stop hook 阻止时注入的原因消息引导 LLM 修复具体问题
- `replan_count` 不计入 delivering 回滚（回滚不是 replan），但可额外追踪 `deliver_rollback_count` 用于未来限速
- aemeath 的 Stop hook（`check-architecture-guards.sh`）是确定性的——要么通过要么不通过，不会随机阻止
...
| graph 走完后是否自动销毁？ | 否——进入 delivering 中间态，Stop hook 放行后才销毁 | delivering 反复回滚的限速策略 |
