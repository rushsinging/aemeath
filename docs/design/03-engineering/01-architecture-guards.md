...
- **故意违规证据**：隔离副本依次注入宽泛 `RuntimeDataUpdated`、未登记事件、跨层 fact 漂移、SDK-only compatibility event（`ToolCallUpdate`、`ToolCallStart`、`ToolProgress`、`MicrocompactDone` 或 legacy `Text`/`Thinking`）恢复为 Runtime producer、Lifecycle terminal 风格 ACK 和 stringly `CompactProgress`；每类均必须由单 Guard 拒绝，恢复后 clean pass。
