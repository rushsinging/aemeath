...
- **兼容边界**：`legacy_broad_names` 显式冻结仍在生产使用的 compatibility names，不进行 SDK wire breaking rename；`sdk_compatibility_variants` 允许旧 SDK variant 继续 dual-read，但 Guard 要求其不得恢复为 Runtime producer 或 TUI fact；新增宽泛 `*Updated`、`*Info`、`*Data`、`*Notification`、`*ProgressUpdated` 仍会被拒绝。
...
- **故意违规证据**：隔离副本依次注入宽泛 `RuntimeDataUpdated`、未登记事件、跨层 fact 漂移、SDK-only compatibility event 恢复为 Runtime producer、Lifecycle terminal 风格 ACK 和 stringly `CompactProgress`；每类均必须由单 Guard 拒绝，恢复后 clean pass。
