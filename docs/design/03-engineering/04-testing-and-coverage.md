...
| L2 | 模块协作测试 | 同一 crate 内 service、port、reducer、assembler 的协作 | `src/<owning-layer>/<module>/tests/` |
...
一个文件对应一个稳定行为或用户故事，测试目录通过正常 Rust 模块树挂载。不分层 crate 可省略 `<owning-layer>`；采用 Hexagonal Architecture 的 crate 则必须使用真实 owning layer，例如 Runtime 的 `domain/<module>/tests/`、`application/<module>/tests/` 或 `adapters/<adapter>/tests/`。测试目录 **NEVER** 引入第二套 VSA，也不得跨 `domain/application/ports/adapters/shared` 建立万能测试层。
...
```text
agent/features/runtime/src/domain/<module>/testing/
agent/features/runtime/src/application/<module>/testing/
agent/features/runtime/src/adapters/<adapter>/testing/
agent/features/provider/src/<owning-layer>/<module>/testing/
apps/cli/src/tui/testing/
```

Runtime 的 Target 结构采用 `domain/application/ports/adapters/shared` 六边形分层；fixture、Fake 和 Scripted Driver 必须留在其真实 owning layer。只有被同一 crate 多个层共同消费、且确实无领域和边界语义的基础能力，才可评审后放 crate-level `src/testing/`。

**NEVER** 建立知道所有领域类型、跨越架构层或构成第二套 VSA 的万能 `test_utils` / `testing`。共享基础设施仅承载真正跨领域且无业务语义的能力，例如确定性 ID 或临时目录封装。
...
- inline、同级 `*_tests.rs`、owning-layer 模块 `tests/`、crate integration test 与场景测试目录规则；
...
| 2026-07-15 | 将 L0-L5、覆盖证据、目录、命名、fixture 与确定性规则同步到 Rust 编码规范，并按 Runtime 六边形 Target 收敛测试归属 | [#1013](https://github.com/rushsinging/aemeath/issues/1013)、[#1023](https://github.com/rushsinging/aemeath/pull/1023) |
