...
一个文件对应一个稳定行为或用户故事，测试目录通过正常 Rust 模块树挂载。不分层 crate 可省略 `<owning-layer>`；采用 Hexagonal Architecture 的 crate 则必须使用真实 owning layer，例如 Runtime 的 `domain/<module>/tests/`、`application/<module>/tests/`、`ports/<seam>/tests/` 或 `adapters/<adapter>/tests/`。`shared` 仅在存在真实共享内容时按需创建；测试目录 **NEVER** 引入第二套 VSA，也不得建立跨层万能测试层。
...
agent/features/runtime/src/ports/<seam>/testing/
agent/features/runtime/src/adapters/<adapter>/testing/
agent/features/runtime/src/shared/<utility>/testing/  # 仅当 utility 真实属于 shared
