...
| #677 issue-tree | sub-issue 调整、PR 前后、父 Issue 关闭前显式执行 | 依赖 GitHub 网络，**NEVER** 进入通用 pre-commit |
...
- `.cargo/hooks/pre-commit` 仅运行 staged fmt、条件 source guard 和 snapshot 草稿检查；#677 issue-tree 网络校验不进入通用 hook。
- #677 双向校验使用 `cargo run -p xtask -- verify-issue-tree 677`，仅在生命周期关键节点显式执行。
