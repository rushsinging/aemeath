...
| Main/Sub 使用同一 Loop 与同一 HookPort，终态规则一致 | L0 / L2 / L4 | `runtime/src/application/loop_engine/engine_architecture_tests.rs`、`application/run/context_factory_tests.rs::{main_run_uses_hooks_from_its_committed_config_snapshot,sub_run_uses_its_committed_hooks_while_parent_hooks_remain_frozen}`、共享 Run/Loop scenario、Composition wiring tests | 统一引擎已有结构证据，Main/Sub 代表装配与场景测试均通过。 |
...
| line/region/function coverage 与 production reachability 分开记录 | L0 | `./scripts/coverage.sh` 的 workspace/per-crate summary；production reachability 独立 gate | 最终实测 Hook regions/functions/lines **83.26% / 95.19% / 88.39%**，Runtime **81.52% / 82.37% / 83.17%**，SDK **78.00% / 77.02% / 76.61%**，CLI **82.10% / 84.69% / 81.83%**，workspace **82.06% / 82.59% / 82.93%**。统一脚本没有 changed-lines 指标，百分比只作风险信号，不替代矩阵。 |
...
- **已闭合实现缺口**：#1312 已将 `hooks.max_attempts` 与 `hooks.max_stop_hook_blocks` 收敛为 `ConfigSnapshot` 的 typed policies；Dispatcher 捕获 `HookExecutionPolicy`，每个 Main/Sub Run 从 `RunConfigSnapshot` 冻结 `StopHookPolicy`。#1543 已将 non-Unix 平台能力缺失收敛为 terminal typed `Unsupported → ExecutionFailed` 且不重试。
...
4. 最终本地 `cargo fmt --all -- --check`、`git diff --check`、Hook/Runtime/SDK/TUI 定向测试、`cargo test --workspace`、`cargo check --workspace`、all-targets Clippy、production reachability、full architecture guards 与 `./scripts/coverage.sh` 均真实通过；本轮首次执行无失败后重跑。`windows-latest` 的 non-Unix compile/typed unsupported/no-retry 证据来自 PR #1554，不能由 Unix 或本机 all-targets 替代。
5. Coverage 最终值：Hook regions/functions/lines **83.26% / 95.19% / 88.39%**，Runtime **81.52% / 82.37% / 83.17%**，SDK **78.00% / 77.02% / 76.61%**，CLI **82.10% / 84.69% / 81.83%**，workspace **82.06% / 82.59% / 82.93%**。
