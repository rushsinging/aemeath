...
| O2 | `check-cola-layer-purity.sh` 仍在 Stop 阶段运行；其检查行为、常量与白名单见 [Architecture Guards](01-architecture-guards.md)。#1002 已确认 Composition Root 当前 `app/provider/runtime/tools/update` 扁平 wiring modules 符合 capability-first；#948 已让 `FeatureGateways` 的 Provider/Tool factory 注入真实进入 Runtime 主 bootstrap，并由 `check-composition-layout.sh` 的零例外正向断言锁定；#1292 已将 Session list/export/import/metadata/delete 从 Context crate-root façade 迁为 `SessionManagementPort`，由 Composition 以同一 AtomicBlob backing 分发给 MainSessionWiring 与 Runtime，并以 ownership Guard 锁定；#1293 已将 Config override backing 的 filesystem 构造上移至 Composition，Config wiring 显式接收 `NativeConfigStore`，并以 ownership Guard 锁定；#1294 已将 Tool Catalog/Execution/binding、Skill Catalog/materializer、Tool Result materializer 与 ActiveRunRegistry 的生产装配上移至 Composition，Runtime 只消费 injected dependencies，并以 ownership Guard 锁定。MCP Ready 生命周期和动态 Catalog 同步因尚无稳定端口而由 #1327 承接。Composition 不机械复制 feature crate 的 Hexagonal/COLA 层 | 由守卫机械验证 capability-first 新规范的窄公开面、跨 feature 依赖、循环依赖与 Composition Root 装配；Composition 继续按装配职责扁平分片，语义接线完整性不由目录形状冒充 | [#982](https://github.com/rushsinging/aemeath/issues/982) 是 #763 的原生实现 leaf；替代守卫证据齐备前 legacy guard **MUST** 保持运行。#948 只闭合既有 Provider/Tool gateway 的主 bootstrap 消费；Tool 双端口生产切线现已由 #911 完成，但全部 Adapter 构造上移仍由 #950 承接，Provider P1/#907 与正式 capability-first 边界 #1022 继续开放 |
...
| R10 | **Session `messages` / `chats` / structured-run 迁移进行中** | #872 删除 live `Session.messages` 与旧 writer；#1282 将 v2 canonical writer 切至 `run_slices + ActiveCompactMarker`，`chats/messages/cwd` 只留 legacy reader DTO；#1277 已让 `freeze_step` 后的 accepted user input 经独立 typed receipt 写入 `CommittedRunStep.accepted_input`，Context backing 在同一 mutation gate 内完成幂等/冲突检查与 durable publish，Main/Sub outcome bridge 不重复 input；#1278 将 outcome bridge 收口为带 cause/receipt/usage/fingerprint/revision 的 `FinalizedOutcomeProjection`，并升级 v2 vector envelope | ContextWindow、resume、list 与 compact 只读取 marker 后的 `run_slices`；#1247 复用 #1278 schema 接入生产 control command，#879 退役旧 runtime 路径 | #1282（schema、marker、legacy upgrade、读取切换）→ #1277（accepted input）→ #1278（outcome）→ #1247 / #879 |
...
该完成口径不外推：#878 的完整 Interaction identity、continuation、`AwaitingUser` / resume / cancel 状态机仍未完成；#879 继续承接旧 looping UI/Hook adapter、重复执行 helper 以及 Tools Context 中 progress/plan 资源的最终 Runtime 所有权收口；#912/#913 仍承接 Runtime/Composition ownership 与装配收口。#914 已物理删除 `legacy-no-agent`、历史 `register_all_tools*`、旧内部 Registry/Profile、legacy Skill DTO 与 `SkillTool`；MCP Ready 的显式连接状态机、断连撤销/refresh、稳定身份、版本与 Catalog revision 协议仍开放。

| # | 缺口 | 现状 | 目标 | 迁移阶段 |
|---|---|---|---|---|
| T1 | **Catalog / Execution 生产双端口与旧 gateway 已收口** | #911 已由 Tools composition factory 在同一私有 backing 上装配双端口；#914 删除 Composition/Runtime 中创建后即丢弃的 `ToolCatalogGateway` / `ToolRegistry` 注册链，Main/Sub 只持 Catalog/Execution trait object 与 PL | 保持 Descriptor/Invocation/Outcome 唯一跨界语言，禁止恢复直接注册入口 | 已完成（#911/#914） |
| T2 | **Scope/Profile 与历史入口已物理退役** | #914 删除 legacy-no-agent、历史 `register_all_tools*`、旧 Registry gateway、无消费者 Profile 兼容面与 legacy Skill DTO；正式 Main/Sub 继续以 capability allow-set 和 `derive_restricted` 双端复验 | Profile 始终只收缩、不扩权；任何新 Scope 必须有明确 RunSpec 语义 | 已完成（#909/#911/#914） |
...
| T8 | **Skill Catalog / Materialization 与 legacy SkillTool 已收口** | #912 建立 Skill-owned `PromptFragment`、Catalog/Materialization ports 与 filesystem adapter；#914 删除 legacy-no-agent、SkillTool 文件、legacy Skill DTO 及 SDK/TUI 的执行型 Skill 投影。历史会话中的 `Skill` ToolCall 仅由 TUI 展示兼容读取，不再构成可执行 Tool | 保持 Skill 不取得 Tool execution capability | 已完成（#912/#914） |
| T9 | **Slash Command Catalog / Router 已切线，Skill Slash 投影与目标 BC handler 继续分阶段收口** | #913 已建立 Tools-owned `CommandDescriptor` / `CommandCatalogPort` / `CommandRouterPort` 与三类 typed route；SDK 直接 re-export 唯一 PL，Composition 将同一 Catalog/Router 注入 TUI/no-TUI。#1302 分离 Skill stable identity / identity aliases 与显式 Slash name / slash aliases：package namespace Skill 仍可物化但默认不进入 PromptInjection Descriptor，避免外部 Skill 元数据阻断 bootstrap。帮助、补全、`/exit`/`/reflect` 和未知命令共用 Router，旧 `builtin_commands` / 静态帮助 / 独立 parser 已删除。现有 Runtime `ChatInputEvent → PendingCommand` 仍作为目标 BC handler adapter 执行 ApplicationControl，不再承担 slash 名称发现或解析 | 保持 Tools 为 Command PL/目录/路由唯一 owner；PromptInjection 只物化 Skill-owned PromptFragment，SnapshotQuery/ApplicationControl 的最终 typed Snapshot/Outcome handler 随各 owner Issue 收口；#947 完成 TUI Effect 化，#878/#879 退役旧 Loop helper | #913（Catalog/Router 完成）；#1302（Skill Slash 投影）；#947（TUI slash I/O 全 Effect 化）；#878/#879（handler/Loop 退役）；#914（legacy 物理退役） |
...
| M5 | **Reflection 领域与执行/TUI 语义部分收口** | #898 迁移 PL、prompt/schema/parse/format/apply；#1284 已将 PreCompact 切入 Runtime 单槽异步协议；#1285 已实现有界 teardown/cancel；#900 删除文档中的旧 `MemoryStore` apply 示例，确认 `ReflectionEngine` 是当前无状态 `ReflectionPromptPort` 实现而非第二 active Memory state。Manual 显式入口由 #1289（归 #860）承接 | 不得恢复同步执行、第二 store 选择或结果正文投影；Manual 完成前不得声明三 trigger 语义已收口 | #898/#900/#1284/#1285 已完成；#1289（#860）待完成 |
...
| M9 | **NoOpMemory、Sub Disabled 与 Reflection 编排部分落地** | #897 发布 owner-owned NoOpMemory；Sub ToolResources 明确装配 NoOp，Composition MemoryMode::Shared 仅 clone active Arc；#1284 已接 PreCompact 单槽生产编排与 history view，#1285 已收口 teardown。Manual 显式入口由 #1289（归 #860）承接 | #900 删除 Runtime 重复 MemoryPort 骨架；不改变已完成的 NoOp/Shared 语义；Manual 未完成前不得冒充完整 Reflection 编排 | #897/#900/#1284/#1285 已完成；#1289（#860）待完成 |
...
| PHA4 | **Hook 单一公开契约已对齐** | #926 删除 `hook::api`、`adapters::legacy::HookRunner`、旧 DTO 与 Runtime concrete runner 持有；Runtime/Main/Sub 改为消费 Hook crate-root `HookPort` / `HookOutcome`，并由 `check-hook-target-facade.sh` 与 crate API façade policy 防回流。 | #1216 继续收口 env_clear / 白名单；#1312 继续将 Hook retry 与 Stop block 上限注入 ConfigSnapshot。 | 已完成（#925/#926） |
| PHA5 | **类型化阻断与投影契约已对齐** | #926 已删除 `emit_json_hook_context`、PostToolBatch 与旧 `HookUi` 的 JSON / stdout 推断；Runtime 统一投影 `HookOutcome`，逐条发布 `RuntimeStreamEvent::HookMessage`，保留 HookPoint/source/ordinal/attempt/kind。 | SDK/TUI 已消费 `ChatEvent::HookMessage`：TUI 以状态色标题、主文本正文和弱化归因详情展示非空 message，空 payload 继续由展示层决定是否渲染。 | 已完成（#924/#925/#926） |
| PHA6 | **非零 exit 语义已对齐** | #924/#926 将任意非零 exit（含 1/2/127）统一为主动 Block、只执行一次；已删除 shared 配置和 legacy runner 中“exit 2 阻断 / 其他非阻塞”的过期说明。 | 未来协议扩展只能修改 Hook-owned `classify_directive` 真值表与其测试，禁止 Runtime 恢复 exit-code 推断。 | 已完成（#924/#926） |
| PHA7 | **Hook Dispatcher 与 Runtime adapter 已完成生产切换** | #926 以 `HooksConfig → HookSubscription → Dispatcher` 作为唯一生产构造；每次 `HookInvocation` 显式携带当前 workspace cwd，adapter 按次投影兼容环境，Main/Sub/Prompt/Notification 均经 `HookPort`。 | #1216 负责 env_clear / 白名单；#1312 负责 ConfigSnapshot 上限；#879 仍负责共享 Loop 其他原子切换。 | 已完成（#923/#924/#925/#926） |
| PHA8 | **Stop Hook completion/continuation 终态控制待收口** | 当前兼容 Main Loop 已保留并提交 Stop block 前的 assistant Step，且将 feedback 与 hook 执行期间到达的普通用户追问合并为同一 continuation request；仍使用 5 次临时上限，且未具备 CancelRunStep / TerminateRun 的 out-of-band 控制入口。 | 依 #743 Target：终态采用 15 次上限与 `StopHookRetryExhausted`，CancelRunStep / TerminateRun 优先于 continuation，并由统一 StepFinalizer 收口。 | 当前修复 / S5 终态控制切换（#878/#1312） |
...
| legacy Registry/Profile/SkillTool 兼容路径 | #914 已删除 `legacy-no-agent`、历史 `register_all_tools*`、`ToolCatalogGateway`、无消费者 Profile/Registry 兼容实现、legacy Skill DTO 与 `SkillTool`；正式 Main/Sub 只经 `tools::composition::wire_builtin_catalog_execution` 装配 | `check-tool-catalog-execution-boundary.sh` 零例外拒绝旧符号；MCP 仅保守 source seam；Tools Context 的 progress/plan adapter 最终收口由 #879 承接 | 已完成（#914；progress/plan → #879） |
...
| 日期 | 变更 | 关联 |
|---|---|---|
| 2026-07-21 | #914 删除 legacy-no-agent、历史 Registry/Profile/SkillTool、legacy Skill DTO 与无效 ToolCatalogGateway 注册链；Tools crate-root façade和 Composition Guard 同步收窄，新增零例外 legacy-symbol Guard。progress/plan adapter 的 Runtime 所有权物理收口登记至 #879 | [#914](https://github.com/rushsinging/aemeath/issues/914)、[#879](https://github.com/rushsinging/aemeath/issues/879) |
| 2026-07-21 | #1060 最终验收 #851 Memory / Reflection 测试体系：#1299 的 mixed apply、PartialApply、history failure、trigger predicate 与 NoOp→active Arc 注入证据已由 #1308 合入；#1298 的 worktree PTY binary 入口修复已由 #1303 合入。L0 守卫、workspace tests、默认慢速矩阵与 coverage 在 `8b841da6` 复采通过，#851 / #1060 已具备关闭条件，等待用户确认。 | [#1060](https://github.com/rushsinging/aemeath/issues/1060)、[#851](https://github.com/rushsinging/aemeath/issues/851)、[#1308](https://github.com/rushsinging/aemeath/pull/1308)、[#1303](https://github.com/rushsinging/aemeath/pull/1303) |
| 2026-07-20 | #1060 完成 Memory / Reflection 初次测试完整性审查：#1283 收窄 Reflection 错误/history ACL，#1284 接通 PreCompact，#1285 收口有界 teardown；Memory/Runtime coverage 为 `87.27% / 84.42% / 87.09%` 与 `71.66% / 71.10% / 72.11%`。Manual 用户入口已移至 #860 的 #1289；事务/history 相邻测试由 #1299 承接，慢速矩阵 worktree binary 缺陷已移至 #1050 的 #1298，#851 仅因 #1299 维持开放 | [#1060](https://github.com/rushsinging/aemeath/issues/1060)、[#851](https://github.com/rushsinging/aemeath/issues/851)、[#1298](https://github.com/rushsinging/aemeath/issues/1298)、[#1299](https://github.com/rushsinging/aemeath/issues/1299) |
...
| 2026-07-21 | #1277 完成 accepted-input first durable handoff：Context mutation gate 内以 `(run_id, step_id, fingerprint)` 执行幂等/冲突、save-before-publish；shared Loop 固定 `freeze_step → accepted append → build_window`，Main/Sub 只传 accepted user facts，outcome bridge 不重复 input | #1277 / #1278 |
| 2026-07-21 | #1282 开始 Session v2 cutover：canonical writer 改写 `run_slices + ActiveCompactMarker`，v1/legacy chats 升级为 synthetic Step，ContextWindow/resume/list/compact 统一读取 marker 后结构化历史；现有 finalized append 暂作 compatibility outcome bridge，#1277/#1278 分别承接 accepted input 与正式 outcome/receipt | #1282 / #1277 / #1278 |
