...
| O9 | #885 已建立 `agent/features/task` 所有者 crate：Task-owned 强类型 ID、私有 Task/Batch 实体、typed create spec、严格局部状态机、领域事件、`TaskStoreState` 骨架与 typed lifecycle 纯策略已落地；`started_at` / `completed_at` 由状态迁移维护为统一执行时间事实。Task 实体不包含 `owner`：Agent 分配归 Runtime-owned `TaskAssignment`（或等价执行绑定）。旧 `shared/src/tool/types/task.rs` 现明确为 pre-#889 Tool/Storage legacy DTO；因现有消费者仍直接读写公开字段（含 legacy `owner: String`），暂不能透明替换。旧 Storage backing 仍持多个 async lock，snapshot/port/adapter 尚未迁移 | Task BC 独占 Published Language、聚合、执行时间事实与 lifecycle 领域策略且不依赖 Agent 身份；Agent Runtime 独占 `TaskId ↔ AgentId` 执行绑定；TUI 只投影 started/completed 并计算展示耗时；schema 构建从 Task-owned 唯一定义生成或受控引用，不恢复同义 façade | [#885](https://github.com/rushsinging/aemeath/issues/885) 完成领域内核；[#886](https://github.com/rushsinging/aemeath/issues/886) 承接 DAG 与跨实体原子命令，[#887](https://github.com/rushsinging/aemeath/issues/887) 承接正式端口和统一 backing，[#888](https://github.com/rushsinging/aemeath/issues/888) / [#890](https://github.com/rushsinging/aemeath/issues/890) 承接含执行时间的 snapshot codec 与安全恢复，[#889](https://github.com/rushsinging/aemeath/issues/889) 迁移 Tool/Runtime ACL、停止消费 legacy owner 并统一 schema 来源，[#891](https://github.com/rushsinging/aemeath/issues/891) 删除 shared/storage legacy façade（含 owner）并收口 Guard；Runtime assignment 由对应 Agent Runtime leaf 承接 |

#972 只对齐设计文档、责任映射与 issue 结构，**NEVER** 修改目录实现、守卫脚本或开始上述迁移。O1–O9 列出的执行 leaf **MUST** 在开工前以本表 Target 同步正文与父 Issue；迁移期固定层级守卫在 #982 替代守卫证据齐备前 **MUST** 保持运行。
