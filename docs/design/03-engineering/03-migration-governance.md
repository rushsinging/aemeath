...
| T9 | **Slash Command Catalog / Router 待生产切线** | #913 开发前盘点确认命令真相至少分散在 `packages/sdk/src/commands.rs::builtin_commands()`、TUI `app/slash.rs` / `slash/help.rs`、no-TUI 独立 `/exit` / `/reflect` parser，以及 Runtime `ChatInputEvent → PendingCommand → idle_commands/loop_runner` 二次分发；查询命令直接读取 TUI model/config view，未知 slash 在 no-TUI 可能退化成普通 LLM 输入，业务 Outcome 与 terminal formatting/`[action:*]` 字符串混合 | Tools BC 独占 `CommandDescriptor`、`CommandCatalogPort`、`CommandRouterPort` 与三类类型化 route；Composition 装配唯一 capability，SDK 只发布交付层窄 PL，CLI/TUI/no-TUI 共用 Catalog/Router；PromptInjection 只提供 `PromptFragment`，SnapshotQuery/ApplicationControl 保留目标 BC 的 typed Snapshot/Outcome，交付层只做 ACL | #913（Catalog/Router 与生产切线）；#947（TUI slash I/O 全 Effect 化）；#914（legacy 物理退役） |
