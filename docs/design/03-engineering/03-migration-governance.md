...
| # | 缺口 | 现状 | 目标 | 迁移阶段 |
|---|---|---|---|---|
| PHA1 | **Policy 无统一端口** | #915 已移除 content warning；当前仍有路径 helper 与 allow_all bool 分散，无 PolicyRequest/Decision | v0.1.0 建 PolicyPort + AllowAllPolicy；Deny/RequireApproval 只保留 PL | S3/S5 |
| PHA2 | **`--yolo` 泄漏为业务 bool** | Runtime/ToolContext 直接传播 allow_all | CLI/Config 映射 PolicyMode::AllowAll，Runtime 只依赖 PolicyPort | S5 |
| PHA3 | **安全 guard 冒充 Policy 风险** | #915 已将 content scan 唯一归回 Context；path security 与 bash safety 仍待归位 | 各自保留为所有者的独立 guard；未形成共同不变量前不并入 Policy Engine | S5/S7 |
...
| PHA11 | **Audit Usage Contracts 与 AppendLog 已建立，处理链待落地** | #927 已建立 Usage PL/query port/统一 ID；#928 已建立 Audit-owned append port、File adapter 与 Storage path-safety primitive | #929 worker/config → #930 query → #931 Composition bridge/生产接线 → #932 旧 Cost 退役 | S5 |
