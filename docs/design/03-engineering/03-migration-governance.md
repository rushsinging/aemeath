...
| CFG4 | **交付层直连 Config 风险** | #683 已将 tool / agent 并发默认值统一收口到 ConfigSnapshot，Runtime 只处理非零 CLI override；#696 已删除 HookRunner/Claude settings 两个无消费者裸 Config 兼容入口，并将 role logs 测试 helper 改为 ConfigSnapshot；生产裸 Config 现仅留 ConfigAppService 内部 active/merge/update 实现。raw ConfigReader / writer / watch / CLI patch 入口仍可能被 TUI / CLI 或 AgentClient application 直接消费 | #933 定义 async `ConfigQuery` / `ConfigWriter` 与 SDK delivery seam；#871 提供唯一 gate-aware implementation。TUI / CLI 经 AgentClient command + SDK event，CLI args 只作 bootstrap source | #683（默认值完成）/ #696（消费审计完成）/ #933 / #871 |
