...
| T5 | **Tool 调用职责已形成 seam，完整 Interaction 状态机待完成** | #911 已将存在性/Scope/Profile/schema/函数调用收进 Execution，并将 Policy/Hook/timeout/并发保留在 Runtime；typed suspension 仍映射到旧 Runtime AskUser oneshot 兼容路径 | Runtime 经唯一 `InteractionPort` 拥有 request identity、continuation、await/resume/cancel/retry；SDK/TUI 只交换纯值 | #911（调用边界完成）/ #877/#878（Interaction 状态机） |
...
| M5 | **Reflection 领域与 #899 执行/TUI 语义已收口，旧符号待最终清理** | #898 已迁移 PL、prompt/schema/parse/format/apply；#899 已将 Interval/PreCompact/Manual 全部切入 Runtime 单槽异步协议，busy skip、静默完成，删除 TUI 主动完整结果展示，并将 `/reflect [limit]` 切为只读安全 history query | #900 只清理最终旧符号/兼容 façade；不得恢复同步执行或结果正文投影 | #898（领域完成）/ #899（完成）/ #900 |
...
| M7 | **Memory 配置与只读注入已切入 Context-owned materialization** | #984 删除 Runtime `memory_inject` 重复 helper；生产 Run 从 shared lease 捕获同一 Memory Arc，并经 Context `MemoryRetrieveAdapter` 调用 `retrieve_for_inject`；统一 Loop 的 Memory 行为由 `RunSpec.memory` / 装配 Port 表达 | #900 仅清理 Storage-owned legacy top query / façade；不得恢复 Runtime-owned render 或 Main/Sub 角色分支 | #897/#984（完成）/ #900（退役） |
...
| M13 | **Main Memory 单次 open、shared lease 捕获与同 Arc 分发已建立** | #871 在 exclusive gate 内安装 candidate；#984 在每个生产 Main Run 开始时调用 `bind_main_run`，在 shared lease 下捕获 Session/Config/Memory 三元组，Context-owned 注入与 Runtime/Tool/Reflection 使用该 Run Arc；跨项目 resume 后下一 Run 捕获目标 identity Arc | 保持 lease 内一致性；后续 RuntimeContext 全量切线不得退化为启动期 stale Arc | #897/#899/#871/#984（完成） |
| M14 | **legacy top query 已退出生产注入** | #984 删除 Runtime 重复注入 helper，生产只经 Memory-owned `retrieve_for_inject` + Context-owned render；Storage `top_for_inject_readonly` / mutating `top_for_inject` 仅剩 legacy 定义 | #900 删除两个 legacy top query；访问统计若需要必须是显式 fallible command | #984（生产退役完成）/ #900（符号删除） |
...
| # | 缺口 | 现状 | 目标 | 迁移阶段 |
|---|---|---|---|---|
| CM1 | **Compact execution ownership 已收口，L2/L4 算法仍待实现** | #876 已将 Main/Sub execution path 的自动 compact 全部切到 ContextPort，同一 mutation gate 串行 append/compact/resume，生产 backing 保留冻结 ChatSegment 前缀并经 AtomicBlob 持久化；Runtime legacy auto/microcompact fallback 已删除。idle `/compact` 与 session command 兼容投影仍由 #872/#879 承接 | L1 在 ToolResult 进入 ContextAppend 前完成；L2-L4 只变换 ContextWindow 读模型；L5 经 ContextPort 修改稳定 Session backing；fingerprint / circuit breaker / manual bypass 语义唯一 | #876（execution ownership 完成）/ #548 / #552 / #554（算法）/ #872/#879（idle 兼容退役） |
| CM2 | **Runtime 自动触发已统一，预算算法仍继续演进** | #876 让 Main/Sub 使用同一冻结 ContextRequest 和 ContextWindow token estimation，删除 Runtime 自动 compact 重复阈值与固定 fallback；resolved max output、provider usage 与 Tool schema token 通过 request/window 单向流动 | Provider capability 提供模型上限；TokenBudgetConfig 单一来源；manual / auto 入口分明；fingerprint 增量估算且 Hook 只在真实 compact 时触发 | #876（Runtime 切线完成）/ #550 / #553（算法） |
| CM3 | **Runtime execution 的 Prompt/Memory/History 入口已统一** | #876 让 Main/Sub provider invocation 只消费 ContextWindow 的 messages/system blocks/tool schemas/token estimation；生产 Context factory 从 committed Memory holder 动态检索，Runtime execution 的直组装 memory/summary/history 路径已删除。`loop_runner` idle gate、`/compact`、`/resume` 与 session commands 仍使用兼容投影/旧 façade，明确不属于 execution 权威 | Prompt 策略进入 domain，I/O detail 终止于 adapter；ContextPort build_window 是唯一 Runtime execution 入口 | #876（execution path 完成）/ #872/#879（idle/session façade 退役） |
...
| PHA4 | **Hook 公开面膨胀且结果分裂** | #987 已将 crate 物理结构迁为 `domain + ports + adapters`，但 `adapters::legacy::HookRunner` 仍经迁移期 `hook::api` 提供多个 on_xxx / plain / JSON / blocking 入口 | 一个类型化 HookPort.dispatch + HookOutcome；#925 切 Runtime adapter，#926 删除兼容 façade 与具体 runner 公开面 | S5 |
| PHA5 | **类型化阻断协议已建立，生产消费待切换** | #924 已由单一 protocol 分类 `exit/stdout`，主动 Block、Context、UpdatedInput 与 typed ExecutionFailed 进入 `HookOutcome`；legacy Runtime 路径仍自行解析 `HookResult` / JSON | #925 切换 Runtime adapter，只消费 HookOutcome；#926 删除旧解析与推断入口 | S5 |
| PHA6 | **非零 exit 语义已在新 Dispatcher 统一，legacy 注释待退役** | #924 已固定任意非零 exit（含 1/2/127）为主动 Block、只执行一次；shared legacy 配置注释仍保留“exit 2 阻断/其他非阻塞错误”的过期说明 | #925 生产接线后由 #926 删除旧注释与 exit code 特判 | S5 |
| PHA7 | **Hook Dispatcher 与仅故障重试已建立，生产接线待切换** | #923 已建立 Unix 受管 ProcessDriver；#924 已实现 Hook-owned Dispatcher：按 `order + 声明顺序` 串行、匹配/聚合/短路，仅对 spawn/wait/IO/timeout/InvalidJson/协议故障最多尝试 3 次，主动 Block 不重试；Stop 耗尽固定 Block 并 best-effort 派发一次不递归的 StopFailure | #925 将 Runtime 生产消费切换到 HookPort/HookOutcome；#926 退役 legacy Runner；env_clear 与环境白名单差距由 #1216 承接 | S5 |
...
| `legacy-no-agent`、历史 `register_all_tools*`、旧内部 Registry/Profile 与 `SkillTool` | #911 已让 Runtime 生产路径只消费双端口，但 Tools 内仍保留兼容 Registry façade、Profile/Scope 与只报告 loaded/path 的 SkillTool；边界切线不等于代码清零 | SkillMaterializationPort 接线后，由 #914 删除兼容 Scope、历史入口、无消费者旧内部 Registry/Profile 与 SkillTool；调用方只用正式 Scope/Skill 装配 | #912/#914 |
