...
| L4 | **全仓 Target 生产消费已对齐 #941** | #936 注册 Update/Workflow/Context 等真实 target；#941 将全部生产日志迁到 crate-root owner `LOG_TARGET`，修正 Context 冒用 Runtime/Storage target，并以 owner-aware 全仓 Guard 拒绝 bare macro、字符串 target、重复/缺失常量和 owner 错配；Config/Memory/Task 无生产日志，未预建空 target | 后续新增生产日志必须先在 Catalog 注册真实 owner，并只经唯一 owner 常量消费 | 已完成（#936/#941） |
