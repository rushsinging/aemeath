...
| R10 | **Session `messages` / `chats` / structured-run 迁移进行中** | #872 删除 live `Session.messages` 与旧 writer；#1282 将 v2 canonical writer 切至 `run_slices + ActiveCompactMarker`，`chats/messages/cwd` 只留 legacy reader DTO，现有 finalized append 以 compatibility outcome bridge 写入结构化 Step | ContextWindow、resume、list 与 compact 只读取 marker 后的 `run_slices`；#1277 补 accepted input writer，#1278 将 outcome bridge 收口为正式 cause/receipt/usage 投影 | #1282（schema、marker、legacy upgrade、读取切换）→ #1277 → #1278；完成后用 codec/Context guard 禁止 writer 与常规读取回流 `chats` |
