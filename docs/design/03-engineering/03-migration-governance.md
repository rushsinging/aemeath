...
| CM5 | **Runtime 维护对话历史租借投影（#871 剩余差距）** | #871 已让 `BoundMainRun.session()` 暴露 `Arc<CanonicalSession>`。`MainSessionWiring` 现已支持注册 `SessionProjectionParticipant`：`resume_prepared` 在 exclusive gate 内、Project/Task/Session/Memory commit 后、Config watch 前，同步调用 `participant.prepare`（纯函数，从 `CanonicalSession` 构造 `SessionRestore`）+ `participant.commit`（同步、无失败、无 await），使 leased backing (`current_chain`/`frozen_chats`/`active_summary`) 在释放 gate 前已是新值。`from_args` 在 resume 前构造 backing Arc 并注册 participant；运行期 `resume_session_to_backing` 不再在 gate 外重建/写投影，loop runner 不再 double-write backing。**#871 可观察性窗口已闭合**：释放 exclusive 后 shared observer 第一次读即看到新 session + 新 projection。**迁移债**：Runtime 仍持有第二份可变 backing（尚未经 `ContextPort::build_window` 直接从 Context 读取），但该 backing 已在 gate 内原子发布 | Runtime 每个 Run 经 `ContextPort::build_window` 直接从 Context 读取；**NEVER** 维护第二份可变对话 backing；租借投影完全退役，resume 后 Runtime 只持 `BoundMainRun` 的 `CanonicalSession` 引用 | #876（Runtime 切线） |
...
| CFG3 | **active Config 非 project-aware 联合切换** | #933 已建立独立 Config BC crate、唯一 ConfigAppService/wiring、ConfigReader/Query/Writer/ProjectConfigParticipant typed seam。#871 已实现 `SessionSwitchGate`（owned shared / exclusive `TokioRwLock`）、`MainSessionWiring::resume_prepared`（Project→Config→Memory→Task prepare / 无失败 commit）、`GateAwareConfigQuery`（shared permit 下 capture snapshot/subscription）与 `GateAwareConfigWriter`（exclusive permit + candidate Memory eager-open + `tokio::spawn` cancellation-shielded critical section）。#871 已完成 CFG5 chain-layer replay（override 为最后一层）、bootstrap active location、删除 global fallback。CM5 可观察性窗口已闭合（projection participant 在 gate 内原子更新）；第二 backing 仍为迁移债 | Config 独占 `{location,snapshot}`；Project→Config ACL；Project→Config→Memory→Task prepare；update handoff 后由 owned cancellation-shielded task 完成 durable Config persist、Memory install、Config install、watch 最后发布；非 Run `ConfigQuery::snapshot/subscribe` 先取得 shared permit | #933（seam/单例完成）/ #871（gate/联合协调已落地）/ #871（CFG5 已完成） |
| CFG4 | **交付层直连 Config 风险** | #683/#696 已收口默认值与裸 Config 消费；#933 已删除 Runtime 散点 `new/load`，model switch/list 复用同一 committed reader，并在 AgentClient 发布 typed `config_view/update_config` SDK DTO；TUI/CLI 无 Config 契约泄漏。#871 已提供 gate-aware `GateAwareConfigQuery` / `GateAwareConfigWriter` façade，Runtime bootstrap 从 `wiring.config_query()` / `config_writer()` 获取 façade | TUI / CLI 经 AgentClient command + SDK event，CLI args 只作 bootstrap source | #683 / #696 / #933（delivery seam 完成）/ #871（gate-aware façade 已落地） |
| CFG5 | **RuntimeOverride 作为独立 chain layer（已对齐 #871）** | #871 已实现完整 chain-layer replay：`prepare_for_project` / `prepare_update` 均通过 `load_config_with_override` 从 `Config::default()` 重放 File → Compatibility → Env → CLI → durable override，override 始终为最后一层。生产 `wire_project_config` / `wire_project_config_with_cli` 接入 `NativeConfigStore`（`FileSystemBlobAdapter` backed by `global_agents_dir()`），`ConfigWriter` 持久化成功。`wire_main_session` bootstrap 从已验证 Project identity 构造 `ProjectConfigLocation`，`active_location` 始终为 `Some`；`prepare_update` 缺 active location 返回 typed `Invalid`，删除 `"global"` fallback。进程重启后 override 不被 File / Env / CLI 覆盖；跨 project override 隔离 | `RuntimeOverrideAdapter` 排在 `CliArgsAdapter` 之后；`prepare_for_project` / `prepare_update` 每次从 default 重放完整 chain 并固定把 override 排在最后；同一 project 下新 Env / CLI 读数 **NEVER** 覆盖已持久化的 runtime override | #934（adapter/durable）/ #871（chain-layer replay 已完成） |
...
| L6 | **Config 参数接线不完整（已对齐 #938）** | Composition 从 committed `ConfigSnapshot` 的细粒度 accessor 构造不可变 `LoggingSettings`；Logging 不读 env，filter/max-level 单次归一化，自定义 logs_dir、mode、rotation 与 retention 静态值完整注入；Runtime 已退出配置装配 | #939 消费 retention 设置实现 lifecycle / 删除机制；配置热更新若需要可更新 policy handle 另行设计 | 已完成（#938）/ #939 |
...
| # | 缺口 | 现状 | 目标 | 迁移阶段 |
|---|---|---|---|---|
| PHA1 | **Policy 无统一端口** | #915/#916 已移除 content/path guard；当前仅 allow_all bool 分散，无 PolicyRequest/Decision | v0.1.0 建 PolicyPort + AllowAllPolicy；Deny/RequireApproval 只保留 PL | S3/S5 |
...
| 日期 | 变更 | 关联 |
|---|---|---|
| 2026-07-18 | #871 闭合 CM5 可观察性窗口：`MainSessionWiring` 新增 `SessionProjectionParticipant` trait（prepare 纯构造 / commit 同步无失败无 await），`resume_prepared` 在 exclusive gate 内、Session/Memory commit 后、Config watch 前同步更新 leased projection；`from_args` 在 resume 前构造 backing Arc 并注册 participant；`resume_session_to_backing` 不再 gate 外重建/写投影；loop runner 不再 double-write backing。第二 backing 仍是迁移债，但可观察性已闭合 | [#871](https://github.com/rushsinging/aemeath/issues/871) |
| 2026-07-18 | #871 完成 CFG5 chain-layer replay：`prepare_for_project` / `prepare_update` 通过 `load_config_with_override` 从 default 重放 File→Compatibility→Env→CLI→durable override，override 始终为最后一层；生产 `wire_project_config` / `wire_project_config_with_cli` 接入 `NativeConfigStore`，`ConfigWriter` 持久化成功；`wire_main_session` bootstrap active location，删除 `"global"` fallback；CFG5 标记为已对齐，CFG3 剩余差距收敛为仅 CM5 | [#871](https://github.com/rushsinging/aemeath/issues/24) |
| 2026-07-18 | #871 实现回写：CFG3 现状更新——`SessionSwitchGate`、`MainSessionWiring::resume_prepared`（Project→Config→Memory→Task prepare / 无失败 commit）、`GateAwareConfigQuery` / `GateAwareConfigWriter` 已落地；新增 CFG5（RuntimeOverride 非独立 chain layer，durable store 已存在但 prepare 不重放完整 chain）；新增 CM5（Runtime 维护 `current_chain`/`frozen_chats`/`active_summary` 租借投影，重建在 gate 窗口外）；CFG4 现状补充 gate-aware façade 已落地；O7 现状标注联合协调器已实现 | [#871](https://github.com/rushsinging/aemeath/issues/871) |
