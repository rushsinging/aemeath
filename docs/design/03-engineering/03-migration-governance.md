...
| R10 | **Session `messages` / `chats` / structured-run 迁移进行中** | #872 删除 live `Session.messages` 与旧 writer；#1282 将 v2 canonical writer 切至 `run_slices + ActiveCompactMarker`，`chats/messages/cwd` 只留 legacy reader DTO；#1277 已让 `freeze_step` 后的 accepted user input 经独立 typed receipt 写入 `CommittedRunStep.accepted_input`，Context backing 在同一 mutation gate 内完成幂等/冲突检查与 durable publish，Main/Sub outcome bridge 不重复 input | ContextWindow、resume、list 与 compact 只读取 marker 后的 `run_slices`；#1278 将 compatibility outcome bridge 收口为正式 cause/receipt/usage 投影 | #1282（schema、marker、legacy upgrade、读取切换）→ #1277（accepted input）→ #1278（outcome）；完成后用 codec/Context guard 禁止 writer 与常规读取回流 `chats` |
