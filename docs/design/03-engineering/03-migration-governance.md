...
| S9 | **Storage 父项测试完整性尚未形成最终矩阵** | #991/#880/#881/#882/#883/#884/#983 已关闭，Blob/Dataset 契约与 crash harness 已存在；SafeStorageRoot 契约、测试 owning-layer、墙钟型锁断言、公开面/过渡 Guard 和消费方相邻证据仍需统一事实复核 | #1057 按[测试治理 §11.9](04-testing-and-coverage.md#119-1057-storage-根因级测试审查执行计划)建立行为—风险矩阵，先修确定性与组织根因，再补真实缺口；公开面跨消费者迁移超出单 PR 时拆原生 sub-issue | #1057（计划已冻结，待确认执行） |
