...
| CFG1 | **Config adapter、durable protocol 与 Env 单一入口** | #934 已完成 File/CLI/Compatibility/Claude adapters、validation 与 AtomicBlob durable flow；#935 将全部业务 env 和 provider keys 收口到 Config EnvAdapter，删除 Config application/Runtime env fallback 与退役的 MAX_REASONING 读取。Guard 注册表仅保留 1 个 Target owner policy，migration exception=0 | adapter 输出 ConfigPatch；Application 只编排 layer / validation；Runtime/其他 BC 只消费 ConfigSnapshot | #934 / #935（已完成） |
