...
| R7 | **Main/Sub PolicyPort 已统一** | #918 已让 Main/Sub 从 Composition 注入的同一 `Arc<dyn PolicyPort>` 构造真实 PolicyRequest，并删除 Runtime/Tool 的 `allow_all` 业务传播 | 保持 Main/Sub 同一 PolicyPort 与 Runtime 唯一控制流；Future Deny/Approval 另行设计 | 已完成（#918；#1062 补齐身份与分支测试证据） |
...
| # | 缺口 | 现状 | 目标 | 迁移阶段 |
|---|---|---|---|---|
| PHA1 | **Policy 统一端口与生产消费已完成** | #917 建立 Policy-owned Request/Decision/Port 与唯一 AllowAllPolicy；#918 将 Composition 注入的同一 PolicyPort 接入 Main/Sub，Runtime 按真实 Run/Step/Tool/capabilities/workspace 构造请求 | 保持 Policy 契约单一 owner 与 Main/Sub 同实例传播；#1062 以 L1-L4 测试矩阵持续核验 | 已完成（#917/#918） |
| PHA2 | **`--yolo` 兼容 ACL 与业务传播退役已完成** | `--yolo` 主名称和 `--allow-all` alias 映射同一 bootstrap ACL；#918 已删除 Tool/Policy 决策链对 `allow_all` 的消费，兼容 bool 仅作为 CLI/SDK/Config 输入及 Runtime/TUI permission-mode 展示投影保留 | 保持兼容入站/展示 ACL，不让 bool 回流 Policy/Tool 业务判断；展示投影随 Runtime Context 收敛另行退役 | 已完成（#917/#918；#1062 补场景证据） |
