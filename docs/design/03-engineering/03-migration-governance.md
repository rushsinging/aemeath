...
| O9 | Runtime 仍以 `api/business/contract/core/gateway/utils` 固定层组织，8 个能力与活资源、生产 bootstrap 分散在层目录 | `agent/features/runtime/src/capabilities/` 包裹 8 个 VSA 切片；`shared/` 只承载 `RuntimeContext` 等 crate 内最小共享基础；`shared` 不依赖 capabilities；具体实现选择、factory 调用与对象图连接只在 `agent/composition` | [#995](https://github.com/rushsinging/aemeath/issues/995) 先做行为等价目录迁移并同步 façade/import/测试；生产 bootstrap 责任迁入 `agent/composition`，Runtime 内不保留第二 Composition Root。退出证据：旧固定层目录与 Runtime `utils/bootstrap` 零生产引用，`lib.rs` 维持窄公开面，`shared → capabilities` 反向依赖为零，测试、clippy、架构守卫及故意违规证明通过 |
