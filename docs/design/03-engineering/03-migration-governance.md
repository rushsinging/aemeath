...
| M13 | **Main Memory 单次 open、同 Arc 分发与 Reflection view 已建立，resume gate 待接** | #897 Composition 按 ProjectIdentity + committed MemoryConfig 打开真实 service，RuntimeResources/ToolResources 复用同一 Arc；ActiveMemoryWiring 提供 prepare/install 与 Arc identity contract；#899 已接 Memory-owned Reflection history view | #871 在 exclusive gate 内安装 candidate；#984 切 Context production injection | #897/#899（对应项完成）/ #871/#984 |
...
| # | 缺口 | 现状 | 目标 | 迁移阶段 |
|---|---|---|---|---|
| L1 | **Main/Sub 日志上下文互相覆盖（生产接线已对齐 #940）** | #937 建立不可变 scope 基础；#940 已将 Main/Sub 的 session→chat/sub-run→turn→physical request 接线迁入 task-local scope，retry 使用新 request ID，Runtime 生产 spawn 显式传播，Provider blocking stream bridge 仅透传 opaque `LogContext`；Runtime/Provider 不再消费 legacy setter，source Guard 阻止回流 | #942 删除 Logging crate 内 legacy setter / `CURRENT_*` / formatter fallback，并将 scope Guard 精确基线收敛为空 | #937/#940 已完成 / #942 |
...
| PHA7 | **Hook 失败回收已收口，故障重试待接线** | #923 已建立 Unix 受管 ProcessDriver：独立进程组、完整 deadline、有界并发 IO、timeout/cancel/IO 的 TERM→KILL→wait；legacy Runner 已消费，尚未实现统一 retry | #924 仅对 ExecutionFailed 最多尝试 3 次；主动 Block 永不重试 | S5 |
