...
| 单一同步 typed Run control 与共享 Loop barrier | #1247 已将 `cancel_run_step` / `terminate_run` 接入 `AgentClient`、ActiveRun registry 与 Main/Sub 共享 Loop；CancelStep 收口到 Drain，Terminate 递归传播同一绝对 deadline；旧 `cancel_run` 与 `Cancelling/Cancelled` 仅为 #879 待退役兼容路径 |

下表只保留当前仍真实存在的结构性缺口；后续实现若改变 Current，**MUST** 在合入时同步本表：

| # | 缺口 | 现状 | 目标 | 迁移阶段 |
|---|---|---|---|---|
| R1 | **Run 控制生产切线已完成，旧取消兼容符号待物理退役** | #1247 已接入 `AgentClient::cancel_run_step/terminate_run`、typed ActiveRun control、统一 await barrier、CancelStep finalization→drain、Terminate→Terminated、pending waiter drain，以及父子 Run 同一绝对 deadline 递归传播；旧 `cancel_run`、`Cancelling/Cancelled` 和 compatibility projection 仍可达兼容路径 | #879 删除旧 `cancel_run`、旧状态/事件/projection 与兼容仲裁；退出证据要求生产只可达新命令且最终只到 Completed/Failed/Terminated | #700/#1247（生产切线）→ #879（物理退役） |
...
| R8 | **SDK identity / Runtime Interaction 基础契约已建立，生产路由仍待完整切换** | #874 发布纯值 Interaction PL；#1245 建立 Run-owned pending/continuation、Runtime waiter bridge 与 reply/cancel 命令；#1247 已让 Run control 接受时 drain 对应 Run（含 descendant）的 waiter。bridge 的 request 注册仍未注入 Main/Sub suspension 生产 adapter，旧 `AskUserBatch.reply_tx` 仍可达 | #1246 切 Main Tool suspension；#943/#944 完成 TUI command；#1248 完成 Sub/Hook/Reasoning 装配；#879 删除旧 AskUser projection。退出证据仍要求 SDK/TUI 生产事件零 sender、Main/Sub identity 无损、projection 唯一 | #874/#1245/#1247（契约与 control drain）→ #1246/#1248/#943/#944/#879（生产切线与退役） |
