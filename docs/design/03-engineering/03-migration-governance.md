...
| # | 缺口 | 现状 | 目标 | 迁移阶段 |
|---|---|---|---|---|
| R1 | **Run 控制目标域已建立，生产入口仍是旧取消语义** | PR #1036 已在 SDK/Run 聚合加入 `CancelRunStepOutcome`、`TerminateRunOutcome`、绝对 `ControlDeadline`、`DrainingInput/CancellingStep/FinalizingStep/Terminating/Terminated` 与 typed events；生产 `AgentClient` / TUI / Loop 仍只调用旧 `cancel_run`，旧 `Cancelling/Cancelled` 与 compatibility projection 仍在 | #1272 先把正常 finalized Step 接入唯一 InputBuffer drain-or-seal，并提供 #1247 可复用的 `DrainDecision` 原语；#1247 原子接入 Run root + per-Step child scope、StepFinalizer 与新 AgentClient commands；#879 删除 `cancel_run`、旧状态/事件/projection、`deferred_user_inputs`、legacy queue 和平行 registry 仲裁。退出证据：生产只可达新命令，正常与控制 Step 共用 drain-or-seal，Main/Sub 同一 InputBuffer contract，最终只到 Completed/Failed/Terminated | #700 → #1272 → #1247/#879 |
