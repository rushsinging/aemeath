...
| R7 | **Main/Sub PolicyPort 已统一** | #918 已让 Main/Sub 从 Composition 注入的同一 `Arc<dyn PolicyPort>` 构造真实 PolicyRequest，并删除 Runtime/Tool 的 `allow_all` 业务传播 | 保持 Main/Sub 同一 PolicyPort 与 Runtime 唯一控制流；Future Deny/Approval 另行设计 | 已完成（#918；#1062 的身份、决策分支与 L4/L5 适用性证据见[测试治理 §11.8](04-testing-and-coverage.md#118-1062-policy-l0l5-覆盖证据)） |
...
| # | 缺口 | 现状 | 目标 | 迁移阶段 |
|---|---|---|---|---|
| PHA1 | **Policy 统一端口与生产消费已完成** | #917 建立 Policy-owned Request/Decision/Port 与唯一 AllowAllPolicy；#918 将 Composition 注入的同一 PolicyPort 接入 Main/Sub，Runtime 按真实 Run/Step/Tool/capabilities/workspace 构造请求 | 保持 Policy 契约单一 owner 与 Main/Sub 同实例传播；#1062 的 L0～L5 行为—测试矩阵见[测试治理 §11.8](04-testing-and-coverage.md#118-1062-policy-l0l5-覆盖证据) | 已完成（#917/#918/#1062） |
| PHA2 | **`--yolo` 兼容 ACL 与业务传播退役已完成** | `--yolo` 主名称和 `--allow-all` alias 映射同一 bootstrap ACL；#918 已删除 Tool/Policy 决策链对 `allow_all` 的消费，兼容 bool 仅作为 CLI/SDK/Config 输入及 Runtime/TUI permission-mode 展示投影保留 | 保持兼容入站/展示 ACL，不让 bool 回流 Policy/Tool 业务判断；展示投影随 Runtime Context 收敛另行退役 | 已完成（#917/#918/#1062；L4/L5 因无生产 Deny/Approval 旅程和系统边界而不适用） |
| PHA3 | **安全 guard 冒充 Policy 风险已收口** | #915/#916 已分别将 content scan、path containment、Bash/read-before-write 归回 Context/Project/Tool；#918 删除 legacy approval gate，Runtime 只编排 PolicyDecision | 所有者局部 guard 保持独立；Policy v0.1.0 只实现 AllowAll decision | 已完成（#915/#916/#918） |
