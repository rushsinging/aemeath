...
|---|---|---|---|
| O1 | Runtime 已完成其经批准的 Hexagonal 目录迁移；Storage 由 #991 消除固定 COLA 层后处于过渡布局；其余普通 feature 仍受迁移期固定层目录约束，context feature 保留脚本级精确顶层例外；完整脚本行为与常量见 [Architecture Guards](01-architecture-guards.md) | 按 [代码组织规范](../01-system/06-code-organization.md) 收敛：递归竖切统一进入私有 `capabilities/`，单能力保持扁平，技术目录按 seam 引入；Runtime 保留 `domain/application/ports/adapters` 显式例外。各模块 Target 判定见 [模块目录结构决策](../02-modules/README.md#目录结构决策) | Storage 由 [#991](https://github.com/rushsinging/aemeath/issues/991) 先行为等价提升为 `memory_store` / `task_store` / `tool_result` 顶层过渡模块，删除 `api/business/contract/gateway`、仅测试可达的旧 History 与 `storage::api`；该布局 **NEVER** 视为 Storage Target。#880/#983 按 Target 建立 `capabilities/{safe_path,atomic_blob,atomic_dataset}`，#883/#884 迁出或退役过渡业务语义。其余 feature **MUST** 在 #743 原生树内由模块 leaf 独立迁移；Project 由 [#892](https://github.com/rushsinging/aemeath/issues/892) 承担。[#763](https://github.com/rushsinging/aemeath/issues/763) 是治理父项，正式 capability-first Guard 与全局故意违规证明归 [#982](https://github.com/rushsinging/aemeath/issues/982) / #1022 |
...
#992 已将 Provider 物理结构迁为 `domain/`、`ports.rs`、`adapters/` 与 crate-root 窄 façade，并删除 13 个 COLA 迁移例外；该变更保持行为等价，以下 P1–P12 语义缺口仍全部有效。
...
| CM3 | **Prompt 多入口且无私有 capability seam** | #994 已删除 Context `api/gateway` 与 Prompt `business/gateway` 固定层，Prompt 物理 owner 已明确；Runtime 仍直接调用 guidance/skill/compact/session façade，Application Service 尚未统一编排 | Prompt 是 Context-private capability；ContextPort build_window 是唯一对外入口；单一 async guidance / prompt pipeline | #994（物理完成）/ #870（语义收口） |
...
| 日期 | 变更 | 关联 |
|---|---|---|
| 2026-07-16 | #991 将 Storage 从 `api/business/contract/gateway` 行为等价迁为 `memory_store` / `task_store` / `tool_result` 顶层**过渡模块**；旧层、`storage::api` façade 与仅测试可达的旧 History 模块已删除。该布局不代表 Storage Target：#880/#983 后续建立 `capabilities/{safe_path,atomic_blob,atomic_dataset}`，#883/#884 迁出或退役过渡业务语义 | #991 |
