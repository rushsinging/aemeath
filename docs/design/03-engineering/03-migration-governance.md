...
| O9 | `agent/shared/src/task.rs` + `task/` 保持 Rust 2018+ 单能力模块布局；#996 已删除无消费者、仅重复转发 `crate::task` 类型的 `task_ops.rs` 与模块注册。Task 类型权威定义暂仍位于 `shared/src/tool/types/task.rs`，lifecycle 仍在 shared 且仅测试可达 | Task BC 独占 Published Language、聚合与 lifecycle 领域策略；schema 构建从 Task-owned 唯一定义生成或受控引用，不恢复同义 façade | [#996](https://github.com/rushsinging/aemeath/issues/996) 只收窄目录与公开面且保持行为等价，不移动类型、不修改状态机、快照或 Guard 白名单；Task PL、聚合及 lifecycle 所有权迁移由 [#885](https://github.com/rushsinging/aemeath/issues/885) 及后续 Task leaf 承接 |
