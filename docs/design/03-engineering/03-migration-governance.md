...
| L4 | **全仓 Target 生产消费已对齐 #941** | #941 将全部生产日志迁到 crate-root owner `LOG_TARGET`，修正 Context 冒用 Runtime/Storage target；Catalog 为根 `Cargo.toml` 的每个 workspace crate 注册独立 target/owner/sink/file，即使当前无生产日志也必须声明唯一 crate-private 常量；owner-aware Guard 拒绝 bare macro、字符串 target、重复/缺失常量、owner 错配与 workspace/Catalog 漂移 | 后续新增 crate 必须同步注册独立 target；生产日志只经当前 crate 的唯一 owner 常量消费 | 已完成（#936/#941） |
