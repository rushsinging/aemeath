...
Tool、Skill、Command 与 MCP 已有不同 Published Language、消费者、生命周期和测试边界，因此 **MUST** 统一收进私有 `capabilities/`。Tool catalog 与 execution 是同一 Tool 能力的两个窄入口，先在 `tool` 切片内共置；MCP 是 Tool 的外部协议 adapter，但其连接状态机和协议测试已形成独立变化轴，因此作为 `mcp` 切片存在并只向 Tool façade 投影：

```text
lib.rs                             # 窄 façade：各能力 PL + composition-only wiring
capabilities.rs                    # 私有切片注册
capabilities/
├── tool.rs                        # ToolCatalogPort / ToolExecutionPort
├── tool/                          # catalog / execute / builtin（按证据展开）
├── skill.rs                       # catalog + materialization
├── command.rs                     # parse + route
├── mcp.rs                         # MCP ACL + connection façade
└── mcp/                           # transport / protocol / projection（按证据展开）
```

各端口与 model **MUST** 归所属切片，**NEVER** 汇总到 crate-root `ports/` 或 `models/`；MCP wire DTO、transport 与认证不得泄漏出 `mcp`。Composition Root 是唯一生产装配入口。
