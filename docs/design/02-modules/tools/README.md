...
tools/
├── tool/
│   ├── catalog/              # Descriptor、Scope/Profile 投影
│   ├── execution/            # schema 校验、函数调用、ToolOutcome
│   └── adapters/
│       ├── builtin/
│       └── mcp/              # MCP ACL + transport
├── skill/
│   ├── catalog/              # SkillDescriptor
│   └── materialization/      # Skill → PromptFragment
├── command/
│   ├── parser/               # Slash 文本 → CommandDescriptor/参数
│   └── router/               # 三类执行机制路由
└── api/                      # BC 对外 facade；不统一 execute
```
...
| 端口 | 消费方 | 职责 |
|---|---|---|
| `ToolCatalogPort` | Runtime / Context Management | 返回当前 Scope/Profile 下的 ToolCatalogSnapshot 与 schemas |
...
| `SkillMaterializationPort` | Context Management | 将 Skill 物化为 PromptFragment |
