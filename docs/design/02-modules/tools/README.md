...
```text
tools.rs                   # 窄 façade：Tool / Skill / Command PL 与 wiring
tools/
├── catalog.rs              # Descriptor、Scope/Profile 投影
├── execute.rs              # schema 校验、函数调用、ToolOutcome
├── builtin.rs              # builtin catalog / dispatch；大到需要时再展开
├── mcp.rs                  # MCP ACL + transport 外部 seam
├── mcp/                    # 仅在 connection / protocol / projection 独立变化时展开
├── skill.rs                # catalog + materialization
├── skill/                  # 仅在两个用例已达到拆分阈值时展开
└── command.rs              # parse + route；按变化证据再拆
```

这是按 [代码组织规范](../../01-system/06-code-organization.md) 给出的非规范性能力投影：可以从单个 `tools.rs` 起步，只在共同变化、外部 seam 或独立测试边界成立时展开。MCP 形成目录是因为它是独立外部协议与生命周期，**NEVER** 由“每个能力都需要 adapters/api/port 层”推导。Composition Root 是唯一生产装配入口。
