...
Tool、Skill、Command 与 MCP 采用 Hexagonal + Clean 组织（`domain ← application ← ports ← adapters`）。Tool catalog 与 execution 是同一 Tool 能力的两个窄入口，在 `domain` / `application` 中共置；MCP 是 Tool 的外部协议 adapter，其连接状态机和协议测试已形成独立变化轴，因此作为 `adapters/mcp/` 子目录存在并只向 Tool façade 投影：

```text
src/
├── lib.rs                             # 窄 façade：各能力 PL + composition-only wiring
├── domain/                            # 领域策略、不变量、Published Language
│   ├── tool.rs                        #   ToolCatalog / ToolExecution 策略、schema 校验
│   ├── tool/                          #   catalog / execute / builtin（按证据展开）
│   ├── skill.rs                       #   Skill catalog 策略 + materialization
│   └── command.rs                     #   parse + route 策略
├── application/                       # 用例编排
│   ├── tool.rs                        #   Tool execute 用例
│   ├── skill.rs                       #   Skill materialize 用例
│   └── command.rs                     #   Command route 用例
├── ports/                             # 对外端口定义
│   ├── tool_catalog_port.rs           #   ToolCatalogPort
│   ├── tool_execution_port.rs         #   ToolExecutionPort
│   ├── skill_catalog_port.rs          #   SkillCatalogPort
│   ├── skill_materialization_port.rs  #   SkillMaterializationPort
│   ├── command_catalog_port.rs        #   CommandCatalogPort
│   └── command_router_port.rs         #   CommandRouterPort
└── adapters/                          # 技术实现、外部 detail
    ├── builtin.rs                     #   builtin tool 实现
    └── mcp/                           #   MCP ACL + connection façade（transport / protocol / projection 按证据展开）
```
...
| 2026-07-16 | 冻结 Tool BC Target 目录：Tool、Skill、Command、MCP 统一进入私有 `capabilities/`；端口、模型和协议 detail 归所属切片 | [#972](https://github.com/rushsinging/aemeath/issues/972) |
