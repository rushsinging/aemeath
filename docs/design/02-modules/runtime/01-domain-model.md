...
/// Provider ACL 标准化后的单次调用用量。
///
/// Provider 原始字段 MAY 继续保留在 Provider 内部供成本与诊断使用，但 Runtime
/// 只消费这里的 provider-neutral 总量，NEVER 自行理解各家 cache token 口径。
...
- Anthropic：`total_input_tokens = input_tokens + cache_read_input_tokens + cache_creation_input_tokens`，`total_tokens = total_input_tokens + output_tokens`。
- OpenAI-compatible：优先使用 Provider 返回的 `total_tokens`；缺失时回退 `input_tokens + output_tokens`。其 cached tokens 已包含在 prompt/input tokens 中，**NEVER** 再次相加。
- Runtime / Context **MUST** 只使用标准化后的 `total_input_tokens` / `total_tokens` 做 context window 与 compact 判定，**NEVER** 按 provider name 分支重算。
...
| 2026-07-17 | 冻结 Provider usage 标准化口径；要求 finalized RunStep identity 随 ContextAppend 保留到 Context backing，扁平化延后到 Provider 出站投影 | compact token reset design |
