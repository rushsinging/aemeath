...
| 对象 | 落盘 | 说明 |
|---|---|---|
| **Session 对话历史**（ChatChain/ChatSegment）| ✅ turn 级快照 | 每完成一步落盘一次，供 `/resume` 恢复对话上下文 |
| **Task 快照 / Workspace 上下文** | ✅ 内嵌 Session | 随 Session 落盘（跨 BC 快照组装，见 `../context-management/01-session.md`）|
| **Run 聚合 / RunStatus 状态机** | ❌ 内存态 | 崩溃即丢，从头开始 |
| **RunStep 中间状态 / ToolCall 进行态** | ❌ 内存态 | 不做 checkpoint |
| **RuntimeContext（活资源）** | ❌ 运行时装配 | 崩溃后重新装配 |
| **StuckGuard 计数** | ❌ 内存态 | 重置 |
...
Context Management 加载最近 Session 快照（对话历史 + Task/Workspace 快照分发回各 BC）
  │
  ▼
用户输入 → agent_execution 新建 Run（全新 RunId，Created 态）
  │
  ▼
...
- ❌ Durable Model Invocation checkpoint 链
- ❌ 每次 LLM 调用前后的原子落盘
- ❌ revision / 单调递增版本号防覆盖
- ❌ partial stream 落盘
- ❌ RecoveryRequired / fail-closed 状态

**保留**：turn 级 Session 快照（现状 `save_chain`）——这是"对话历史可 resume"，不是"执行状态可恢复"。
