...
- **唯一生产装配入口**：`agent/composition`。Runtime 的 `capabilities/` 只定义行为、用例及其消费的 Port，`shared/` 只承载 `RuntimeContext` 等 crate 内最小共享基础；二者都 **NEVER** 选择具体生产实现或触发生产 factory。
...
- Runtime crate 内 **NEVER** 建立 `bootstrap/`、service locator 或第二个 Composition Root；现有 Runtime `utils/bootstrap` 的生产构造责任迁入 `agent/composition`，纯无业务辅助代码才可按所有者留在能力或最小 `shared`。
- `shared` **NEVER** 依赖 `capabilities`，也 **NEVER** 通过保存具体 Provider、Registry、Store 或全局 Config reader 绕过 Composition Root。
...
| 2026-07-15 | 明确 Runtime `capabilities/` / `shared/` 不承担生产装配，`agent/composition` 是唯一对象图与 factory 入口 | [#995](https://github.com/rushsinging/aemeath/issues/995) |
