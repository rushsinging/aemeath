...
| Finishing | Stop Hook Block 且 stop_block_count≤15 | FinalizingStep（提交当前 Step 后 feedback + drain batch 进入下一 Step）|
| Finishing | Stop Hook Block 且 stop_block_count>15 | FinalizingStep（提交当前 Step 后 Failed(StopHookRetryExhausted)）|
...
| Stop Hook 阻断（含执行失败 3 次耗尽），累计≤15 | 当前 Step FinalizingStep 持久化 → feedback + 同次 drain 用户追问 → PreparingContext，同一 Run 继续 |
| Stop Hook 阻断累计>15 | 当前 Step FinalizingStep 持久化 → Failed(StopHookRetryExhausted) |
| timeout>0 且墙钟超时 | Failed |
| StuckGuard HardPause | AwaitingUser（Main）/ Failed（Sub，无人应答）|
| CancelRunStep 且 Drain 无新输入 | StepFinalizer → DrainingInput → Completed(`StepCancelledAndInputDrained`) |
