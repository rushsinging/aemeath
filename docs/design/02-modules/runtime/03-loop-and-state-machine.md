...
| 2026-07-20 | #1247 将 `CancelRunStep` / `TerminateRun` 接入 AgentClient、typed ActiveRun registry 与共享 Loop；所有可取消 await 先解释 typed control，CancelStep 收口后 Drain，Terminate 递归向 child Run 传播同一绝对 deadline 并 drain pending interaction waiter；旧 `cancel_run` / `Cancelled` 留待 #879 物理退役 | [#1247](https://github.com/rushsinging/aemeath/issues/1247) |
