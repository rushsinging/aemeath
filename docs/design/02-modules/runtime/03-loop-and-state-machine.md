...
- `result` 不是独立类型——**统一经 `EventSink`**：Run 到达终态时 agent_run 显式 emit `RunCompleted{ result }`（载荷=最后 assistant 文本/结构）。Main→TUI 通知完成；Sub→父 Run，父从 `RunCompleted` 提取 result 继续。**靠终态领域事件识别，不靠遍历 message**
