...
2. 若已取消：**NEVER** 在 barrier 直接 `finish_cancellation()`，也不调用 `run.fail()`、应用 Tool 结果、启动新的 commit 或 `run.finish()`；只 `break 'run` 跳到唯一 epilogue；
3. `?` 操作符 **NEVER** 直接退出 `run_loop`——错误必须先映射为 Run mutation，或在 cancellation 已获胜时跳到同一 epilogue；
4. cancellation epilogue 的固定顺序是 `await_quiesced → rollback_partial_results → finish_cancellation`。回滚只覆盖未提交 partial；已由 commit 成功提交并标记 persisted 的 step **NEVER** 回滚；
5. 所有非取消出口也经过 epilogue 校验，保证 Run 最终进入 `Completed` / `Failed` / `Cancelled` 三态之一。

> Provider / Tool / Compact / Hook 的 future **MUST** 接受 `CancellationSignal` 参数，在 Run cancel 触发时主动中止（取消 HTTP 请求 / kill 子进程 / abort compact），而不是等 future 自然返回后才发现 Run 已取消。Run cancellation 的 `await_quiesced` 只在统一 epilogue 调用，用于确认这些在途工作及派生子 Run 均已收口。独立的 active deadline 获胜时，`await_run_operation` 必须取消并 join **当前 operation** 后立即产生 `RunTimeout → Failed`；这不是 Run cancellation epilogue，也不得产生 `RunCancelled`。
...
5. 未取消时 `run.finish()` → Completed；任何取消路径均由统一 epilogue 收口。
...
/// 伪代码中的每个 `run.*` mutation 都是 `mutate_and_publish(run, &ctx.events, |run| ...)` 的缩写：
/// mutation 返回前必须立即按聚合顺序 drain + emit，任何模块都不得稍后批量补发。
/// 每个 `.await` 都由 `await_run_operation` 包裹；为突出业务顺序，调用点省略该机械包装。
async fn run_loop(
    run: &mut Run,
    ctx: &RuntimeContext,
    ...
    'run: loop {
        ...
        if run.is_cancelling() { break 'run; }
        ...
        let inv = match model_invocation::invoke_with_retry(  // 普通 retry 复用冻结 request
            ...
            |attempt| mutate_and_publish(run, &ctx.events, |run| run.record_retry(attempt))).await {
            Ok(inv)               => inv,
            // ── cancellation barrier ──
            Err(_) if run.is_cancelling() => { break 'run; }
            Err(ContextExceeded)  => {
        ...
        run.apply_response(step, inv);                       // → ApplyingResponse；helper 立即 drain/publish
    ...
    // 唯一 epilogue：取消必须先等待 quiescence、回滚未提交 partial，再进入终态。
    if run.is_cancelling() {
        ctx.cancel.await_quiesced().await;
        run.rollback_partial_results();
        mutate_and_publish(run, &ctx.events, |run| run.finish_cancellation());
    } else {
        // 非取消出口只允许已经由 mutation 进入 Completed / Failed。
        run.assert_terminal();
    }
    let terminal = run.terminal_result();
    run.assert_no_pending_events();
    terminal
}
```
...
伪代码中所有可能阻塞的 Provider / Tool / Context / Hook await（以及 retry backoff）实际都必须通过 `await_run_operation(deadline_clock, cancellation, future)` 驱动：helper 同时 select operation、per-Run cancellation 与**扣除 AwaitingUser 暂停区间后的 active deadline**。deadline 获胜时先触发 operation cancellation 并等待 quiescence，再以 `RunTimeout` mutation 进入 Failed；不能只依赖 loop 顶部 `check_timeout`。Interaction await 进入 `AwaitingUser` 前暂停 active timer，离开后恢复，因此用户等待本身永不消耗 RunSpec timeout。
...
| | 谁管 | 循环 |
|---|---|---|
| **单个 Run** | `loop_engine::run_loop` | Run 内 Run Step 循环；`AwaitingUser` 在同一 future 内暂停，函数只在 `Completed / Failed / Cancelled` 后返回 typed terminal result |
...
- `result` 是 `run_loop` / `derive_sub_run` 的 typed terminal return（`AgentRunTerminal::Completed { result } | Failed { error } | Cancelled`），供父 Run 的 `tool_coordination` 直接消费；`EventSink` 只承载这些权威领域事实的外向投影。Main 把 terminal event 投影给 TUI；Sub 同样可投影诊断，但父 Run **NEVER** 订阅或反向消费 EventSink 来取得业务结果，也 **NEVER** 靠遍历 message 推断结果
