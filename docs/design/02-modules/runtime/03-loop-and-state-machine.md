...
| InvokingModel | Retryable 错误(超时/5xx/429) | InvokingModel（梯度重试 T0-T3，见 §5）|
...
fn run_loop(run: &mut Run, ctx: &RuntimeContext, guard: &mut StuckGuard) {
    ...
    loop {
        ...
        let inv = match model_invocation::invoke_with_retry(  // Retryable 退避重试
            &ctx.provider, window, effort, run.spec.retry, |a| ctx.events.emit_retrying(a)) {
            Ok(inv)               => inv,
...
`model_invocation` 按重试次数**逐级升级应对手段**，每级对应不同的根因假设。Fatal(4xx) 直接跳到 T4。

| 层级 | 触发 | 应对（根因假设）| 可观测 |
|---|---|---|---|
| **T0 即时** | 流中断 / 连接瞬断 | 立即重试 1 次，无退避（瞬时抖动）| `ModelInvocationRetrying{T0}` |
| **T1 退避** | 超时 / 5xx / 429 | 指数退避 1s→2s→4s + jitter；429 尊重 `Retry-After`（临时过载）| `{T1, attempt}` |
| **T2 降级** | T1 仍失败 | 降参重试：降 reasoning effort / 减 max_tokens / compact 缩上下文（请求过重）| `{T2}` |
| **T3 故障转移** | T2 仍失败 | 切换备用模型 / provider（该模型或 provider 故障）——**依赖 pool 配置** | `{T3, model}` |
| **T4 放弃** | 无备用 / 仍失败 / Fatal | `RunFailed{ error }` | `RunFailed` |

- **context 超限**归 T2（compact 缩上下文），不是单独分支
- **梯度可配**（config/RunSpec）：每级次数、退避基数、是否启用 T2 降级 / T3 故障转移
- **T3 故障转移依赖多模型 pool**（CLAUDE.md 开放决策）——先落 T0-T2 同模型，pool 就绪后启用 T3
- **T2 降级影响结果质量**（降 effort/token），emit 事件让用户知情
