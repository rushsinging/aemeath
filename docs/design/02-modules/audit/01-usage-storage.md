...
Envelope 版本属于 Audit schema；Storage 只读写 bytes，不解释字段。
...
- Storage 负责文件/IO 层错误隔离，并把字节行顺序交给 Audit；
- Audit 负责 JSONL schema 解析与行级损坏判断。
...
- 不允许 Storage adapter 自行迁移 Audit schema。
...
Future retention 必须由 Audit Config 定义，并通过 Storage Port 执行；不能挂接 Session lifecycle 自动级联。
