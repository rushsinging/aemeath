...
- `commit_project` **MUST** 同步、无 I/O、无失败：一次替换 Config-owned `{ location, active Config baseline, snapshot }`，再以 `send_replace` 发布同一个 snapshot。baseline 与 snapshot 必须来自同一 prepared candidate，保证其后的 `prepare_update` 不会从切换前配置重新构造。prepare 之后的任何 fallible 工作 **NEVER** 进入提交段。
