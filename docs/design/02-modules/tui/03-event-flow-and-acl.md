...
| Session | `SessionReset` / `UserMessagesWithdrawn` / `SessionResumed` / `SessionResumeFailed` / `TaskStatusChanged` / `SessionSaved` / `CurrentTurnChanged` / `CommandResultText` | 更新 session id、resume/save/task/current-turn 投影；失败 / 文本事件按上表同时进入 Diagnostic |
| Config | `ModelSwitched` / `ContextEstimated` | 更新 provider/model/context-capacity 投影；`ThinkingChanged` 若影响展示同时产生 Conversation Intent |
...
用户取消 → ConversationIntent → reducer
  ...
  → InteractionCancelled / InteractionReplyFailed result Intent → reducer
```
...
| 8 | Interaction reply isolation | arch test：Runtime request id 只经 SDK DTO → TUI newtype → AgentClient command 无损贯穿；TUI 全树零 sender / pending waiter / 自生成协议 id |
