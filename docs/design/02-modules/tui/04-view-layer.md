...
- 逻辑树深度与视觉角色分离：`ToolGroup` 按普通 ToolCall header 渲染并聚合成员状态；直属 `ToolCall` 组成 result rows，仅首成员绘制 `⎿`，后续成员保持同列空 gutter；成员自身 `ToolResult` 仍相对成员缩进
...
> 以下编号是**视图层专属门禁**（`V1`–`V8`），与 [01-architecture-and-dataflow.md §9](01-architecture-and-dataflow.md) 和 [03-event-flow-and-acl.md §8](03-event-flow-and-acl.md) 的全局门禁（数字 1–10）是独立体系，**NEVER** 复用相同数字表示不同规则。全局门禁覆盖 TUI 全树，视图层门禁只约束 view / render / assembler 子树；`V1` / `V2` / `V6` 与对应全局门禁内容重叠，此处标注交叉引用而非重复定义新规则。

| V# | 门禁 | Target 证明 |
|---|---|---|
| V1 | Adapter / ViewAssembler isolation（= 全局门禁 #1） | `adapter/` 与 `view_assembler/` **NEVER** import `render::*` |
| V2 | ViewAssembler boundary（= 全局门禁 #4） | 只读 Model accessor + ViewState，禁止 I/O、副作用、ratatui 与 Model mutation API |
| V3 | ViewModel purity | `view_model/` **NEVER** import ratatui、crossterm、tokio、Model 或 SDK 类型 |
| V4 | Render isolation（扩展全局门禁 #3，加入 Cache 约束） | `render/` 只读 ViewModel / ViewState / Cache，禁止 Model 引用与状态变更逻辑 |
...
| 日期 | 变更 | 关联 |
|---|---|---|
| 2026-08-05 | 收敛 ToolGroup result gutter 为首成员单一 `⎿`，并增加 group header 成员状态聚合 | — |
