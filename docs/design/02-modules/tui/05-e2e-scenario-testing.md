...
它的目标不是替代现有单元测试，而是捕获跨层组合错误，例如事件已到达但未触发 dirty、Effect 未回灌、ViewAssembler 丢字段、Render 绕过消费入口或布局在特定尺寸下错位。全仓 L0-L5 测试分层、目录组织、覆盖率与生产可达性规则见 [testing-and-coverage.md](../../03-engineering/testing-and-coverage.md)。
...
`testing/` 和 `scenario_tests/` 仅在 `cfg(test)` 下编译。`insta` 作为 `apps/cli` 的 dev-dependency；ratatui 与 crossterm 复用生产版本，**NEVER** 为测试引入独立版本。
...
P0 必须进入普通 PR 必跑门禁。P1 可按耗时拆成独立 test filter，但不得成为长期手动测试。PTY 测试因平台差异单独运行，不与 TestBackend 快照混合。
...
- [../../03-engineering/architecture-guards.md](../../03-engineering/architecture-guards.md)：架构守卫与验证治理
- [../../03-engineering/testing-and-coverage.md](../../03-engineering/testing-and-coverage.md)：全仓测试分层、覆盖率、生产可达性与 dead-code 治理
