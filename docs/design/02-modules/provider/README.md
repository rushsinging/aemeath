...
这是按 [代码组织规范](../../01-system/06-code-organization.md) 给出的非规范性能力投影。单一 driver 起步时可全部共置；只有某 driver 内的 request / stream / error 已互相共同变化且有独立测试证据时，才把它从 `anthropic.rs` 等单文件展开为子目录。`api/` / `port/` 不是固定层；driver 是真实协议 ACL，不是为了对称而创建的通用 adapter 目录。
