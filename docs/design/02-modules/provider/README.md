...
> **当前落地（#1033）**：Provider 已把单次 attempt 的机械流程收敛进 crate-private `adapters::http_attempt::HttpAttemptExecutor`——cancellation-aware 请求发送、安全 response header 提取、16KiB 有界 error body 读取、typed network/HTTP transport failure 分类，以及唯一一条 `llm-api-error` diagnostic 落盘；Anthropic、OpenAI-compatible（Chat Completions + Responses）与 Ollama 的 stream/non-stream 路径均已迁入，driver 不再各自手写 `.send()` / `response.text()` / 错误日志拼装。这只是“一次 attempt 怎么发、怎么判失败、怎么记一条日志”的收敛，**不代表** Runtime 已完整拥有跨调用 retry/backoff 或 stream→non-stream fallback；该所有权仍是 P6/P7 迁移债，详见 [Migration Governance §4](../../03-engineering/03-migration-governance.md#4-provider-现状缺口s2-代码盘点)。后续承接边界：pull-based `InvocationStream`（P4，见上表"单次调用的请求发送与流解析"未来形态）由 [#903](https://github.com/rushsinging/aemeath/issues/903) 承接；跨调用重试/退避/降级（P6）、stream→non-stream fallback（P7）与错误分类统一（P9，见上表"stop reason、usage、错误分类"）由 [#905](https://github.com/rushsinging/aemeath/issues/905) 承接。
...
Workflow 根据 Reasoning Node 与 Config 静态上限产生 requested effort。Runtime 在构建 Context Window 前调用 Provider resolver；Provider 根据目标模型能力做最后一道 clamp，并返回带 capability fingerprint 的不可变 options。Runtime 把同一个 effective 档位交给 Prompt 与 `InvocationRequest`，Provider `invoke` 只校验后映射到 wire 字段。Provider 不读取散点 env，也不拥有用户默认值。
