...
Provider 只有一条统一 invoke/stream ACL 生命周期；capability resolution、invoke 与 error mapping 共同服务该能力，不构成业务切片。因此 Provider **NEVER** 创建 `capabilities/`，保持扁平核心，并只对拥有独立 wire type、错误映射和协议测试的 provider/protocol 启用诚实命名的技术目录：

```text
provider.rs                    # 窄 façade：Runtime-owned ProviderPort 的实现 / wiring
provider/
...
`ProviderPort` 由 Runtime 拥有，Provider **NEVER** 为其复制 crate-root `ports/`；wire DTO 与具体 client 必须留在 provider/protocol 技术模块内。`api/`、`model/`、通用 `adapters/` 也 **NEVER** 作为固定层。
