...
- Snip / Microcompact：每次 `PreparingContext` 常驻执行，按完整 Run 保护最近 3 个 Run；
- Auto-compact：仅当最近 Provider 标准化 `last_total_tokens > threshold` 时进入
  `Compacting`。recent tail 单独使用 heuristic 估算，并限制在
  `context_size * 30%`，不包含 summary/system/tool schemas。
...
| 2026-07-17 | Provider ACL 标准化 total tokens（Anthropic 纳入 cache read/create）；自动 compact 只由 last_total_tokens 触发；heuristic 改为诊断与 recent-tail 30% 预算 | compact token reset design |
