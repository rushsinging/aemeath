...
git context **不属于** system prompt 或 `PromptRequest`：Runtime 仅在 session 启动时采集一次，并以带 `SystemGenerated` metadata 的普通用户消息投递首个 Run。此消息不会成为用户输入持久化投影；后续 Git 状态由 LLM 按需调用工具获取。Prompt capability 不读取 Git、当前工作目录或时钟。
...
- 本期不测量供应商实际 cache read / creation token；不同 OpenAI-compatible 服务的隐式缓存策略由服务端决定，不能由统一 wire 字段保证。
...
- L4：`runtime/src/application/prompt/build/prompt_build_tests.rs` 覆盖 Git 初始快照不进入 static prompt；`runtime/src/application/chat/looping/loop_runner_tests.rs` 覆盖 Main Run fixtures 的首次 Git 上下文链路。
