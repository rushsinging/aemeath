...
> 本文定义 Context Management 私有 Prompt capability——系统提示组装、Guidance 解析、Skill 物化 integration、安全扫描与 prompt cache 稳定性。Prompt 不独立成 BC，也不向 Runtime / TUI 发布第二个 OHS；实现差距见 [迁移治理](../../03-engineering/migration-governance.md)。
...
- Current → Target 差距与退役责任：[../../03-engineering/migration-governance.md](../../03-engineering/migration-governance.md)
