...
idle `/compact` 与 reset 分别调用 `ContextPort::manual_compact` / `clear_session`；启动 resume 与运行期 `/resume` 统一调用 `MainSessionWiring::resume_session`，只向 Runtime 返回 `SessionResumeProjection` Published Language。Session list/export/import/metadata/delete 经 Context crate-root façade；legacy `messages` / `cwd` 只存在于 `envelope.rs::LegacySession` reader DTO。旧 `session_storage.rs` writer 与 Task capture compatibility façade 已删除，所有新写入复用 AtomicBlob canonical writer。

静态 Guard 扫描 Runtime 全部生产源码，禁止已退役的 `context::session::*`、`ChatChain` / `ChatSegment`、第二投影 backing、索引式消息归属、save callback 与 legacy compact helper 回流；Context crate-root façade 仍是跨 BC 入口。
