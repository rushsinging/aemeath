...
### 7.2 Runtime 兼容投影边界
#876 已切断 Main/Sub **执行路径**对 Runtime 租借投影的依赖：模型输入只消费 `ContextWindow`，自动 compact 与 finalized Step 提交只经 `BoundMainRun.context()`；Main/Sub execution adapter 的静态 Guard 禁止 `ChatChain` / `ChatSegment` / `save_chain` 与 legacy compact helper 回流。
`RuntimeHandle.current_chain / frozen_chats / active_summary` 当前仅作为 idle gate、SDK/TUI 同步、`/resume` 与旧 session command 的兼容投影保留，不再拥有 provider invocation 或 durable append 权威。它们的最终删除、`process_chat_loop` 返回值收口和旧 `session_storage` façade 退役由 #872/#879 承接；在此之前，兼容投影不得被重新用于 Context Window、auto-compact 或持久化。
...
| 2026-07-19 | #876 将 `BoundMainRun` 扩展为同 lease 下的 ContextPort；Main/Sub execution 只消费 ContextWindow 并按 finalized Step append，Runtime 租借投影降为 idle/session compatibility，由 #872/#879 承接退役 | [#876](https://github.com/rushsinging/aemeath/issues/876) |
