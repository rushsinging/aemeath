...
```text
context.rs                  # 窄 façade：ContextPort + composition-only wiring
context/
├── session.rs              # Session / ChatChain / identity façade
├── session/                # 仅当多文件共同变化时展开 persistence / resume
├── compact.rs              # 五级管线总协调
├── compact/                # budget_reduction / snip / microcompact / collapse / auto
├── token_budget.rs         # Token Budget 单一真相
├── prompt.rs               # 私有 PromptPipeline
├── prompt/                 # guidance source adapter / scan / render（按证据展开）
└── memory_injection.rs     # render / placement / token budget / dedup
```

这是按 [代码组织规范](../../01-system/06-code-organization.md) 给出的非规范性能力投影：小能力可继续保持单文件，只有共同变化与独立测试证据成立时才展开子目录；`port/` / `api/` **NEVER** 是固定层。Context 的 crate 升格、物理路径、迁移期白名单与 Current 映射只在系统 crate 决策和 [Migration Governance](../../03-engineering/03-migration-governance.md) 记录，本文不复制现行 guard shape。
