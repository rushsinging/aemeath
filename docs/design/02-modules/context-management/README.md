...
Context Management 采用 Hexagonal + Clean 作为 crate 内部默认组织方式（`domain ← application ← ports ← adapters`）。领域策略（ChatChain、Compact 管线、Token Budget、Prompt Pipeline）收在 `domain`，用例编排（ContextPort 四方法的实现逻辑）收在 `application`，对外端口（ContextPort、GuidanceSourcePort）定义在 `ports`，技术 adapter（文件 I/O、Guidance 解析、Memory integration）终止在 `adapters`：
