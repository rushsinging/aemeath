...
2. **ContextPort OHS**：Context Management 通过 `ContextPort` 向 Runtime 开放能力，Runtime 只调 5 个方法（`build_window` / `needs_compaction` / `compact` / `compact_status` / `append_and_persist`），不接触 Session 内部结构。
...
├── application/                        # 用例编排
│   ├── build_window.rs                  #   build_window 实现（L2-L4 投影 + prompt + memory）
│   ├── compact.rs                       #   compact / needs_compaction 实现
│   └── append.rs                        #   append_and_persist 实现
├── ports/                              # 对外端口定义
...
- Session 持久化策略在 `domain/session.rs` 拥有不变量，技术 adapter 终止在 `adapters/session_storage.rs`；
...
| 端口 | 方向 | 消费方 | 职责 |
|---|---|---|---|
| `ContextPort` | Context-owned OHS（对外） | Agent Runtime | async `build_window` / `compact` / `compact_status` / `append_and_persist` 与纯 `needs_compaction` |
...
Runtime 与 Context Management 的上下文交互经 5 个方法：
...
| `compact_status` | 查询 L5 派生状态 | 返回 coverage、pending tokens、reused shards、phase、circuit breaker 与 compact usage；不暴露 sidecar |
