...
trait ReflectionHistoryQuery: Send + Sync {
    ...
    /// 仅返回安全摘要；完整 record 保持在 Memory adapter 内部。
...
- `/reflect [limit]` 经 Runtime/SDK 将 `ReflectionSafeSummary` 映射为交付 DTO：id、timestamp、trigger、status、三个数量、apply status、error category、token usage、duration。完整 `ReflectionRecord`、prompt、raw response、对话、Memory content 与 output 正文 **NEVER** 越过 Memory query 边界。
