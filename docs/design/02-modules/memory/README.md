...
```text
memory.rs                  # 窄 façade：Memory PL / ports + composition-only wiring
memory/
├── entry.rs               # MemoryEntry 与局部不变量
...
├── reflection/            # 仅在 prompt / parse / apply 已独立变化时展开
└── persistence.rs         # Memory-owned storage ACL；真实外部 seam
```

这是按 [代码组织规范](../../01-system/06-code-organization.md) 给出的非规范性能力投影。起步时可全部共置在 `memory.rs`；只在用例已有独立变化与测试证据时拆文件，`reflection/` 也不预先建空目录。`persistence.rs` 存在是因为持久化是可替换外部 seam，不是为了补齐通用 adapter 层；`api/` / `port/` **NEVER** 是固定目录。
