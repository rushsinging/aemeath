...
Memory 采用 Hexagonal + Clean 组织（`domain ← application ← ports ← adapters`）。MemoryEntry 等共享不变量收在 `domain`，write/retrieve/compact/reflection 用例编排收在 `application`，对外端口定义在 `ports`，持久化 integration 和技术 detail 终止在 `adapters`：
...
│   ├── retrieve.rs                 #   eligibility / scoring / BM25 / search 策略
│   ├── compact.rs                  #   eviction + archive 共同换代策略
│   └── reflection.rs               #   prompt / parse / apply 策略
├── application/                   # 用例编排
│   ├── write.rs                    #   write 用例编排
