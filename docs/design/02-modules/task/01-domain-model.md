...
- Context Management 先取得 exclusive session-switch lease，再调用 `prepare_restore` 完整校验并构造 opaque token；同一 lease 阻止 Task mutation 发生在 token 生成与 commit 之间。`commit_restore` 随后无失败地**全量替换**内存状态（清空后写入）。
