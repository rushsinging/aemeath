...
Task-owned wire envelope 当前版本为 V2。V1 是无 Task schema version 的 legacy 对象；reader 兼容 V1，新 writer **MUST** 只输出 V2：

```text
{ "schema_version": 2, "revision": "…", "tasks": […],
  "next_task_id": "…", "next_batch_id": "…",
  "current_batch": "…" | null, "batches": […] }
```

| 字段 | 类型 | 说明 |
|---|---|---|
| `schema_version` | `u32` | V2 固定为 `2`；future version fail-closed |
| `revision` | `TaskRevision` | snapshot 对应完整 state 的权威提交版本；V1 缺失时升级为 `0` |
| `tasks` | `Vec<PersistedTask>` | 所有非 Deleted Task 的持久化 DTO；只含权威字段，**NEVER** 含派生 `blocks` |
| `next_task_id` | `TaskId` | 下一个待分配 Task ID；V1 的 `next_id` 由 Task-owned codec 升级 |
| `next_batch_id` | `BatchId` | 下一个待分配 Batch ID；V1 缺失时由最大已存 BatchId + 1 派生 |
| `current_batch` | `Option<BatchId>` | 唯一 Active Batch；没有 Active 时为 `None`，**NEVER** 使用 0 哨兵值 |
| `batches` | `Vec<PersistedBatch>` | 所有 Batch 权威字段；运行时 `Batch` 不直接 serde |

V2 的 Task/Batch/counter/revision ID 均使用非零（revision 可为零）十进制字符串。V1 reader 接受历史 Task `id: String` 与 Batch/counter 数字组合；V2 envelope 内出现数字 ID 或同一 V2 文档混用数字/字符串时必须拒绝。对象未知字段忽略以允许 additive compatibility，但缺失 V2 必填字段、非法枚举或无法表示的值必须返回 typed decode error。

### 5.2 快照边界

- 快照收集时**过滤 Deleted 状态**的 Task（不落盘已删除任务）；`TaskAccess::delete` 已先原子移除其全部依赖边，因此任何合法 live state 收集出的快照 **MUST** 不含 dangling reference，并能被同版本 validator 接受。
- `PersistedTask` / `PersistedBatch` **MUST** 显式列出权威字段；`TaskSnapshot` **NEVER** 直接 serde 运行时 `Task` / `Batch`。`blocked_by` 是唯一持久化依赖事实，validator 只从已验证的 `blocked_by` 构造全部反向 `blocks`；旧 wire 若携 `blocks` 作为未知字段会被忽略。
- codec 与 aggregate validation 分阶段执行：codec 只负责 JSON/version/wire vocabulary/ID 表示升级；validator 负责 ID 唯一、非零、引用、DAG、Batch/current_batch、next ID、状态/时间戳与 revision 不变量。两阶段都不接触 live store。
- `#888` 提供 Task-private 的候选 state 构造与原子 install seam：完整验证成功后才在一把同步锁内一次替换 `TaskStoreState`（其中包含 revision），失败时 live state/revision 完全不变。该 seam 不构成公开 persistence port。
- `#890` 才发布 `TaskPersist`、opaque prepare/commit token 并由 Context Management 在 exclusive session-switch lease 内协调；Session 落盘和 absent/captured-empty ACL 也不由 `#888` 接线。
...
wire 格式解析与 typed state 校验是两个阶段：Task-owned codec 先把 raw JSON 的 V2 全字符串格式或 V1 历史格式转换为 `TaskSnapshot`，拒绝 invalid V2 / mixed V2 representation；纯 validator 接收强类型快照，校验 ID 唯一且实体 ID 非零、引用、环、状态/时间戳组合，以及两个 next ID 严格大于各自已存最大值。`TaskRevision(0)` 合法并随候选 state 一起安装；revision 不参与实体 ID 的非零约束。
...
| 2026-07-17 | #888 锁定 V1 reader / V2 canonical envelope、revision wire、unknown-field 策略、纯 typed validation 与内部 validate-then-swap seam；正式 TaskPersist/恢复协调仍归 #890 | [#888](https://github.com/rushsinging/aemeath/issues/888) |
