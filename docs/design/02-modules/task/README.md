...
判定依据见[代码组织规范](../../01-system/06-code-organization.md) §3.1 / §3.2 与 [02-modules 目录结构决策](../README.md#目录结构决策)：`transition`、`dependency`、`batch`、`snapshot` 共同守护同一个 `TaskStoreState` 聚合，在同一次 mutation 内锁步变化、共享同一状态所有权，**NEVER** 构成三个以上互不相关、可独立测试演进的候选能力；因此 Task **MUST** 保持单能力扁平叶子（§3.2），**NEVER** 建立私有 `capabilities/` 竖切容器：
...
- 这是按 §3.2 给出的扁平能力投影，不是必须一次创建的目录清单；小实现可以从 `task.rs` 开始，只在某个用例已有独立共同变化与测试边界时才拆文件。
- Task **NEVER** 建立 `capabilities.rs` / `capabilities/`：`state` / `transition` / `dependency` / `batch` / `snapshot` 都不是独立候选能力，只是同一聚合不变量下共同守护 `TaskStoreState` 的用例文件；为对称性预建 `capabilities/` 违反[代码组织规范](../../01-system/06-code-organization.md) §2 全局不变量 4。
- `port/` / `api/` / `published_language/` **NEVER** 作为固定横向层；窄契约（`TaskAccess` / `TaskPersist`）与所有者 façade 共置在 `task.rs`。
