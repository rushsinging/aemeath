...
`#887` 只建立 `TaskAccess`、revision 与单一事务 backing。`#888` 定义 Task-owned V1/V2 wire codec、`TaskSnapshot`、纯 typed validator 与 Task-private 的 validate-then-swap install seam；它不发布跨 BC persistence capability。`#890` 才发布 `TaskPersist`、opaque prepare/commit token，并完成 Context/Composition 接线。

因此下列 `PreparedTaskRestore` / `TaskPersist::prepare_restore` / `commit_restore` 是 **#890 Target**，不是 #888 的公共 API。#888 的候选 `TaskStoreState` 只能在 Task crate 内构造和安装，外部不能绕过 validator：
...
| PL 类型 | 说明 | 消费方 |
|---|---|---|
| `Task` | 运行期命令 / 查询 read model；含派生 `blocks`，**NEVER** 用作 Session 持久化 DTO | Runtime |
| `TaskView` | Task-owned、只读、稳定 serde 输出视图；`TaskGetResult` / `TaskListResult` 共用这一唯一字段集，wire 契约由 Tools L3 schema + 精确 JSON 测试锁定，不含 `owner` 或可变字段权限 | TaskTool → Runtime adapter |
...
- 需要进入 SDK event 的 read model 可实现 serde；Session 持久化只编码 `TaskSnapshot` 内的专用 wire DTO，**NEVER** 直接编码运行时 `Task` / `Batch`。运行时 opaque token 不实现 serde。
- V2 枚举使用稳定 `snake_case`；Task/Batch/counter/revision ID 写为十进制字符串。对象未知字段忽略以支持 additive compatibility，V2 必填字段、非法 enum 与 future schema version fail-closed。
- V1 reader 接受无 Task schema version 的历史结构：Task ID 字符串、Batch/counter 数字、`current_batch=0` 哨兵、缺失 `next_batch_id` 与新增字段。V2 envelope **MUST** 拒绝数字 ID 或 mixed representation；Context 的 Session reader 只委托 Task codec，不解析 Task / Batch ID。
- 新 writer 只输出 V2，tasks/batches/blocked_by/tags 按稳定顺序编码；同一逻辑 state 必须产生相同 bytes。
