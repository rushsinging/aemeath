...
| INV-4 | 栈非空时禁止 enter（不允许嵌套 worktree） | `enter` 检查 `stack.is_empty()`；仅当 control 用例的 fallible probe 明确返回非 linked worktree 时可清理残栈，probe 失败或仍为 linked 时 fail-closed |
...
`enter` 的目标由 `resolve_worktree_path` 统一解析：

- `path` 为非空 `PathBuf` 时，绝对路径原样使用，相对路径基于当前 `path_base` 拼接。
- `path` 为 `None` 或 empty `PathBuf` 时，**MUST** 从非空 `branch` 推导
  `path_base/.worktrees/<safe-branch>`；`path` 与 `branch` 均缺失或为空时返回
  `MissingPathAndBranch`。

`enter` 的 optional `base` 默认值由 Project domain 唯一维护：`None`、空串或纯空白
**MUST** 归一为 `DEFAULT_WORKTREE_BASE`（`main`）；显式非空值原样作为新 worktree 的
创建起点。`base` **MUST** 仅在目标路径不存在、需要创建新 worktree 时使用；进入已有
linked worktree 时忽略。
...
当 `enter` 发现栈非空时，control 用例 **MAY** 通过 fallible `GitWorktreeOps::is_linked_worktree(path_base)` 复核残栈；只有明确返回 `false` 才可清栈并继续。返回 `true` 必须以 `NestedWorktree` fail-closed，probe 失败必须返回结构化 Git 错误且 live state 保持不变。普通 `WorkspaceRead::in_worktree()` 只读已提交 `worktree_kind`，**NEVER** 执行该自愈 probe。
...
本文描述最终聚合模型，不表示单个 leaf 同时实现全部 Target：

- #892 只落地 `WorkspaceService` 私有化、唯一 state slot、独立 control-operation mutex、state lock 不跨 I/O、candidate 一次提交、私有 Git seam、opaque wiring 与 fork 隔离。
- #893 承接 Composition/Runtime/Tool/Context 的窄 view 消费切换，并负责在 Context Management exclusive session-switch gate 内调用 Project restore 协议。
- #894 承接 `ProjectIdentity`、`WorkspaceId`、`WorktreeKind`、NonGit、snapshot DTO 与 `prepare_restore` / `commit_restore`。#892 **NEVER** 为提前满足该 Target 而复制临时 identity 或 restore 状态机。
...
| 2026-07-17 | 锁定残栈自愈的 fail-closed 条件，并拆清 #892 内部锁/wiring、#893 消费/gate、#894 identity/snapshot 的实施责任 | [#892](https://github.com/rushsinging/aemeath/issues/892) / [#893](https://github.com/rushsinging/aemeath/issues/893) / [#894](https://github.com/rushsinging/aemeath/issues/894) |
| 2026-07-21 | 对齐 EnterWorktree 的 empty path 推导、Project-owned base 默认值与 worktree 错误分类 | [#1297](https://github.com/rushsinging/aemeath/issues/1297) |
