...
| **seed_isolated** | 子 agent 如何继承工作区上下文 |
...
4. **seed_isolated 隔离范式**：子 agent 从父 agent 当前快照派生独立实例——继承 `workspace_root` + `path_base` + `initial_cwd`，空栈，独立锁，共享 git 端口。子 agent workspace 能力 ≤ 父 agent。
...
- **MUST** 子 agent workspace 能力 ≤ 父 agent（seed_isolated 只继承位置，不继承栈）。

## 7. 文档导航

| 文档 | 内容 |
|---|---|
| [01-domain-model.md](01-domain-model.md) | Workspace 聚合根、Frame 栈、状态转换规则、seed_isolated、错误模型 |
...
| 日期 | 变更 | 关联 |
|---|---|---|
| 2026-07-12 | 初稿：Workspace 聚合根、Frame 栈、seed_isolated、三端口、git 上下文供给 | #791 |
