...
- `api.rs` **MUST** 是 Project 的唯一跨 feature 公开入口，只发布 `WorkspaceRead` / `WorkspaceControl` / `WorkspacePersist` 及其稳定 Published Language。
- `workspace.rs` 与 `workspace/` **MUST** 共同拥有 workspace 状态、转换规则和三个对外端口；`WorkspaceService` 仍是唯一可变状态源。
...
| 2026-07-14 | 对齐 capability-first Project 目标树、Rust 2018+ 模块形状、窄公开 façade 与 `WorkspaceRead` 分支查询契约，实现路径收口到 Migration Governance | [#972](https://github.com/rushsinging/aemeath/issues/972) |
