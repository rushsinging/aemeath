...
| **ExitWorktree Tool** | `exit` | 用户 / agent 请求退出 worktree（无参数，纯弹栈） |
...
- `change_directory` **MUST** canonicalize 并验证路径存在且位于当前 `workspace_root` 内，再委托 Project 私有 `set_path_base` 转换；`enter` / `exit` **MUST** 通过 Project 内部转换统一守护 Project identity、包含关系与 repository identity。
...
- `ExitWorktree` 无输入参数：退出即弹栈恢复最近一次 `enter` 前的上下文，**NEVER** 提供直接切换到任意路径的 Control 用例。
- NonGit project 只支持 `change_directory`；`enter` / `exit` **MUST** 返回 `UnsupportedForNonGit`，**NEVER** 尝试 git 命令或制造 frame。
...
- #892 交付本 crate 的目标物理结构、私有 `GitWorktreeOps` / `GitCli`、crate-private 注入构造、`wire_production_workspace` / opaque `WorkspaceWiring`、唯一 state slot、同步 control-operation mutex、I/O 外 state lock 与 fork 隔离。
- #893 交付 `CompositionWorkspaceScope` 生命周期、factory 的唯一跨 crate 消费点、逐 Tool 窄 view 装配及 Context Management exclusive session-switch gate 编排。
- #894 交付 `ProjectIdentity` / `WorkspaceId` / `WorktreeKind` / NonGit、`WorkspacePersist` 的完整 DTO 与 prepare-commit restore 协议，以及 legacy Session ACL。
- #982/#1022 交付 factory/handle 仅 Composition 可消费、具体 service/adapter 零泄漏等机械守卫和故意违规证明。

Target API 仍以本文为唯一真相，但实施 leaf **MUST** 按上述顺序依赖，**NEVER** 在 #892 中新增临时 Published Language、半升级 DTO 或 Project-owned session-switch gate。

## 8. 目标代码组织

Project 采用 Hexagonal + Clean 最简形态（`domain + adapters`）。`workspace` 能力收在 `domain`，`git` 外部 seam 终止在 `adapters`，窄 crate-root façade 发布稳定 PL：

```text
project/src/
├── lib.rs                  # 窄 crate façade + composition-only factory
├── domain.rs               # Workspace Published Language、窄 views 与能力根
├── domain/
...
└── adapters/
    └── git.rs              #   GitWorktreeOps + 单一 GitCli adapter；FakeGit 放测试支持中
```

- crate root **MUST** 是 Project 的唯一公开 façade。面向业务消费者的稳定表面 **MUST** 只发布 `WorkspaceRead` / `WorkspaceControl` / `WorkspacePersist` 及其 Published Language；`wire_production_workspace` 与 opaque handle 是仅供 Composition Root 的 wiring 例外，**NEVER** 成为业务契约。
- `domain.rs` 与 `domain/` **MUST** 共同拥有 workspace 状态、转换规则、用例和三个窄 view；稳定 trait **MUST** 靠近其消费用例定义，**NEVER** 为对称性创建空泛 `ports.rs`。每个 Main / Sub workspace context **MUST** 各有且仅有一个 `WorkspaceService`，同一 context 内 **NEVER** 复制第二份可变状态或缓存。
- `adapters/git.rs` **MUST** 将 `GitWorktreeOps` 与单一 `GitCli` adapter 共置在 Project 内部；只有出现多个独立 adapter 或独立演进压力时才升级为 `adapters/git/` 技术子目录，**NEVER** 为单文件集成预建目录，也 **NEVER** 把 git CLI 或 wire detail 泄漏到对外 façade。
- 该结构遵循 [代码组织规范](../../01-system/06-code-organization.md) 的 Hexagonal 默认分层与按证据启用判据。
