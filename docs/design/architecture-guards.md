...
架构守卫是仓库的"机械式宪法"——把 [outline.md](./outline.md) 中"依赖铁律 / COLA 分层 / 薄入口 / 单一真相"等设计原则固化为可执行的静态检查。所有守卫通过 `.agents/aemeath.json` 的 `Stop` 钩子触发，串联执行，**任一失败即阻断会话**。
...
- **守护**：[outline.md](./outline.md) §依赖铁律——固化 feature 依赖方向：cli→{composition, sdk}；runtime→全部 supporting；supporting→share；share/sdk→∅。默认拒绝未声明的业务依赖，防双向/横向乱依赖。
- **白名单（`business_allow`）**：
...
- **例外**：
  - `tools → {project, storage}`：横向依赖登记（[outline.md](./outline.md) §6.4.7），仅经各自 `api` facade 接入。
...
- **守护**：[outline.md](./outline.md) §薄入口——CLI 不得直连 runtime 内部或任何 supporting feature，业务能力一律经 composition 装配 + `sdk::AgentClient` 契约接入。
- **白名单**：
...
- **守护**：[outline.md](./outline.md) §依赖铁律 `share → ∅`——share 是最底层共享内核，禁止反依赖上层。
...
- **守护**：[outline.md](./outline.md) §6.4.5 rule6——kernel 只放数据契约与纯函数。
...
- **守护**：[outline.md](./outline.md) §6.4.8 分层纯度——内层只能内→外、不能外→内；domain/business 不得依赖 core 编排 / gateway / contract；utils 保持叶子。
...
- **守护**：[outline.md](./outline.md) §6.4.5 rule5——`share::adapter` / `shared::adapter` / `agent/shared/src/adapter` 只能在 composition 装配处引用，feature 与 cli 不得直接 import。
...
- **守护**：[tui-design.md](./tui-design.md) §TEA 架构——`update()` 不得直接 `await` / `spawn` / IO / 调 hook。
