...
>
> 状态：原生 sub-issue 树已建立，按实施发现持续维护
...
| #763 全局 Guard + Verify | 治理父 Issue | 汇总设计基线 #972 与 Guard 实现 leaf #982；自身不承载代码 PR |
...
1. Runtime-owned 出站 ports、私有 RuntimeContext、crate-root `agent_client` 窄入站 façade 与 `agent_run` 能力骨架；命名和公开面以 [Runtime 模块边界](../../design/02-modules/runtime/02-module-boundaries.md)及[代码组织规范](../../design/01-system/06-code-organization.md)为准，**NEVER** 预建通用 `api/` 技术层。
...
- #982：正式 Guard、legacy 固定层规则退役与故意违规验证；#763 汇总端到端验收和父级状态。
...
- #982 的每条 Guard 均通过故意违规证明可拦截，#763 已汇总端到端验收结论。
