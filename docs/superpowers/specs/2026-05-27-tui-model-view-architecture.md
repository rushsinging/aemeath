...
### 已收敛切片：Input/status selection

Input/status selection 已完成进一步结构性收敛：`view_state.input_sel` 与 `view_state.status_sel` 是 input/status 两区选区锚点和拖拽状态真相，`InputArea` / `StatusBar` 的 `is_selecting`、`selection_start`、`selection_end`、`selection_row`、`selection_width` 仅作为渲染高亮镜像。鼠标事件只写 view_state；渲染前由 `adapter/input_widget.rs::apply_input_selection_to_widget` 与 `adapter/status_widget.rs::apply_status_selection_to_widget` 单向写回 widget 镜像。

复制选中文本不再依赖 widget 选区镜像的时序：input/status 的 copy path 改为读取对应 `*SelectionViewState` 的归一化区间，再借 `InputArea` / `StatusBar` 的文本投影计算 plain 文本。`InputArea::get_selected_text`、`StatusBar::get_selected_text`、`clear_selection` 仅保留为测试辅助；resize/reset 不再直接清 widget 选区镜像，清选区必须表达为 view_state 真相变更。
...
Spinner/task/queued live status 已完成 Phase 2 结构性收敛：spinner active/phase 真相归 `RuntimeModel.spinner`，task 行真相归 `RuntimeModel.task_status.lines`，排队输入预览真相归 `ConversationModel::queued_submissions`，动画 frame/verb 归 `view_state.spinner`。`OutputArea` 不再持有 `spinner` / `task_status_lines` / `queued_submission_lines` 这三类 live-status widget mirror，也不再保留 `SpinnerState` 这类基于 `Instant` 的 widget 状态。

渲染路径统一为 `App::live_status_view_model()` 从 `RuntimeModel`、`ConversationModel::queued_submissions` 与 `view_state.spinner` 派生 `LiveStatusViewModel`，再由 `OutputArea::render(..., &LiveStatusViewModel)` 直接只读绘制。复制、选区与鼠标坐标换算同样显式接收同一个 `LiveStatusViewModel`，不再读取 widget mirror；`adapter/live_status_widget.rs` 的写回路径已退役，`apply_live_status_to_widget(...)` 不得恢复。对应地，`check-tui-spinner-task-single-source.sh` 已升级为结构规则：禁止恢复三类 `OutputArea` live-status mirror、禁止恢复 `SpinnerState` 与 `apply_live_status_to_widget`，并继续禁止旧 spinner/task widget 方法绕过 `RuntimeModel` / `view_state.spinner`。
...
### Phase 1 真实状态：mirror 写入口收敛，不是终态退役

上述已完成切片的共同结果是：生产写入口已收敛，widget 内部状态已降级为渲染 mirror，并由 guard 防止绕回旧路径。但这**还不是**本节退役判据要求的终态：只要 `InputArea` / `OutputArea` / `StatusBar` 仍物理持有 app/domain 态的第二份可写字段，对应 single-source 守卫就仍是必要脚手架，**不得**宣称已退役。

具体校正：

1. `check-tui-input-single-source`、`check-tui-status-single-source`、`check-tui-spinner-task-single-source`、`check-tui-output-scroll-selection-single-source`、`check-tui-selection-single-source` 当前仍应保留在 `check-architecture-guards.sh` 中。
2. `InputArea` 仍持有 `TextArea<'static>`，因此 input text/cursor 当前只是「`model.input.document` 真相 + widget mirror + adapter 单写入口」，不是「无第二副本」。
3. input/status selection 仍有 widget mirror 字段；output selection/scroll 与 live status 已完成 stateless projection，不再有对应 `OutputArea` widget mirror。
4. TUI conversation/domain 当前仍由 TUI 本地 model 基于 intent/event 维护；尚未完全变为 `AgentClient` snapshot/change stream 的只读投影。

### Phase 2 首切片：InputArea text/cursor 去第二副本

Phase 2 的第一刀只处理 input text/cursor 第二副本，**不**同时拆 selection/history/content_width。目标是让 `InputArea` 不再长期持有文本与光标状态，渲染时只读 `model.input.document` 派生的投影。

#### 验收标准

1. `InputArea` struct 中不得再包含 `textarea: TextArea<'static>` 或任何长期保存 input text/cursor 的字段。
2. `InputArea` 不得提供生产可用的 `set_text`、`set_cursor_byte_index`、`text_snapshot`、`get_text` 等 text/cursor mirror API；测试辅助必须 `#[cfg(test)]` 且不得被生产路径引用。
3. `adapter/input_widget.rs` 不再把 `InputChange::TextChanged` / `InputChange::CursorMoved` 投影到 widget text/cursor mirror；这些 change 只用于失效渲染缓存、滚动/光标可见性等非真相副作用。
4. input 渲染直接消费 `InputModel::document` 派生的 `InputRenderModel`（或等价 view model），由渲染函数在本帧临时构造 `tui_textarea::TextArea` 或等价绘制数据。
5. 光标位置由 `InputDocument` 的 byte cursor 投影为渲染行列；投影逻辑必须集中定义并覆盖 Unicode 边界测试。
6. selection/history/content_width 可暂时保留在 `InputArea` 或现有位置，但必须在文档和 guard 中明确它们不属于本切片验收。
7. `check-tui-input-single-source.sh` 从「禁止乱写 InputArea text/cursor mirror」升级为「`InputArea` 结构上不得持有 text/cursor mirror」。

#### 推荐实现路径

1. 新增 `InputRenderModel`（位置优先在 `apps/cli/src/tui/view_state/` 或 `apps/cli/src/tui/view_assembler/` 附近，按现有依赖方向选择）：包含渲染所需 text lines、cursor byte index/row/col、placeholder、focus、pending images、completion view state 等只读数据。
2. 在 input view assembler / adapter 边界从 `InputModel::document` 构造 `InputRenderModel`。
3. 将 `InputArea::draw` 改为接收 `&InputRenderModel`（可分阶段先新增 `draw_with_model`，再删除旧入口）。
4. 在 draw 内部按帧临时构造 `TextArea` 并设置 cursor；临时对象不得存入 `InputArea` 字段。
5. 将需要文本内容的复制/选区函数改为读取 `InputRenderModel` 或 `InputDocument`，而不是读取 widget snapshot。
6. 删除 `apply_input_changes_to_widget` 中 text/cursor mirror 写入分支，只保留 selection/history/尺寸等本切片未处理的适配。
7. 收紧 input single-source guard 并运行对应 TUI 输入测试与 architecture guard。

#### 明确不做

1. 不在本切片拆 `InputArea` selection mirror；后续以 input/status selection stateless 化单独处理。
2. 不在本切片拆 history/saved_input；后续归位到 `InputModel` 或 app/domain state。
3. 不在本切片替换 `tui_textarea`；允许作为本帧局部绘制 helper 使用，但不得跨帧保存。
4. 不在本切片完成 AgentClient domain snapshot projection；该工作范围更大，应单独设计。

### 与 AgentClient 的关系（双模式通用）

TUI 是**入站 adapter**，目标态只依赖 `dyn AgentClient`（其只读快照 + 变更通道）。domain 真相在 runtime / AgentClient，TUI 投影——结构上不应有第二份。这套收敛对**本地直连**与**远程 server 模式**同样成立（TUI 不区分 `AgentClientImpl` 与远程客户端实现）。当前代码尚未完全达到该目标：conversation/domain 仍有 TUI 本地 model，需作为 Phase 3 单独推进。
