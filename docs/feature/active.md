...
| 75 | TUI Model/View 架构迁移 | 高 | 待确认 | 未确认 | 按 `docs/superpowers/specs/2026-05-27-tui-model-view-architecture.md` 与 M1-M11 plan 分阶段迁移：M1-M5 已建立 `view_model` / `view_state` / `view_assembler`、Conversation/Input/Runtime/Diagnostic model、Effect/EffectResult/Effect boundary；M6-M10 已补齐 root reducer/coordinator、agent event mapper、input/resize/key/mouse mapper，并将 legacy `Cmd` 收敛为 `Effect`；M11 已删除 production path 的 legacy `Cmd` adapter 与旧 effect executor/result 文件，合并 `core::msg::Msg` 到 `update::msg::TuiMsg`，封装 `ChatState` / `InputState` / `SessionState` / `UiLayout` 访问，强化 architecture guards（TEA purity、effect boundary、legacy output、unsafe text/index、文件行数等）并通过相关验证。剩余说明：`core::App` 仍保留 runtime shell 兼容状态 facade，后续若继续迁移可将 facade 内部完全替换为 `TuiModel` + `ViewState`。 |
