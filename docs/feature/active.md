...
2. **share 回归最小共享内核（「收口 pub」根本解，由 #1 认知修正解锁）**：既然 domain 间经 api 门面横向依赖合理，就**不必为「共享」把带行为/状态类型塞 share**——它们可回归各自 domain，消费者经 `<crate>::api` 横向依赖。**移出**（带行为/状态；移走后 share 不再需 `tokio`/`futures` 等，根除 `share_warn_io`）：`tool::ToolRegistry`→tools、`task::TaskStore`→storage/runtime。**保留纯 kernel**（协议无关 value object + 契约 trait）：`error`/`message`/`session_types`/`string_idx`/`tool::{Tool,AgentRunner}` trait + `ToolContext`/`ToolResult`/`Task`/`TaskSnapshot`/`MemoryEntry`/`Skill` DTO/`config`（纯 DTO 无 IO）。这是 D2 share 瘦身的**彻底版**（D2 仅移 fs IO 实现 worktree/MemoryStore/skill loader）。判断点：`Tool` trait `async fn call` 引入 `async-trait`（非 tokio，可接受）。**命名决策**：瘦身后 share 名实相符地回归 spec 原 `core`「最小共享内核」语义（撤销 2026-05-29「share 臃肿不配 core」的 core→share 校订前提）；但**不宜直接叫 `core`**——Rust 内置 `core` crate（no_std）会与之冲突（`use core::` 歧义，疑为 P11 用 share 而非 core 之由）。建议改名 `kernel`/`agent-kernel`（表「最小内核」且无冲突）或保持 `share`；落地瘦身时一并定名并反向校订 047 spec。
