...
| 47 | 以 DDD 思路重新设计 Aemeath 架构 | 高 | 设计中 | 未确认 | DDD 架构设计已按讨论结果写入 [spec](specs/047-ddd-redesign.md)，并已纳入 [GLM review](specs/047-ddd-redesign-review-by-glm.md) 与 [DeepSeek review](specs/047-ddd-redesign-review-by-deepseek.md) 的修正意见。Phase 1/2/3 已完成 Chat application 边界清理；Phase 4 修正版收束 `ChatBootstrap` 与 `ChatModeSelection`，把 `run_chat` 启动准备逻辑移入 bootstrap 边界；后续已持续拆分 `bootstrap_chat` 的 setup helper。当前首轮 `apps/contexts/shared` workspace 目录重排已完成但用户反馈不好理解，设计已修正为严格方案 B：目标顶层采用 `apps/` + `crates/`，`apps/cli` 只直接依赖 `crates/runtime` 和纯技术库，`runtime` 是唯一编排者，supporting domains 默认只依赖 `core`；依赖图、import、public API visibility 与 Stop hook 守卫必须共同防止双向依赖和边界绕过。不恢复 #36 server/agents/proto/infra。`ChatRuntimePort` 的 `?Send` 过渡限制仍建议作为后续独立重构处理。 |
...
**状态**：设计中（当前目录结构反馈不好理解，已转向严格方案 B，等待实施计划）
