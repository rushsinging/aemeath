...
| 68 | 项目指令搜索增强：全局 fallback ~/.claude/CLAUDE.md + 向上/向下 5 级目录探索 | 中 | ✅ 已完成 | 未确认 | 全局指令优先 ~/.agents/AGENTS.md，不存在时 fallback ~/.claude/CLAUDE.md；项目指令从 cwd 向上 5 级祖先 + 向下 5 级子目录搜索 CLAUDE.md/AGENTS.md，每层级 Claude 优先 |
