...
| 71 | AGENTS.md 渐进式披露重构：根指令瘦身 + specs/ 分片按需加载 | 中 | 进行中 | 未确认 | 将单文件全量的根 `AGENTS.md` 重构为「宪法 + 工作流 + 渐进式披露路由表」，detailed 规则下沉到 `specs/` 10 个分片，按路径/场景触发按需加载；顺带校正文档中已漂移的旧 crate 名（`aemeath-core`/`aemeath-llm`/`aemeath-tools` → `agent/features/*`、`agent/shared/*`）|
...
### #67 去除 mod.rs 旧写法 + 架构 guard

**状态**：已完成（22fe614 + d7e31ff + 31ad26c），待确认

**背景**：项目中有 78 个 `mod.rs` 文件，这是 Rust 2015 版的旧约定。Rust 2018+ 推荐使用与目录同名的文件（如 `foo/mod.rs` → `foo.rs` + `foo/`），避免 IDE 中大量同名 tab、改善导航体验。项目已完成 DDD 目录重组（#47），但文件命名仍遗留旧模式。

**目标**：
1. 将所有 `mod.rs` 重命名为父目录同名文件（如 `foo/mod.rs` → `foo.rs`），涉及约 78 个文件
2. 添加架构 guard 脚本，在 CI 或 pre-commit 中检测新增 `mod.rs` 并拒绝

**核心要求**：
1. 纯重命名操作，不修改文件内容
2. 所有 `cargo check`/`cargo test` 通过
3. guard 脚本放在 `.agents/hook/` 下，可在 CI 或 pre-commit 中调用
...
## #70 统一 input_queue 到事件驱动

**现状分析**：
用户输入当前有三条路径，其中 `input_queue`/`QueueDrainPort`/`DrainQueuedInput` 是死代码：
...
`input_queue` 只在路径 2 中被写入（`push_queue`），但 `DrainQueuedInput` 的消费者（`spawn_processing` Done 后、runtime `drain_sources`）在 processing 期间执行，此时 `input_queue` 永远为空——因为 processing 期间正常文本走 `InputEventPort`，slash 命令在非 processing 时才入队但 `pending_slash` 不经过 drain。
...
**涉及文件**（含精确行号，基于 main 分支 `9a005c5`+）：

### SDK 层（`packages/sdk/`）

- **`src/tui.rs:13`** — `pub type QueueFuture<'>` 类型别名，删除
- **`src/tui.rs:30-31`** — `pub trait QueueDrainPort` trait 及其 `drain_queued_input` 方法，删除
- **`src/chat.rs:3`** — `use crate::{ChatInputEventPort, ChatMessage, QueueDrainPort}`，移除 `QueueDrainPort`
- **`src/chat.rs:107`** — `ChatRequest { pub queue_drain: Option<Arc<dyn QueueDrainPort>> }` 字段，删除
- **`src/lib.rs:37`** — re-export `QueueDrainPort, QueueFuture`，删除

### Runtime 层（`agent/features/runtime/`）

- **`business/chat/looping/queue.rs`** — **整个文件删除**。包含：
  - `QueueFuture` 类型别名（L6）
  - `QueueDrainPort` trait（L8-9）
  - `append_queued_input` 函数（L12-31）
  - 3 个测试（L33-127）
- **`business/chat/looping/input_gate.rs:2`** — `use ... queue::{QueueDrainPort, QueueFuture}`，删除
- **`business/chat/looping/input_gate.rs:101`** — `run_gate<Q, I, S>` 的 `Q: QueueDrainPort` 泛型参数，删除
- **`business/chat/looping/input_gate.rs:109-122`** — `drain_sources<Q, I>` 函数的 `Q: QueueDrainPort` 泛型参数和 `queue.drain_queued_input()` 调用（L115-121），删除
- **`business/chat/looping/input_gate.rs:226-230`** — `EmptyQueueDrainPort` struct 及其 `QueueDrainPort` impl，删除
- **`business/chat/looping/input_gate.rs:272-274`** — 测试中的 `TestQueuePort` 及其 `QueueDrainPort` impl，删除
- **`business/chat/looping/input_gate.rs:298,332,360,387,415`** — 测试中 `&EmptyQueueDrainPort` 参数，替换为无 queue 参数的签名
- **`business/chat/looping/loop_runner.rs:15`** — import `QueueDrainPort`，删除
- **`business/chat/looping/loop_runner.rs:29`** — `ChatLoopContext<Q>` 的 `Q: QueueDrainPort` 泛型参数，删除
- **`business/chat/looping/loop_runner.rs:64`** — `process_chat_loop<Q>` 的 `Q: QueueDrainPort` 泛型参数，删除
- **`business/chat/looping/loop_runner.rs:489`** — `run_single_turn<Q>` 的 `Q: QueueDrainPort` 泛型参数，删除
- **`business/chat/looping/loop_runner.rs:510-524`** — 测试 `SequenceQueueDrainPort` 及其 impl，删除
- **`business/chat/looping/loop_runner.rs:668`** — 测试中 `SequenceQueueDrainPort::new(...)` 调用，删除
- **`business/chat/looping.rs:25`** — import `EmptyQueueDrainPort`，删除
- **`business/chat/looping.rs:30`** — `pub use queue::{append_queued_input, QueueDrainPort, QueueFuture}`，删除
- **`business/chat.rs:9`** — import `EmptyQueueDrainPort, ... QueueDrainPort, QueueFuture`，删除
- **`business/chat.rs:10`** — import `QueueDrainPort, QueueFuture`，删除
- **`core/client/event.rs:42-60`** — `RuntimeQueueDrainPort` struct + `new` + `QueueDrainPort` impl，删除
- **`core/client/event.rs:261-303`** — 测试 `CountingQueueDrainPort` 及 2 个测试函数，删除
- **`core/client/trait_chat.rs:9`** — import `RuntimeQueueDrainPort`，删除
- **`core/client/trait_chat.rs:23,43`** — `queue_drain` 传递和 `RuntimeQueueDrainPort::new(queue_drain)`，删除

### CLI/TUI 层（`apps/cli/`）

- **`tui/app/state/input.rs:10`** — `pub input_queue: VecDeque<String>` 字段，删除
- **`tui/app/state/input.rs:17-18`** — `clear_queue()` 方法，删除
- **`tui/app/state/input.rs:21-23`** — `push_queue()` 方法，删除
- **`tui/app/state/input.rs:26-27`** — `drain_queue()` 方法，删除
- **`tui/app/state/input.rs:30-31`** — `queued_count()` 方法，删除
- **`tui/app/state/input.rs:34-35`** — `queued_front()` 方法，删除
- **`tui/app/state/tests.rs:78`** — `assert!(state.input_queue.is_empty())` 断言，删除
- **`tui/app/event.rs:97-98`** — `DrainQueuedInput { reply_tx: oneshot::Sender<Vec<String>> }` 枚举变体，删除
- **`tui/app/update/enter.rs:33`** — `self.input.push_queue(input.clone())`，改为 `SendChatInputEvent`
- **`tui/app/update/enter.rs:34-38`** — `UpdateResult { pending_slash: Some(input) }` 返回值，需评估是否改为事件驱动
- **`tui/app/update/key_nav.rs:24`** — `app.input.push_queue(command.clone())`，改为 `SendChatInputEvent`
- **`tui/app/update/key_nav.rs:25-29`** — `UpdateResult { pending_slash: Some(command) }` 返回值，需评估
- **`tui/app/update/ui_event.rs:142`** — `self.input.clear_queue()` 调用，删除
- **`tui/app/update/ui_event.rs:283-292`** — `UiEvent::DrainQueuedInput` arm 整个删除
- **`tui/app/update/spawn_context.rs:16`** — `queue_request_tx: ui_tx.clone()`，删除
- **`tui/app/runtime.rs:9`** — `self.input.clear_queue()` 调用，删除
- **`tui/effect/session/processing.rs:6-35`** — `TuiQueueDrainPort` struct + `new` + `drain_queued_input` + `QueueDrainPort` impl，全部删除
- **`tui/effect/session/processing.rs:184`** — `SpawnContext.queue_request_tx` 字段，删除
- **`tui/effect/session/processing.rs:196-197`** — `queue_drain: Some(Arc::new(TuiQueueDrainPort::new(...)))`，删除
- **`tui/effect/session/processing.rs:212`** — `let queue = TuiQueueDrainPort::new(ctx.queue_request_tx)`，删除
- **`tui/effect/session/processing.rs:219-231`** — Done 后 `queue.drain_queued_input()` 及 `sync_current_messages`，删除
- **`tui/effect/session/processing.rs:285-336`** — 3 个 `test_drain_queued_input_*` 回归测试，删除

### 注意事项
- `enqueue_submission_echo`/`clear_queued_submission_echo`（`notice.rs`）需保留——processing 期间仍需显示排队块
- `pending_slash` 机制需评估是否一并移除，改为事件驱动处理 slash 命令
- runtime `drain_sources` 简化为只 drain `InputEventPort`
- `ChatInputEventPort`/`TuiInputEventPort`/`InputEventDrainPort` 这些是**保留**的事件驱动端口，不要误删
- 所有测试需同步更新

### #71 AGENTS.md 渐进式披露重构

**状态**：进行中

**背景**：根 `AGENTS.md`（`CLAUDE.md` 软链指向它）当前是单文件全量约 185 行，宪法 + 项目结构 + 编码规范 + 架构约定 7 节 + 验证门禁 + 日志 + 测试 + Bug/Feature 联动一次性全部注入，无论当前干什么活都全量加载。参考 `wanaka-platform` 的 Progressive Disclosure 做法，根文件应只保留全局始终适用的「宪法 + 工作流 + 路由表」，detailed 规则按工作范围下沉到 `specs/` 分片，按需加载。

**附带发现（文档漂移）**：现有「架构约定」章节大量引用 `aemeath-core/src/command/`、`aemeath-core/src/provider.rs`、`aemeath-llm/src/providers/`、`aemeath-tools`、`aemeath-core/src/error.rs`、`aemeath-core/src/tool.rs` 等旧 crate 名，但当前 workspace 早已是 `agent/features/*` + `agent/shared/*` 的 DDD 分层，这些 crate 已不存在。本次重构顺带校正为真实路径。

**核对到的真实路径**（替换旧 crate 名的依据）：
- slash 命令系统：`agent/features/runtime/src/core/command/`（`CommandDescriptor` + `inventory::submit!` + `registry.rs`）
- `AemeathError`/`ErrorDisplay`/`is_retryable`：`agent/shared/src/error.rs`
- `ToolRegistry`/`Tool` trait：`agent/features/tools/src/core/tool_registry.rs`、`contract/tool.rs`
- MCP：主体 `agent/features/tools/src/business/mcp*`，加载器 `agent/features/runtime/src/utils/bootstrap/mcp_loader.rs`，adapter `agent/shared/src/adapter/mcp.rs`
- provider 实现：`agent/features/provider/src/business/providers/{anthropic,ollama,openai_compatible}.rs`
- provider 默认值/base_url/env/默认 model + 配置分层 + Claude Code 兼容：`agent/shared/src/config/`（`config.rs`、`paths.rs`、`hooks.rs`、`models/`）
- pricing/成本：`agent/features/runtime/src/business/cost/pricing.rs`；token 估算：`agent/features/runtime/src/business/compact/token_estimation.rs`
- Guidance 系统：`agent/features/prompt/src/business/guidance/`
- hook 执行环境注入（`CLAUDE_PROJECT_DIR`/`AEMEATH_PROJECT_DIR`）：`agent/features/hook/src/business/hook/runner.rs`

**目标**：
1. 根 `AGENTS.md` 瘦身为：标题 + Constitution（保持原 5 条不变，不夹带新宪法条款）+ 项目结构 + 运行时目录 + 工作流约束 + 渐进式披露段（含路由表）+ 开放决策。
2. 新建 `specs/` 10 个分片，按真实 crate 边界划分，每个 spec 顶部带 scope 声明：
   - `rust-coding.md`（横切 `**/*.rs`：编码/测试/日志/验证门禁/错误处理）
   - `tui-cli.md`（`apps/cli/src/**`：TUI/REPL、update+Cmd 副作用、ToolDisplayEntry）
   - `runtime.md`（`agent/features/runtime/**`：Agent 循环、tool 编排、token budget、compact、pricing/成本、slash 命令）
   - `tools.md`（`agent/features/tools/**`：Tool trait、ToolRegistry、MCP 主体）
   ...
   - `storage.md`（`agent/features/storage/**`：memory、task、history、tool_result_storage）
   - `policy-hook-audit.md`（`agent/features/{policy,hook,audit}/**`：权限、hook 执行环境、审计）
   - `bug-feature-tracking.md`（无路径触发：docs/ 编号/状态/归档门禁）
3. 「命令系统」并入 `runtime.md`（用户决定不单列）。

**验收标准**：
1. 根 `AGENTS.md` 不再包含 detailed 规则，仅保留全局适用内容 + 路由表；行数显著下降。
2. `specs/` 下 10 个分片齐全，每个 spec 顶部有 scope 声明，正文不含旧 crate 名。
3. 路由表每行的主触发路径在仓库中真实存在（可被 `ls`/`find` 命中）。
4. 现有 AGENTS.md 中所有规则在新结构中均有对应落点，无规则丢失。
5. 纯文档变更，无 cargo 验证项；以「规则零丢失 + 路径真实」为验收。

---

## #72 Edit diff 显示真实文件行号

**现状**：Edit 工具结果的 diff 渲染中，行号是 `old_string`/`new_string` 片段内部从 1 开始计数的相对行号。例如修改文件第 100-105 行，diff 显示的行号是 1-5 而非 100-105，用户无法直接定位到文件中的真实位置。

**状态**：待确认

**改造目标**：diff 显示文件中的真实行号。

**实现**：
1. `FileEditTool` 在写入前通过 `start_line_of_match(content, matched_old)` 计算匹配片段在原文件中的 1-based 起始行号。
2. Edit 工具结果 diff 标记从 `---DIFF---` 扩展为 `---DIFF:LINE:{start_line}---`，前后两个标记都携带同一起始行号。
...
**数据流**：
1. `agent/features/tools/src/business/file_edit.rs` — Edit 工具执行替换后，在 `---DIFF---` 标记中附带 `matched_old` 在原文件中的起始行号
2. `apps/cli/src/tui/render/output/blocks/edit_diff.rs` — 解析新标记格式，提取起始行号
3. `apps/cli/src/tui/render/output/diff.rs` — `build_diff_lines` 接受起始行号偏移，行号从起始行开始计数

**涉及文件**（含精确行号）：

### 数据生产侧（`agent/features/tools/`）

- **`src/business/file_edit.rs:146-164`** — 替换成功后的 `ToolResult::success` 构造：
  - 新增：在替换前用 `content.find(&matched_old)` 计算 `matched_old` 在原文件中的起始行号（1-based），即 `content[..pos].lines().count() + 1`
  - 修改 diff 标记格式：`---DIFF---` → `---DIFF:LINE:{start_line}---`，前后两个标记都带行号
  - `replace_all` 场景：多次替换需收集每个 occurrence 的起始行号，或简化为只显示第一次的位置（因 replace_all 时 matched_old 全部相同，行号连续递推）

### 解析侧（`apps/cli/src/tui/render/output/blocks/edit_diff.rs`）

- **`edit_diff.rs:10`** — `DIFF_MARKER` 常量：改为解析 `---DIFF:LINE:N---` 格式
- **`edit_diff.rs:13-16`** — `EditDiff` struct：新增 `start_line: usize` 字段
- **`edit_diff.rs:29-38`** — `parse_edit_diff`：解析新标记格式，提取 `LINE:N` 中的行号。向后兼容：无 `LINE:` 后缀时默认为 1（旧行为）
- **`edit_diff.rs:79-87`** — `render_edit_diff`：将 `parsed.start_line` 传递给 `diff` 函数

### 渲染侧（`apps/cli/src/tui/render/output/diff.rs`）

- **`diff.rs:17-22`** — `build_diff_lines` 签名：新增 `old_start: usize, new_start: usize` 参数
- **`diff.rs:32-33`** — `old_line` / `new_line` 初始化：从 0 改为 `old_start - 1` / `new_start - 1`（因为循环中先 +=1 再使用）
- **`diff.rs:28`** — `line_num_width`：基于 `max(old_start + old_line_count, new_start + new_line_count)` 而非纯行数

### 原语转发侧（`apps/cli/src/tui/render/output/primitives/diff.rs`）

- **`diff.rs:8`** — `diff` 函数签名：新增 `start_line: usize` 参数，转发给 `build_diff_lines`

### 测试

- `agent/features/tools/src/business/file_edit.rs` 测试 — 更新 `---DIFF:LINE:N---` 格式断言
- `edit_diff.rs` 测试 — 覆盖新旧两种标记格式的解析
- `diff.rs` 测试 — 覆盖非 1 起始行号的渲染
- `primitives/diff.rs` 测试 — 转发新参数

**注意事项**：
- 向后兼容：旧格式 `---DIFF---` 等价于 `---DIFF:LINE:1---`，解析时 fallback
- `replace_all` 场景：多次替换的 diff 片段是同一段 old/new（只替换了一个），行号取首次出现即可
- `line_num_width` 计算需考虑起始行号偏移后的最大值，确保列宽对齐
