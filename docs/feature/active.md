...
| 36 | Multi-Agent 框架 | 高 | 实施中 | 未确认 | Sprint 0/Sprint 0.5 已完成；Sprint 1 已完成 Workspace/Chat API 最小切片：REST Workspace/Chat/Message、PATCH Chat、REST/gRPC AnalyzeMessage、WebSocket BoardSnapshot 与新消息增量推送、ChatService/BoardService gRPC 骨架、gateway 配置与 idempotency_key 去重；下一步收口 WS payload 到正式 BoardSnapshotUpdate、补 GET messages 分页与 EventBus 骨架。详见 [spec](specs/036-multi-agent-framework.md) |
