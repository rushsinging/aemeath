...
| 27 | 日志分化：input.log / output.log / tool.log | 中 | 🔧 实施中 | 未确认 | agent 交互日志从 `aemeath.log` 分离为三个 JSON 文件：input.log（LLM 输入快照）、output.log（LLM 完整输出）、tool.log（工具调用请求+结果）。日志目录移至 `logs/`，`aemeath.log` 收窄为应用诊断日志。详见 [spec](../superpowers/specs/2026-05-09-log-split-design.md) |
...
**目标**：agent loop 结束时（正常结束、用户打断、API 错误、达到 MAX_TURNS 等所有退出路径）执行统一收尾流程，确保 task 状态一致性、资源释放、session 持久化和日志审计完整。当前所有退出点几乎无收尾逻辑。

#### 当前退出路径分析

| # | 退出路径 | 触发位置 | 当前清理 | 缺失 |
|---|----------|----------|----------|------|
| 1 | 正常结束（EndTurn / 无 tool calls） | `stream.rs:390` | reflection、Stop hook | task 未完成项标记、session 保存 |
| 2 | 用户打断（interrupted flag） | `stream.rs:98-113` | messages truncate、Cancelled 事件 | **无 Stop hook**、task 状态清理、资源释放 |
| 3 | API 错误 | `stream.rs:1028-1061` | StopFailure hook、Error 事件 | task 状态清理、资源释放 |
| 4 | 达到 MAX_TURNS（100） | `stream.rs:1063` 循环退出 | Stop hook（有）、messages truncate（有） | task 状态清理 |
| 5 | 用户 Ctrl+C（外层） | `update.rs` CtrlC 处理 | cancel token 触发 | 取消传播到 sub-agent |

#### 收尾工作清单

##### P0（必须）

1. **统一收尾函数**：抽取 `agent_loop_cleanup()` 纯函数，所有退出路径复用
2. **Task 状态清理**：
   - in_progress 的 task → 回退为 pending（可恢复）
   - 或标记为 `Interrupted`（新增状态，表示被用户中断）
   - 全部 completed 的 batch → 归档（与 Feature #25 联动）
3. **Stop hook 统一触发**：用户打断（退出路径 2）也需要触发 Stop hook，携带打断原因
4. **日志收尾**：所有退出路径统一写入 `agent_loop_finished` 日志，包含退出原因、turn 数、耗时、task 统计

##### P1（重要）

5. **Session 持久化**：收尾时确保当前 session 已保存（messages + task_store）
6. **Tool 资源释放**：通知 ToolRegistry 执行全局清理（如 MCP server 连接缓存刷新、临时文件清理）
7. **Sub-agent 收尾**：打断时向所有运行中的 sub-agent 发送 cancel 信号，等待其自行收尾（超时后放弃）
8. **Cost 统计摘要**：收尾时在 agent.log 写入本次对话的 token 用量 + 估算成本

##### P2（可选，后续扩展）

9. **反思触发**：非正常退出（打断、错误）时也触发反思，记录"未完成原因"
10. **Memory 写入**：关键决策和 pitfall 写入 Memory 系统
11. **用户摘要**：在 output area 显示本次对话摘要（turn 数、工具调用数、task 完成率）

#### 实施分解

1. **定义 `AgentLoopExitReason` 枚举**：`EndTurn | Interrupted | ApiError(error) | MaxTurnsReached`
2. **实现 `agent_loop_cleanup()` 函数**：接收 exit_reason、task_store、hook_runner、session 等，统一执行收尾
3. **改造 5 个退出路径**：每个退出点调用 `agent_loop_cleanup()`，传递正确的 exit_reason
4. **Task 状态清理逻辑**：根据 exit_reason 决定 in_progress task 的最终状态
5. **补充 Stop hook 触发**：打断路径增加 Stop hook 调用
6. **单元测试**：覆盖每种退出路径的收尾行为

#### 涉及路径

- `aemeath-cli/src/tui/app/stream.rs`（退出路径改造 + cleanup 函数）
- `aemeath-core/src/task.rs`（TaskStatus 新增 `Interrupted`、批量状态清理接口）
- `aemeath-core/src/hook.rs`（Stop hook 携带 exit_reason）
- `aemeath-cli/src/tui/app/update.rs`（Done/Cancelled 处理中追加收尾）
- `aemeath-cli/src/agent_runner.rs`（sub-agent 的收尾逻辑）

#### 关联

- Bug #27/#29（task 状态不更新）—— 收尾逻辑中 task 状态清理可部分弥补
- Feature #25（task 跨轮次生命周期）—— batch 归档与 auto_clear 可在收尾时触发
- Feature #26（TEA 架构纯化）—— 收尾逻辑也应走 Cmd 描述，而非直接执行
- Feature #9（反思系统）—— 非正常退出时触发反思

#### 开放问题

- Task `Interrupted` 状态是否必要？还是直接回退为 `Pending`（让下次可恢复）？
- Sub-agent 收尾的超时阈值？建议 2s
- 收尾函数是否同步？如果包含 hook 调用则是 async——需要确保在 tokio::spawn 退出前完成
- MAX_TURNS=100 退出时是否应 warn 用户？当前静默退出
