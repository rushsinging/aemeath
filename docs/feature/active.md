...
| 60 | TUI Block 抽象 trait 化 + 真正渲染树（嵌套规则）+ gutter | 高 | 设计中 | 未确认 | 接 #58 留白（2026-05-29 对照 main 二次修订，详见 spec §0）。真实价值（非修 bug）：①BlockComponent trait 抽象（取代自由函数+match）；②ViewModel 升 BlockNode 树，渲染器递归+DFS 展平+嵌套规则表（allowed_child/MAX_BLOCK_DEPTH=3 构造期校验）；③tool result 由"内联富渲染行"升为独立子块（复用现有 render_edit_diff/render_fenced_markdown，非新建）；④行首标志槽 gutter（main 无此能力）：每行固定宽度 [depth 缩进 + marker 列]，marker 按 kind/status 取静态字形（●/✓/✗，UserMessage 等固定字形），仅首行画、后续等宽空白，组合期注入。决策：缓存不折叠子 version、缩进/gutter 不进 plain（需改 render_fenced_markdown 去 indent 契约）、marker 静态不动画。注：#65 已由 main 结构性隔离，本设计仅加固不认领修复；#76 属 #58 域已摘除。详见 [spec](../superpowers/specs/2026-05-29-tui-block-trait-nesting-design.md) |
