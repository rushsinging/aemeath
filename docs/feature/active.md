...
| 65 | Resume 模式输入历史：上下键翻阅过往输入 | 中 | ✅ 已完成 | 未确认 | Resume 会话恢复后，从已加载 session messages 中提取所有 user 文本消息写入 `InputModel.history`；输入框继续复用已有 `MoveHistoryPrevious/MoveHistoryNext` 上下键逻辑翻阅历史，保留当前草稿恢复行为。新增 `ReplaceHistory` intent，补充历史替换/翻阅与 resume 用户输入提取测试。 |
...
1. **补 cross-crate api-boundary guard（「代码硬限制」正解，2026-05-30 认知修正）**：参考 wanaka core-api 的 `no-feature-cross-import`/`no-cross-feature-internals`——**domain 间横向依赖是合理的，只要经对方 published 门面（aemeath 的 `<crate>::api`，对应 core-api 的 `context/`），禁 reach into 内部**。`tools→{project,storage}` 已经经 `project::api`/`storage::api` 门面（合规），**本身非债务**——原「端口注入 / share re-export」判断系**过度设计，撤销**（share re-export 上游还会违反 share-no-upstream guard）。真正缺口：无模块级 guard 强制「跨 crate 只经 `<crate>::api`、禁 reach `<crate>::{business,core,utils,内部mod}`」（现 `check-cargo-dependency-graph.sh` 仅 crate 级）。**待做**：新增 `check-crate-api-boundary.sh`（跨 crate import 只许 `<crate>::api::*`；share 作 kernel 例外；runtime 编排者亦须经 api），接入架构 guard + sanity。
...
### #65 Resume 模式输入历史：上下键翻阅过往输入

**状态**：✅ 已完成，待确认

**需求**：Resume 模式进入后，加载该会话过往所有用户输入作为历史记录，在输入框中按上/下键可翻阅历史输入条目（类似 shell history 行为）。

**完成情况**：Resume 会话恢复后，从已加载 session messages 中提取所有 user 文本消息写入 `InputModel.history`；输入框复用已有 `MoveHistoryPrevious` / `MoveHistoryNext` 上下键逻辑翻阅历史，并保留当前草稿恢复行为。新增 `ReplaceHistory` intent，补充历史替换/翻阅与 resume 用户输入提取测试。

**验证**：`cargo test -p cli`、`cargo clippy -p cli -- -D warnings`、`.agents/hooks/check-architecture-guards.sh` 通过。
