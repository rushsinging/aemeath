...
SDK 层（`packages/sdk/`）：
- `src/tui.rs`：删除 `QueueDrainPort` trait 和 `QueueFuture` 类型
- `src/chat.rs`：`ChatRequest` 删除 `queue_drain` 字段
- `src/lib.rs`：移除 `QueueDrainPort`、`QueueFuture` re-export

Runtime 层（`agent/features/runtime/`）：
- `business/chat/looping/queue.rs`：整个文件删除
- `business/chat/looping/input_gate.rs`：删除 `QueueDrainPort` 泛型参数、`drain_sources` 中 queue 逻辑、`EmptyQueueDrainPort`
- `business/chat/looping/loop_runner.rs`：删除 `QueueDrainPort` 泛型参数、简化 `ChatLoopContext` 和函数签名
- `business/chat/looping.rs`：移除 `QueueDrainPort`/`QueueFuture`/`append_queued_input`/`EmptyQueueDrainPort` re-export
- `business/chat.rs`：移除相关 re-export
- `core/client/event.rs`：删除 `RuntimeQueueDrainPort` 和相关测试
- `core/client/trait_chat.rs`：移除 `queue_drain` 参数传递

CLI/TUI 层（`apps/cli/`）：
- `tui/app/state/input.rs`：删除 `input_queue`、`push_queue`、`drain_queue`、`clear_queue`、`queued_count`、`queued_front`
- `tui/app/state/tests.rs`：删除相关断言
- `tui/app/event.rs`：删除 `DrainQueuedInput` 事件
- `tui/app/update/enter.rs`：slash 命令改用 `SendChatInputEvent`
- `tui/app/update/key_nav.rs`：对话框选模型改用 `SendChatInputEvent`
- `tui/app/update/ui_event.rs`：删除 `DrainQueuedInput` 处理、删除 `clear_queue` 调用
- `tui/app/update/spawn_context.rs`：删除 `queue_request_tx`
- `tui/app/runtime.rs`：删除 `clear_queue` 调用
- `tui/effect/session/processing.rs`：删除 `TuiQueueDrainPort`、`SpawnContext.queue_request_tx`、`spawn_processing` Done 后的 drain 逻辑及 3 个回归测试

**注意事项**：
- `enqueue_submission_echo`/`clear_queued_submission_echo` 需保留（processing 期间仍需显示排队块）
