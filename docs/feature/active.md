...
| 47 | 以 DDD 思路重新设计 Aemeath 架构 | 高 | ✅ 已完成 | 未确认 | DDD 架构设计已写入 [spec](specs/047-ddd-redesign.md)，并按 2026-05-31 target-alignment plan 完成目标态落地：新增 `agent/composition`，CLI 仅经 composition + SDK 接入；`agent/shared` 保持最小共享内核并隔离生产 adapter；runtime/provider/tools/supporting domains 全部迁入 `agent/features/*`；各 feature `api.rs` 收窄为 contract/gateway；目标态架构守卫与 sanity 自测已固化。待用户确认后归档。 |
