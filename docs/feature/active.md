...
| 75 | TUI Model/View 架构迁移 | 高 | 待确认 | 未确认 | 按 `docs/superpowers/specs/2026-05-27-tui-model-view-architecture.md` 与 M1-M18 plan 分阶段迁移：M1-M11 已建立 `view_model` / `view_state` / `view_assembler`、Conversation/Input/Runtime/Diagnostic model、Effect/EffectResult/Effect boundary，补齐 root reducer/coordinator 与事件 mapper，并删除旧 `Cmd` adapter；M12-M18 已接入 `core::App` 双轨 `TuiModel`/`AppViewState` 基线、让 Agent ChatEvent 先进入 ConversationModel reducer、键盘输入/提交先走 InputModel、runtime/session/diagnostic 状态回写 legacy 状态栏、集中 effect runtime 边界、增加 ViewModel→ratatui Line 渲染适配层，并强化/通过 architecture guards。当前仍保留 legacy runtime shell 作为兼容 facade，生产渲染与部分输出状态仍逐步由 legacy 承载；后续彻底删除 facade 需单独 feature。 |
