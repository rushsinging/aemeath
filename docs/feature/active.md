...
| 74 | TUI spinner 时长显示改进 | 低 | ✅ 已完成 | 未确认 | spinner 渲染 verb 后缀从 `…` 改为 `...`；保留 verb 后的总时长（如 `12s`）；phase_text 括号内追加 `⏱ Xs` 计时，让用户直观看到当前 Thinking/Generating/Hook 等阶段已执行多久 |
