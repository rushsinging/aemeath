...
Assistant Pool 大小 = f(pending_requirement_count, max_concurrent_assistant)
```
...
|---|---|---|
| ChatMessage | 用户消息 / Agent 回复的可检索摘要 | ChatMessage 写入时 |
...
| Reflections | 模式总结 + Skill/MCP 产出描述 | Reflection 写入时 |
