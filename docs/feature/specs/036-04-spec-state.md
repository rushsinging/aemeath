...
Scheduler Watch 到 Pending Project
  ...
  Pending ──(分配 Executor)──▶ Assigned
    ...
    - **级联规则**：Project 进入 Cancelled 终态时，该 Project 下所有非终态的 Task（Pending / InProgress / InReview / Retrying）一律级联为 Cancelled；已终态（Completed / Failed / Cancelled）Task 不受影响。Pending Task 需要被级联取消，因为 Project 不再可调度。
...
4. 强制超时：默认 `cancel_timeout_sec`（配置见 036-02 `scheduler.*` 段）。若 Executor 在超时内未回调确认，由 Scheduler 调用 `ProjectService.ForceCancel`（或 `ProjectTaskService.ForceCancel`）RPC，由 API Server 原子执行：设 `status=cancelled`、清零 `cancel_requested_at`、释放 merge_lock、按崩溃恢复处理级联 Task
