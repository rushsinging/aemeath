...
**当前落地状态**：已完成内存版 Workspace/Chat/ChatMessage store、REST Workspace/Chat/Message、PATCH Chat、AnalyzeMessage、WS 初始 BoardSnapshot、ChatService/BoardService gRPC 骨架、gateway compose 配置与 idempotency_key 去重；MongoDB 持久化和 Watch 增量恢复语义留到后续切片。
...
**产出（后端）**：
1. ProjectService gRPC（Create/Update/Get/List/Assign/Accept/Resume/Retry/Complete/Block/Fail/Cancel/ConfirmCancel/ForceCancel/Watch）
2. ProjectTaskService gRPC（Create/Get/Update/Complete/List/Cancel/ConfirmCancel/ForceCancel/Fail/Retry/Watch）
