...
10. `run_orchestration::setup` 已继续把 `bootstrap_chat` 的技术性启动准备拆成局部 helper：`concurrency`、`permissions`、`model_runtime`、`provider_client`、`prompt_bundle`、`runtime_support`、`tooling`。这些模块只表达 CLI bootstrap 过渡边界，不等同于新的领域上下文；后续若要进一步下沉，应先处理 slash command registry 的持锁 await 与 Agent Runtime 边界。
