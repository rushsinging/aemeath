...
impl Tool for GrepTool {
    ...
    async fn call(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let search_path = input.get("path").and_then(|v| v.as_str())
            .map(String::from).unwrap_or_else(|| ctx.cwd.to_string_lossy().to_string());
        ...
        let output = if is_rg_available().await {
            ...
            if let Some(g) = glob_filter { cmd.arg("--include").arg(g); }
        ...
        match output {
            Ok(out) => {
                ...
                if stdout.is_empty() { ToolResult::success("no matches found") }
                else {
...
async fn is_rg_available() -> bool {
    Command::new("rg").arg("--version").output().await
