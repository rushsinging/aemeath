//! 诊断日志格式化：`&log::Record` → 8 字段紧凑 JSON 行。
//!
//! 字段固定：`ts / session / chat / turn / model / level / target / msg`。
...
pub fn format_diag_json_line_from_parts(level: &str, target: &str, msg: &str) -> String {
    let line = json!({
        ...
        "chat": context::current_chat_id().as_deref().unwrap_or("-"),
        "turn": context::current_turn(),
        "model": context::current_model().as_deref().unwrap_or("-"),
        "level": level,
        "target": target,
        "msg": msg,
    });
...
///
/// 输出形态：`{ts, session, chat, turn, model, type, role, ...payload}`。
/// `type` 取自调用方（`"input" | "output" | "tool_call" | "tool_result"` 等），
...
pub fn format_audit_json_line(audit_type: &str, role: &str, payload: Value) -> String {
    ...
    line.insert("ts".to_string(), Value::String(timestamp_rfc3339()));
    line.insert(
        "session".to_string(),
    ...
    line.insert("type".to_string(), Value::String(audit_type.to_string()));
    line.insert("role".to_string(), Value::String(role.to_string()));
...
mod tests {
    ...
    #[test]
    fn audit_line_includes_type_role_and_payload() {
        let payload = json!({ "messages": [{"role": "user"}] });
        let line = format_audit_json_line("input", "default", payload);
        let value: Value = serde_json::from_str(&line).expect("valid json");
    ...
    fn audit_line_merges_payload_fields() {
        ...
        let line = format_audit_json_line("output", "default", payload);
        let value: Value = serde_json::from_str(&line).expect("valid json");
