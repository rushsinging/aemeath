...
use aemeath_core::tool::{AgentProgressEvent, AgentProgressKind, ToolContext};
...
impl<'a> SubAgentRun<'a> {
    ...
    fn progress_turn_start(&self, turn_number: usize) {
        let msg_tokens = aemeath_core::compact::estimate_messages_tokens(&self.messages);
        (self.progress)(
            Some(turn_number),
            &format!(
    ...
    fn progress_api_ok(&self, turn_number: usize, resp: &aemeath_llm::types::StreamResponse) {
        (self.progress)(
            Some(turn_number),
            &format!(
                "API ok: in={} out={} stop={:?}",
    ...
    fn log_output(&self, turn_number: usize, resp: &aemeath_llm::types::StreamResponse) {
        if let Some(ref jl) = self.runner.json_logger {
    ...
    fn send_text_progress(&self, turn: usize, resp: &aemeath_llm::types::StreamResponse) {
        if let Some(ref tx) = self.progress_tx {
    ...
    fn log_tool_calls(&self, turn_number: usize, tool_calls: &[aemeath_core::agent::ToolCall]) {
        if let Some(ref jl) = self.runner.json_logger {
