...
fn content_block_counts(messages: &[Message]) -> (usize, usize, usize, usize, usize) {
    ...
    for msg in messages {
        for block in &msg.content {
            match block {
                kernel::message::ContentBlock::Text { .. } => text += 1,
                kernel::message::ContentBlock::Thinking { .. } => thinking += 1,
                kernel::message::ContentBlock::ToolUse { .. } => tool_use += 1,
                kernel::message::ContentBlock::ToolResult { .. } => tool_result += 1,
                kernel::message::ContentBlock::Image { .. } => image += 1,
            }
        }
    }
