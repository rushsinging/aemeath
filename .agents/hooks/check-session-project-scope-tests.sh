...
mkdir -p "$TMP/agent/features/context/src/ports" \
  ...
  "$TMP/agent/features/runtime/src/application/loop_engine/chat"
...
RS
cat >"$TMP/agent/features/runtime/src/application/loop_engine/chat/idle_commands.rs" <<'RS'
fn execute() {
    list_for_project();
...
sed -i.bak 's/export_for_project/export/' "$TMP/agent/features/runtime/src/application/loop_engine/chat/idle_commands.rs"
expect_failure runtime-export 'Runtime session export must use project-scoped export'
mv "$TMP/agent/features/runtime/src/application/loop_engine/chat/idle_commands.rs.bak" "$TMP/agent/features/runtime/src/application/loop_engine/chat/idle_commands.rs"

sed -i.bak 's/update_metadata_for_project/update_metadata/' "$TMP/agent/features/runtime/src/application/loop_engine/chat/idle_commands.rs"
expect_failure runtime-rename 'Runtime session rename must use project-scoped metadata update'

sed -i.bak 's/import_for_project/import/' "$TMP/agent/features/runtime/src/application/loop_engine/chat/idle_commands.rs"
expect_failure runtime-import 'Runtime session import must use project-scoped import'
mv "$TMP/agent/features/runtime/src/application/loop_engine/chat/idle_commands.rs.bak" "$TMP/agent/features/runtime/src/application/loop_engine/chat/idle_commands.rs"

sed -i.bak 's/delete_for_project/delete/' "$TMP/agent/features/runtime/src/application/loop_engine/chat/idle_commands.rs"
