...
7. **关闭关联 issue 时的 Release Gate 同步**：关闭任意关联了 milestone 的执行 issue 前，**MUST** 检查并更新对应 Release Gate issue：
...
6. **子 issue 拆分原则**：大型工作的子 issue **SHOULD** 不超过 7 个，**MUST** 包含以下两类收尾 issue：
   ...
   - **收尾退役 issue**：按「代码修改后检查」规则清理旧路径、散点读取、多路并存和死代码；更新 `specs/` 分片和 `docs/design/`。
