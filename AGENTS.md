...
- **MUST** 合并 PR 的策略按 PR 方向区分：
  - **feature/bugfix → release**：使用 **Squash merge**，将分支上的多个提交压缩为一个提交后合入；head 分支合入后删除。
  - **release → main**：使用 **Merge commit**（`--no-ff`，非 squash、非 rebase），保留合并关系让 git 追踪已合入 commit，避免假冲突。
...
- 大型未提交改动在拉取集成分支前，**SHOULD** 先落盘续作 Plan、通过最低编译与定向测试门禁，并创建不含无关文件的 checkpoint commit；若当前改动不可编译或无法排除无关内容，**SHOULD NOT** 为 checkpoint 强行提交。
- **MUST** 同一 milestone 的所有执行 issue 合入 release 分支后，将 `release/vX.Y.Z` 通过 PR 合入 `main`（base: `main`，head: `release/vX.Y.Z`），使用 **Merge commit**（`--no-ff`）合入后在 main HEAD 打 `vX.Y.Z` tag 触发发版 workflow。
