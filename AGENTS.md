...
6. **MUST** 遵循 TDD（测试先行）。新增或修改核心逻辑前，**MUST** 先新增或修改对应测试（feature 先写表达期望行为的测试、bug 先写复现问题的失败测试、重构先确认现有测试覆盖目标行为），再用 `cargo test` 验证修改结果；细节见 `specs/3.2-rust-coding.md`。
7. 所有跨层功能 **MUST** 为**每一层**补相邻单元测试或契约测试，并用场景测试验证最终组合，**NEVER** 只测首尾或用端到端测试替代中间层覆盖。跨 Domain → Runtime/Application → Adapter → SDK/Published Language → Consumer Adapter → Model/Reducer → UI/外部行为的链路，涉及持久化时还 **MUST** 覆盖落盘与 Resume；每层都要验证输入语义、字段完整性、无覆写/旁路、无重复/冲突事件以及下一层输出。典型反面教材：PR #635 数据全链路正确但 TUI 不显示，根因是 `AgentDisplay::format_header_line_with_result` 覆写了 trait 默认方法绕过消费逻辑。
8. **MUST** 区分 Runtime 生命周期状态与业务终态。**Runtime 生命周期完成不等于业务成功**；内部 `Completed` 若包含取消、失败、终止、超时或其他非成功原因，Runtime 出站事件 **MUST** 保留真实用户语义。Runtime 是用户可见终态的唯一真相源，SDK/TUI 等消费方只透传或渲染权威终态，**NEVER** 通过 `run_id`、`step_id` 或多个生命周期事件重新推导业务结果。
9. `specs/*.md` 每个文件 **MUST** 拥有唯一且稳定的文档序号；文件名 **MUST** 以 `<序号>-` 开头，文件标题与所有正式章节 **MUST** 继承同一序号形成可索引章节号。Specs 正文 **NEVER** 挂 Issue/PR 编号或用外部追踪号表达架构状态；跨规范引用使用“文件路径 + §章节号”。删除章节后 **NEVER** 自动压缩后续编号。

## 2. 工作流（路径 / 目录结构 / Git / 发版）

以下内容已按渐进式披露原则移至 `specs/3.14-workflow.md`，**涉及对应场景时 MUST 加载**：
...
- 日志目录：`~/.agents/logs/`。日志按 target 路由到多个文件；具体映射 **MUST** 以 `specs/3.15-logging.md` 的 `TargetCatalog` 为准，未注册 target 才写入 `aemeath.log`。
...
完整配置优先级与环境变量规则见 `specs/3.9-config-compat.md`，Session 落盘规则见 `specs/3.10-storage.md`。
...
2. **场景触发（次，语义式）**：当工作触及某分片的范围但未碰它的路径——典型是别处在*消费*该能力（如改 provider 默认值会影响 `specs/3.7-prompt.md` 的 guidance 选择）——也 **MUST** 加载。
...
|---|---|---|
| `specs/3.2-rust-coding.md` | 横切，任意 `**/*.rs`、`**/Cargo.toml` —— 编码 / 测试 / 日志 / 验证门禁 / 错误处理 | 跑验证门禁、新增任何核心逻辑、调试构建或测试失败；任何跨层功能的逐层测试设计 |
| `specs/3.3-tui-cli.md` | `apps/cli/src/**` —— TUI（ratatui）、旧版 REPL（rustyline）、主题色板（Catppuccin Macchiato） | 改输入 / 渲染 / 快捷键 / 选区复制，新增工具显示，或消费 Runtime/SDK 权威终态、实时/Resume 展示 |
| `specs/3.4-runtime.md` | `agent/features/runtime/**` —— Agent 循环、tool 执行编排、token budget、compact、成本、slash 命令 | 改暂停 / 恢复 / 重试 / 取消 / 超时 / terminate / 终态投影（同步检查 `token_estimation`）、成本追踪（同步更新 `pricing.rs`）、新增 slash 命令 |
...
| `specs/3.8-project.md` | `agent/features/project/**` —— worktree 工作区上下文（`WorkspaceService` 单一可变状态源、COLA 分层、git 出站端口） | 改 worktree 进入 / 退出 / 持久化（同步 `3.10-storage.md` 会话落盘），或经 slash 命令操作 worktree（涉及 `3.4-runtime.md`） |
...
| `specs/3.10-storage.md` | `agent/features/storage/**` —— memory、task、history、tool_result 持久化 | 改会话 / 记忆 / 任务 / 历史的落盘格式或路径，或改 receipt / finalize cause / Session Resume 语义 |
...
| `specs/3.12-update.md` | `agent/features/update/**` —— 版本检查（GitHub API + semver + 缓存）、`aemeath update` 子命令 | 改版本检查逻辑 / 缓存策略 / 更新渠道配置 |
| `specs/3.13-bug-feature-tracking.md` | 无路径触发 | 任何 bug 修复或 feature 实现；操作 GitHub Issues（迁移自 `docs/bug/`、`docs/snapshot/`） |
| `specs/3.14-workflow.md` | 无路径触发 | 任何 bug 修复 / feature 实现 / PR 创建 / 发版 / Hook 阻断处理；多 worktree、多进程、Session/Resume 调试 |
