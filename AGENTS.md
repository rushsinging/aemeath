...
12. 执行 merge、冲突解决、cherry-pick、rebase 或代码移植时，**NEVER** 直接丢弃、ours/theirs 覆盖或静默删除任一侧的单元测试、契约测试、场景测试及测试夹具修改。Agent **MUST** 逐项核验双方测试所保护的行为与回归边界在合并结果中仍有等价或更强覆盖，并运行相关测试确认通过；确需删除或改写测试时，**MUST** 记录被替代的断言、替代证据与删除理由。
