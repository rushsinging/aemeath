...
4. **必有 issue 类型**：除功能 / bug 执行 issue 外，每个 milestone **MUST** 包含以下三类 issue：
   ...
   - **收尾退役 issue**：清理本 milestone 引入的死代码、多路并存、过期兼容层与旧路径；去除新增编译 warning。
...
6. **子 issue 拆分原则**：大型工作的子 issue **SHOULD** 不超过 7 个，**MUST** 包含以下两类收尾 issue：
   ...
   - **收尾退役 issue**：按「代码修改后检查」规则清理旧路径、散点读取、多路并存和死代码；去除新增编译 warning；更新 `specs/` 分片和 `docs/design/`。
...
- **MUST NEVER** 由 agent 合并 PR。PR 创建后由用户 review，由用户手动完成 GitHub merge 操作；agent 不执行 merge、不代理用户触发 merge。
...
每次完成代码修改后（含 bug 修复、feature 实现、重构），**MUST** 检查以下两类技术债并做出处理决策：

1. **多路并存场景**：同一功能、同一抽象或同一数据流是否出现多个并存的路径、实现分支、feature flag 或兼容层。典型信号包括：
   - 新旧两套实现同时服役（如旧函数与新函数返回相同语义）。
   - 同一能力通过不同入口暴露（如 slash 命令与菜单项逻辑分叉）。
   - 临时兼容代码长期存在且没有明确退役条件或追踪 issue。

2. **需要退役的代码**：是否产生了应当被移除的旧代码、废弃路径、过期兼容层、仅被测试引用的死代码，或已被新实现取代的临时方案。

发现上述任一情况时，**MUST** 向用户报告：问题位置、成因、未清理的风险，以及建议的清理方案（清理范围、是否在本次 PR 内处理、是否需要另开 issue 追踪）。**NEVER** 在知情的情况下让多路并存或待退役代码静默遗留，除非用户明确接受并记录退役计划。
