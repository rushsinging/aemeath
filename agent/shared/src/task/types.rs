...
//!
//! These canonical public-field DTOs live here (`share::task::types`) so that
//! `tools/build.rs` can generate precise JSON Schema for the tool inputs/outputs
//! that embed them. Task BC owns the richer Published Language in
//! `agent/features/task`; these DTOs remain temporarily because existing
//! consumers mutate them directly (#889 migrates those consumers, #891 removes
//! this façade).
