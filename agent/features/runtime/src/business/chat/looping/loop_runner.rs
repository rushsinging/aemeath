...
use crate::business::agent_run::{Run, RunSpec};
use crate::business::chat::looping::compact::manual_compact;
...
use crate::business::chat::looping::input_gate::apply_gate;
use crate::business::chat::looping::loop_phases::handle_turn_boundary_config;
...
use crate::business::loop_engine::run_loop;
    ...
    I: InputEventDrainPort,
{
            ...
            match $cmd {
                PendingCommand::Compact => {
                    if let Some(outcome) = manual_compact(
                        ...
                        &chain.messages_flat(),
                        &system_prompt_text,
                        &super::token_budget_config(context_size, client.max_tokens()),
                        &memory_config,
                        &memory_cwd,
                        &client,
                        &language,
    ...
    'session: loop {
        ...
        let mut port = MainRunPort {
            ...
            chain: &mut chain,
            token_budget: super::token_budget_config(context_size, client.max_tokens()),
            workspace: &workspace,
