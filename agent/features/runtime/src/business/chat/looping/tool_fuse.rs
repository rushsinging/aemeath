...
pub(crate) enum SessionBudgetDecision {
    Allow,
    Warning(&'static str),
    HardPause(&'static str),
}

#[derive(Debug, Clone)]
pub(crate) struct SessionBudgetFuse {
    context_window: u64,
    cumulative_input_tokens: u64,
    consecutive_tool_only_rounds: u64,
}

impl SessionBudgetFuse {
    pub(crate) fn new(context_window: usize) -> Self {
        let context_window = if context_window == 0 {
            log::warn!(
                target: LOG_TARGET,
                "session budget fuse initialized without context window; using conservative default"
            );
            32_000
        } else {
    ...
    pub(crate) fn record_tool_only_round(&mut self) -> SessionBudgetDecision {
        self.consecutive_tool_only_rounds += 1;
        if self.consecutive_tool_only_rounds >= 40 {
            SessionBudgetDecision::HardPause("consecutive tool-only rounds reached hard limit")
        } else if self.consecutive_tool_only_rounds >= 20 {
            SessionBudgetDecision::Warning("consecutive tool-only rounds reached warning limit")
        } else {
            SessionBudgetDecision::Allow
        }
    }

    pub(crate) fn record_assistant_text(&mut self) {
    ...
    pub(crate) fn record_usage(
        ...
        if turn_count >= 200 {
            return SessionBudgetDecision::HardPause("session model turns reached hard limit");
        }
        if input_tokens >= self.percent_of_context(30) {
            return SessionBudgetDecision::HardPause(
                "single-turn input tokens reached context hard limit",
            );
        }
        if self.cumulative_input_tokens >= self.context_window.saturating_mul(2) {
            return SessionBudgetDecision::HardPause("cumulative input tokens reached hard limit");
        }
        if turn_count >= 100 {
            return SessionBudgetDecision::Warning("session model turns reached warning limit");
        }
        if input_tokens >= self.percent_of_context(15) {
            return SessionBudgetDecision::Warning(
                "single-turn input tokens reached context warning limit",
            );
        }
        if self.cumulative_input_tokens >= self.context_window {
            return SessionBudgetDecision::Warning("cumulative input tokens reached warning limit");
        }
        SessionBudgetDecision::Allow
    }

    fn percent_of_context(&self, percent: u64) -> u64 {
...
mod tests {
    ...
    #[test]
    fn session_budget_uses_context_window_for_single_turn_input() {
        let mut fuse = SessionBudgetFuse::new(1000);

        assert_eq!(fuse.record_usage(1, 149), SessionBudgetDecision::Allow);
        assert_eq!(
            fuse.record_usage(2, 150),
            SessionBudgetDecision::Warning(
                "single-turn input tokens reached context warning limit"
            )
        );
        assert_eq!(
            fuse.record_usage(3, 300),
            SessionBudgetDecision::HardPause("single-turn input tokens reached context hard limit")
        );
    }

    #[test]
    fn session_budget_uses_context_window_for_cumulative_input() {
        let mut fuse = SessionBudgetFuse::new(1000);
        ...
        assert_eq!(
            fuse.record_usage(1, 149),
            SessionBudgetDecision::Warning("cumulative input tokens reached warning limit")
        );
        for _ in 0..6 {
            assert_eq!(
                fuse.record_usage(1, 149),
        ...
        assert_eq!(
            fuse.record_usage(1, 149),
            SessionBudgetDecision::HardPause("cumulative input tokens reached hard limit")
        );
    }

    #[test]
    fn session_budget_tracks_tool_only_rounds() {
        let mut fuse = SessionBudgetFuse::new(1000);
        for _ in 0..19 {
            assert_eq!(fuse.record_tool_only_round(), SessionBudgetDecision::Allow);
        }
        assert_eq!(
            fuse.record_tool_only_round(),
            SessionBudgetDecision::Warning("consecutive tool-only rounds reached warning limit")
        );
        fuse.record_assistant_text();
        assert_eq!(fuse.record_tool_only_round(), SessionBudgetDecision::Allow);
    }
}
