...
use super::runtime_preparation::{
    ParentRunCapabilities, PreparedRun, RunPreparationRequest, SessionState,
};
...
fn preparation_rejects_child_capability_above_parent_ceiling() {
    ...
    assert!(matches!(
        result,
        Err(super::runtime_preparation::RunPreparationError::CapabilityEscalation)
    ));
}
...
fn session_snapshot() -> super::runtime_preparation::SessionSnapshot {
    SessionState::new(
        "session-1",
        PathBuf::from("/workspace"),
        "model-a",
