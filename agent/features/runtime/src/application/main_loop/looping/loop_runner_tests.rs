...
impl QueueDrainPort for SequenceQueueDrainPort {
    fn drain_queued_input<'a>(&'a self) -> crate::application::main_loop::looping::QueueFuture<'a> {
...
impl InputEventDrainPort for ChannelInputEvents {
    fn drain_input_events<'a>(
        &'a self,
    ) -> crate::application::main_loop::looping::InputEventFuture<'a> {
        Box::pin(async move {
    ...
    fn recv_next_input<'a>(
        &'a self,
    ) -> crate::application::main_loop::looping::InputEventOptFuture<'a> {
        Box::pin(async move {
            let mut rx = self.rx.lock().await;
            rx.recv().await
        })
