...
/// Owned by the Main input strategy, scoped to one `Run`. User messages accumulate here from
...
///
/// # Lifecycle
/// 1. Created fresh when the Main input strategy is constructed.
...
impl RunInputBuffer {
    ...
    ///
    /// Still used by the Main input strategy for StopHookFeedback and
