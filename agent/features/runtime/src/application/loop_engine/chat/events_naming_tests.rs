//! #1679 对账测试：承接 check-runtime-event-naming 守卫退役。
//! RuntimeStreamEvent 变体集必须与 `.agents/runtime-event-naming-baseline.json`
//! 逐一对账（新增/删除/更名都需要同步 baseline，防止静默漂移）。

use std::collections::BTreeSet;
use std::path::PathBuf;

fn repo_root() -> PathBuf {
    // agent/features/runtime → 仓库根上溯三级。
...
fn baseline_variants(layer: &str) -> BTreeSet<String> {
    let raw =
        std::fs::read_to_string(repo_root().join(".agents/runtime-event-naming-baseline.json"))
...
fn source_variants(source: &str, enum_name: &str) -> BTreeSet<String> {
    ...
    for line in source.lines() {
        ...
        // 变体行：首 token 以大写字母开头（payload ` { … }` / `( … )` / 尾逗号剥离）。
...
#[test]
fn runtime_stream_event_variants_match_baseline() {
    let source = include_str!("events.rs");
    let actual = source_variants(source, "RuntimeStreamEvent");
    let baseline = baseline_variants("runtime_stream");
    ...
    assert!(
        ...
        "RuntimeStreamEvent 与 baseline 漂移：缺 {missing:?}；多 {unexpected:?}（同步 baseline 需评审）"
    );
}
