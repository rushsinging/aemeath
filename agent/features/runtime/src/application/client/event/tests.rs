...
fn test_runtime_tasks_snapshot_emits_sdk_event() {
    ...
    let event = runtime_event_to_sdk_event(
        crate::application::chat::RuntimeStreamEvent::TasksSnapshot {
...
async fn test_runtime_queue_drain_port_forwards_to_sdk_queue() {
    ...
    let drained = crate::application::chat::QueueDrainPort::drain_queued_input(&queue).await;
...
fn test_runtime_text_maps_to_sdk_token_with_context() {
    ...
    let event = runtime_event_to_sdk_event(
        crate::application::chat::RuntimeStreamEvent::Text {
...
fn test_runtime_user_messages_adopted_injects_input_ids() {
    ...
    let event = runtime_event_to_sdk_event(
        crate::application::chat::RuntimeStreamEvent::UserMessagesAdopted {
            items: vec![(
                accepted_id.clone(),
...
fn test_run_lifecycle_events_preserve_run_id() {
    ...
    let cancelling = runtime_event_to_sdk_event(
        crate::application::chat::RuntimeStreamEvent::RunCancelling {
            run_id: run_id.clone(),
        },
        &change_tx,
    );
    let cancelled = runtime_event_to_sdk_event(
        crate::application::chat::RuntimeStreamEvent::RunCancelled {
            run_id: run_id.clone(),
        },
        &change_tx,
    );
...
fn test_runtime_hook_event_maps_status_and_result() {
    let view = runtime_hook_event_to_sdk(RuntimeHookEvent {
        ...
        result: Some(crate::application::chat::RuntimeHookExecutionResult {
            exit_code: Some(1),
