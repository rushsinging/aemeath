...
use crate::application::interaction::{validate_reply, InteractionBridge, InteractionPort};
use crate::application::interaction_coordinator::{CoordinationError, InteractionCoordinator};
...
async fn coordinator_begin_fails_when_port_unavailable() {
    let mut run = run_in_executing_tools();
    let port = crate::application::interaction::UnavailableInteractionPort;
    let (body, continuation) = question_body();
...
async fn coordinator_full_roundtrip_reply() {
    ...
    match &completion {
        crate::application::interaction::InteractionCompletion::Replied(reply) => {
            let result =
