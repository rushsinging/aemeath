...
async fn test_process_chat_loop_stop_hook_blocked_continues_until_success() {
    ...
    let ctx = ChatLoopContext {
        ...
        workspace: project::WorkspaceService::new(std::env::current_dir().unwrap()),
...
async fn test_process_chat_loop_uses_workspace_workspace_root_for_stop_hook_env() {
    ...
    let workspace = project::WorkspaceService::new(path_base.path().to_path_buf());
    project::WorkspacePersist::restore(workspace.as_ref(), &workspace_dto)
