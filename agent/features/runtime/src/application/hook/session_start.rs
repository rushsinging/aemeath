...
use hook::{HookDispatchContext, HookInvocation, HookPort, SessionInput};
...
/// 失败明细由 Dispatcher 保留在 HookOutcome.executions 并经日志 target 观测。
pub(crate) async fn emit_session_start(
    hook_port: &Arc<dyn HookPort>,
    workspace_root: &Path,
    session_id: &str,
) {
    let invocation = HookInvocation::SessionStart(SessionInput {
        session_id: session_id.to_string(),
    });
    let context = HookDispatchContext::new(workspace_root).with_session_id(session_id);
    let outcome = hook_port
        ...
        .await;
    if outcome.directive != hook::HookDirective::Continue {
        log::debug!(
