...
use super::*;
use crate::application::hook_types::{RuntimeHookExecutionStatus, RuntimeHookReason};
...
#[tokio::test]
async fn proceed_when_hook_continues() {
    let port = continue_hook_port();
    let decision = evaluate_stop_hook(&port, 3, std::path::Path::new("/tmp")).await;
    assert!(
        matches!(decision, StopHookDecision::Proceed),
...
fn feedback_material_truncates_long_output() {
    use crate::application::hook_types::{RuntimeHookBlockDetail, RuntimeHookExecution};
