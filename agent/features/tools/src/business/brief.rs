...
impl Tool for BriefTool {
    ...
    async fn call(&self, input: Value, ctx: &ToolExecutionContext) -> ToolResult {
        ...
        match format {
            ...
            _ => {
                return ToolResult::error(serde_json::json!({"status": "error", "message": format!("Unknown format: {}. Use markdown, text, or json.", format), "data": null}).to_string());
            }
        }
        ...
        ToolResult::success(serde_json::json!({"status": "success", "message": "Brief generated successfully", "data": {"content": brief_content}}).to_string())
    }
}
