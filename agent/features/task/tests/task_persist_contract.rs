use task::{
    wire_task, BatchCreateSpecData, TaskCreateSpecData, TaskPriorityData, TaskSnapshotData,
};
...
fn task_persist_round_trips_nonempty_snapshot_between_capability_views() {
    ...
    source_access
        .create_batch(
            BatchCreateSpecData::try_new("source batch".into()).unwrap(),
            1,
        )
        .unwrap();
...
fn task_persist_rejected_snapshot_keeps_live_view_unchanged() {
    ...
    access
        .create_batch(
            BatchCreateSpecData::try_new("live batch".into()).unwrap(),
            1,
        )
        .unwrap();
    ...
    let error = persist.prepare_restore(&invalid).unwrap_err();
    assert!(
