...
use crate::api::core::memory::MemoryEntry;
use crate::api::hook::hook::HookRunner;
use crate::api::storage::memory::{memory_base_dir, project_hash_from_path, MemoryStore};
...
pub async fn load_agents_md(cwd: &Path, hook_runner: &HookRunner) -> String {
    ...
    let warnings = crate::api::policy::security::scan_content("AGENTS.md", &agents_md);
    if !warnings.is_empty() {
        ...
        if let Some(prefix) = crate::api::policy::security::format_warnings(&warnings) {
            agents_md = format!("{}\n\n{}", prefix, agents_md);
        }
    }
