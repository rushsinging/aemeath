...
pub(super) async fn run_agent_turns(
    ...
    compact_state: &mut aemeath_core::compact::AutoCompactState,
    turn_count: usize,
    verbose: bool,
    markdown: bool,
) -> TurnRunResult {
...
async fn maybe_compact_after_tools(
    ...
    let urgency = if last_api_input_tokens > 0 {
        let new_tokens = messages
            .last()
            .map(|m| aemeath_core::compact::estimate_messages_tokens(std::slice::from_ref(m)))
            .unwrap_or(0) as u64;
        aemeath_core::compact::compaction_urgency(last_api_input_tokens + new_tokens, context_size)
    } else if aemeath_core::compact::needs_compaction_full(
        messages,
        system_prompt_text,
        context_size,
        tool_schema_tokens,
    ...
    if urgency >= 1 && messages.len() > 4 {
        ...
        aemeath_core::compact::microcompact(messages, 6);
