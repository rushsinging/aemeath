...
impl OutputArea {
    ...
    pub fn render(&mut self, area: Rect, buf: &mut ratatui::buffer::Buffer) {
        ...
        let table_render_cache =
            render_blocks::render_table_cache(self.lines.iter(), &vis_lines, &table_block_lines);
    ...
    fn render_output_line(
        ...
        // 优先使用行内分段着色（diff 语法高亮 + 行号）
        if let Some(ref span_parts) = output_line.spans {
            self.render_span_line(idx, span_parts, screen_map, lines);
            return;
        }
