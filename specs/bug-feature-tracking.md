...
- bug 修复或 feature 完成后，**MUST** 等待用户确认，确认后从 `active.md` 移除条目并将完整详细描述**移动**到 `archived/<编号>-<slug>.md`：
  1. 删除表格中对应行；
...
- **active.md 仅保存活跃条目**。**NEVER** 在 active.md 中保留已归档/已修复条目的详情段或带"已修复/已归档/已确认修复"标记的章节——这类内容 **MUST** 一次性迁移到 `archived/` 或删除。
