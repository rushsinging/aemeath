...
`allow_all` **MUST** 属于 Run 级，**NEVER** 在同一 Run 的 tool round / Step 中改变授权语义。`context.snip_enabled` 与 `context.microcompact_enabled` 也 **MUST** 从所属 Run 的 frozen `ConfigSnapshot` 消费；两者独立控制，默认均为 `true`，同时开启时顺序固定为 Snip → Microcompact。guidance、`AGENTS.md`、`CLAUDE.md` 是 prompt assets，不属于 JSON Config：内容变化仅在下一 Run 注入带路径的 Read reminder，**NEVER** 重建 cacheable system prompt。
