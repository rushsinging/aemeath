...
- #1438 已完成 Skill ownership：Main/Sub Context 仅注入 metadata directory；唯一 `Skill` Tool 经 ToolExecution 在调用时加载正文；Runtime 只接纳 typed `SkillRequest` 并投影模型意图，不读取 Skill 文件。
