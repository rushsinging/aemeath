...
## 2.3. 大型工作的拆分与跟踪（GitHub Sub-issues）

### 2.3.1. 创建权限与拆分确认

Agent **NEVER** 自行创建、拆分、关闭或重组 Issue。发现当前工作存在可独立验证的子问题、隐含前置条件或可能需要多个 PR 时，Agent **MUST** 先向用户说明候选边界、依赖方向、合并为同一 Issue 与拆分为多个 Issue 的取舍，并询问用户意图。只有用户明确同意创建或拆分后，Agent 才能调用 GitHub API 创建 Issue、设置 parent/sub-issue 或 dependency；用户要求“在一个 Issue 内完成”时，Agent **MUST** 将新发现的范围、验收和测试门禁并入该 Issue，**NEVER** 擅自建立前置 Issue。
...
## 2.4. 开发环境（Cargo build-dir / Git hooks / sccache）
...
开发期间需要在 worktree 中编译并手动运行 TUI 验证交互行为时，使用以下方法。
...
日志级别由 `AEMEATH_LOG_LEVEL` 环境变量控制（替代旧 `RUST_LOG`），支持全局级别或 per-target directive：

```bash
AEMEATH_LOG_LEVEL=debug cargo run                                      # 全局 debug
AEMEATH_LOG_LEVEL=aemeath:tui=debug,aemeath:agent:runtime=trace cargo run  # per-target
```

- TUI 模式默认输出到 `~/.agents/logs/aemeath.log`；加 `-v` 同时输出到 stderr。
- `-q` 安静模式（仅 error 级别写入文件）。

### 查看日志

```bash
# 查看最近的日志
tail -50 ~/.agents/logs/aemeath.log

# 过滤特定模块
grep 'tui\.link' ~/.agents/logs/aemeath.log | tail -20
```
...
# 3. 在 TUI 中操作（如 Cmd+Click 链接）

# 4. 退出后查看日志
grep -E 'link_at|link_click' ~/.agents/logs/aemeath.log | tail -20
```
