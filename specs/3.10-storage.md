...
**次触发**：改会话 / 记忆 / 任务 / 历史的落盘格式或路径，或改 receipt / finalize cause / Session Resume 语义。
...
- `memory_store/`、`task_store/` 与 `tool_result.rs` 是过渡实现，**NEVER** 作为新增 Storage 机制的放置位置。
...
- **History**：旧 `storage/src/business/history.rs` 无生产消费者，不得作为后续 History 持久化能力的入口。
- **Tool result**：`agent/features/storage/src/tool_result.rs`——大体积 tool 结果落盘（默认上限见 MCP/工具配置）。
- Session schema v6 在 CanonicalSession 中保存按 `(SkillLoadScope, canonical skill name)` 唯一的内容 revision 记录；相同 revision 不写盘，更新 revision 与首次加载必须通过 Context mutation gate 原子 durable save 后再 publish。compact 保留记录，clear 清空记录，Resume 恢复记录；v1-v5 reader 升级为空记录，unknown future schema 仍 fail-closed。
...
## 3.10.4 Tool 生命周期持久化与恢复

- 已接受的 Tool 调用 **MUST** 先持久化 durable receipt 再开始执行；terminate、崩溃或取消 **NEVER** 让已接受调用从恢复历史中消失。
- Tool terminal receipt **MUST** 在 Step finalize 前完成持久化收口；Runtime **NEVER** 通过提前 drop execution future 跳过该协议。
- 崩溃遗留的 Pending/Running receipt **MUST** 按兼容策略恢复为不确定终态，**NEVER** 自动重放、伪装成功或伪装已确认取消。
- Resume 投影 **MUST** 保留 `finalize_cause`、耗时与必要终态字段；实时与 Resume 对同一持久化事实 **MUST** 产生等价业务语义。
- Session schema 变更 **MUST** 逐层测试旧版本兼容、当前版本读写、未知未来版本 fail-closed、primary/previous 恢复，以及 Runtime→SDK→TUI 的字段完整性。

## 3.10.5 Session Lock 文件
