...
### 3.15.1.1 TargetCatalog

合法 target、owner、sink ID 和日志文件名只在 `packages/global/logging/src/domain/routing.rs` 定义一次。Catalog 覆盖具有独立运行时边界的 owner；Provider 另有专用的 LLM API Error target，Runtime 的 Prompt target 保留为专用子能力路由。生产调用必须使用 owner 常量，并由全仓 Guard 从 workspace members 反向校验 runtime owner 与非 runtime member 分类。
...
| — | emergency | `emergency.log` | logging 自身 | File 模式下 sink degrade / fallback 的兜底输出（**NEVER** 写 stderr，避免污染 TUI alternate screen，） |
| — | `panic.log` | `panic.log` | panic_hook.rs 直写 | panic 信息（不纳入 UnifiedLogger） |
| — | native stderr | `native-stderr.log` | 当前进程的 native/FFI 与意外继承 FD 2 的辅助进程 | 非结构化原生诊断字节流；不属于 TargetCatalog/14 字段 JSONL；Bash/Hook/MCP 业务 stderr MUST 继续走各自 pipe |
...
| 7 | `run_step` | number \| null | usize | scope-local `LogContext.run_step` | `5` |
...
| `info`  | 用户/运维关心的**关键生命周期事件** | 低频，**NEVER** 含完整 JSON payload 或 per-chunk/per-run-step 高频日志 |
...
- per-chunk / per-run-step / per-hook 事件级日志（应为 `debug` 或 `trace`）。
...
### 3.15.4.7 Per-Layer 细则

#### 3.15.4.7.1 provider 层（`agent/features/provider/**`）

| 场景 | Level | 说明 |
|---|---|---|
| provider client 创建、model 选定 | `info` | 一次性生命周期 |
| `send_message` / `stream_message` 入口 | `debug` 或不加 | 非 lifecycle，是 per-run-step |
| SSE chunk raw 行 | `trace` | 高频 |
| reasoning / content delta 长度统计（per-chunk） | `trace` | 高频累计 |
| `tool_use_start` / first delta（一次性） | `debug` | 每 run step 一次 |
...
#### 3.15.4.7.2 runtime 层（`agent/features/runtime/**`）

| 场景 | Level | 说明 |
|---|---|---|
| session 启动/结束、run step 起止摘要 | `info` | 低频生命周期 |
...
| 每 run step 的 tool_call 独立日志 | `debug` | per-run-step 中频 |
| sub-agent 每 run step 进度回调 | `debug` | per-run-step |
| 每 run step 边界的 config_reload 检测 | `debug` | per-run-step |
...
| invocation reminder 产生、once-per-Run 附着/跳过、ContextRequest 转发、ContextWindow 返回与 Provider request 就绪 | `debug` | per-run-step 全链路诊断，只记录类型、数量、identity、结构计数与正文 preview |
| invocation reminder 完整渲染正文 | `trace` | 默认关闭；写入前必须做凭据脱敏，正文不得出现在 `info` / `debug` |

#### 3.15.4.7.3 tui 层（`apps/cli/src/tui/**` + `apps/cli/src/chat/**`）
...
| `tool_call_start` / `tool_result`（一次性） | `debug` | 每 run step 有限次 |
...
#### 3.15.4.7.4 hook / tools / storage / prompt / update 层
...
### 3.15.4.8 强制约束（MUST / NEVER / SHOULD）
...
| 统一要求生产代码日志带 `target:` | **MUST**（已落地，守卫强制） | `3.2-rust-coding.md` + `domain/routing_guard.rs` 已实现 |
| 从 `log` 迁移到结构化 `tracing` | **推迟**，单开 issue 追踪 | 当前无 trace 可视化后端消费 span 因果链，迁移成本高且与 level 规范正交 |
...
| TRACE | 完整内容 | 可记录完整 JSON、chunk 文本；invocation reminder 正文写入前仍 MUST 脱敏凭据 |
...
### 3.15.5.2 request_id 生命周期

- **生成**：每次 LLM 请求前（`loop_run.rs` 中 `uuid::Uuid::new_v4()`）
- **贯穿**：每次物理请求创建不可变 child `LogContext`，该 scope 覆盖请求开始到最后一个 chunk / 终态响应
- **清理**：request child scope 结束后自动恢复父级 run step context；retry 创建新的 request ID 与新 child scope
