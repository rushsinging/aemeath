# 3.8 工作区（Project / Worktree Context）
...
**配套**：worktree 状态的会话落盘见 `3.10-storage.md`；经 slash 命令触发 worktree 操作的编排见 `3.4-runtime.md`。消费 `share::session_types::PersistedWorkspaceContext`（最小共享内核）。
...
- `WorkspaceState`（`src/domain/state.rs`）持 `initial_cwd` / `workspace_root` / `path_base` / `worktrees_root` / worktree 栈 `stack`；进入、退出 worktree 即栈 frame 的压入 / 弹出。
- 默认基线分支 `main`（`DEFAULT_WORKTREE_BASE`）。`EnterWorktree` 省略 `path` 时默认推导为 `<worktrees_root>/<仓库名>/<安全分支名>`；`worktrees_root` 由 wiring 注入：未配置时默认 `~/.agents/worktrees`（`global_worktrees_dir()`，跟随 `AEMEATH_AGENTS_DIR`），可经 `storage.worktrees_dir`（json，见 `3.9-config-compat.md`）或 `AEMEATH_WORKTREES_DIR`（env）配置，相对配置值相对 workspace root 解析（配 `.worktrees` 可恢复仓库内布局）。仓库名段取 `workspace_root` 目录名并按分支同一字符白名单清洗，避免多仓库同名分支在共享根目录下冲突。

## 3.8.2 Project 专用 Hexagonal 分层
