...
5. **通过 PR 自动关闭 Issue**：交付范围完整覆盖 Issue 且全部门禁已闭合时，PR **MUST** 使用 `Closes #<issue>`，由 GitHub 在 PR 合入默认分支后自动关闭 Issue；无需另行取得关闭 Issue 的授权。Agent **NEVER** 通过 `gh issue close`、网页操作或其他直接方式自行关闭 Issue。若 PR 仅提供部分交付，或 Issue 合入后仍需继续跟踪剩余工作，PR **MUST** 使用 `Refs #<issue>`，**NEVER** 使用 closing keyword。PR 未合并、合并失败或仅被关闭时，Issue **MUST** 保持开放。
...
## 3.14.2 Milestone / Release Gate 管理
...
## 3.14.3 大型工作的拆分与跟踪（GitHub Sub-issues）
...
## 3.14.4 开发环境（Cargo build-dir / Git hooks / sccache）
...
排查跨进程、Session/Resume、“修改未生效”或多次复现结果不一致的问题前：

- **MUST** 记录 PID、进程 CWD、worktree/分支、Git SHA、实际 binary 路径、`AEMEATH_AGENTS_DIR`、Session ID、日志文件与时间窗口。
- **NEVER** 仅凭脚本名、终端提示符、当前 shell 目录、分支名或 target 文件名判断运行的是新旧代码；每次对比实验都 **MUST** 将运行实例绑定到明确 binary 与 SHA。
- **MUST** 联合检查 Session primary、`.previous`、receipt ledger、Step finalize 事实与 Runtime 日志，**NEVER** 只搜索聊天消息推断执行和持久化是否发生。
- 对反复出现、跨组件或证据不足的问题，**MUST** 先补 debug 日志建立每个边界的输入、输出、identity 与终态证据，再修改实现；**NEVER** 继续依赖猜测。
- 调试证据 **MUST** 区分用户看到的 UI、Runtime 实际事件、外部资源状态与 Session 最终落盘事实，避免把其中一个层次当成整个链路结论。

## 3.14.6 Git 工作流

日常开发在 `main` 上进行。发版不走长期 release 分支：由 `release.sh <vX.Y.Z>` 直接在 `origin/main` 当前 commit 创建轻量 tag 触发；**NEVER** 为发版引入 `release/*` 分支或 merge commit 流程。

- **MUST** 所有 feature / bugfix 在独立 worktree 中开发，worktree 基于 `origin/main` 最新 commit 创建；**NEVER** 直接 push 到 `main`。
- **MUST** feature / bugfix 分支完成后通过 **Pull Request** 提交到 `main`。
- **MUST** 合并 PR 使用 **Squash merge**；**NEVER** 使用 rebase merge 或 merge commit。
- **MUST** agent 默认不得合并 PR；PR 创建后应等待用户 review。只有同时满足以下条件时，agent **MAY** 代用户执行合并：
  ...
  4. **MUST** 严格使用本节规定的策略：一律 Squash merge；**NEVER** 使用 admin bypass、rebase merge 或修改保护规则。
  5. 若 PR 使用 `Closes #<issue>`，合并前 **MUST** 再次确认 PR 完整覆盖该 Issue、Issue checklist 全部闭合且没有合入后仍需跟踪的剩余工作；满足条件时，用户对该 PR 当前 head 的明确合并授权同时允许 GitHub 自动关闭关联 Issue，Agent **NEVER** 另行直接关闭。条件不满足时，合并前 **MUST** 将 closing keyword 改为 `Refs`，保持 Issue 开放。
  6. 合并后 **MUST** 查询 PR 状态与目标分支结果，确认实际合入；同时核对 closing keyword 对应 Issue 的状态符合预期。失败时报告真实状态，**NEVER** 重复或改用其他策略静默重试。
- **MUST** 创建 PR 前，在 worktree 分支上执行 `git pull origin main` 拉取最新主分支。
- **MUST** 发版经 `release.sh <vX.Y.Z>` 执行：脚本校验 tag 格式与唯一性，在 `origin/main` 当前 commit 创建轻量 tag 并 push，触发发版 workflow；**NEVER** 手工 `git tag` / `gh api` 绕过脚本。

### 3.14.6.1 分支保护规则（Rulesets）
...
发版通过 `release.sh` 创建并 push `v*` tag，触发 `.github/workflows/release.yml` 自动完成。**MUST** 遵守：

- **MUST** 由用户明确指定版本号，agent **NEVER** 自行决定发版或推演版本号。
- **MUST** tag 由 `release.sh <vX.Y.Z>` 创建，指向 `origin/main` 当前 commit（脚本内校验）；**NEVER** 手工创建 tag、**NEVER** 在历史 commit 上补 tag、**NEVER** 为发版切 `release/*` 分支。
