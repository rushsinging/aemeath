...
- `api.user_agent` 是所有 Aemeath 自发 HTTP 请求的默认 HeaderValue，属于 Run scope；Provider 与 WebSearch 在下一 Run 使用冻结快照。MCP 配置中的显式 `User-Agent` header（大小写不敏感）优先，绝不覆盖。
...
| `user_agent()` | `&str` | 所有 Aemeath 自发 HTTP 请求的统一默认 User-Agent；MCP 显式 `User-Agent` header 优先。|
| `model_name()` | `&str` | 模型名称 |
