...
**次触发**：新增 provider。
**配套**：provider 的默认 base URL / 默认 model / API key 环境变量名等**配置**在 `agent/shared/src/config/`，见 `3.9-config-compat.md`；model guidance 文件见 `3.7-prompt.md`。
...
2. 在 `agent/shared/src/config/`（见 `3.9-config-compat.md`）补默认 base URL / 默认 model / API key 环境变量名。
