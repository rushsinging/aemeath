...
**次触发**：新增 provider。
**配套**：provider 的默认 base URL / 推荐 model / API key 环境变量名 / 官方 SDK User-Agent 等**配置**由 Config BC `ProviderCatalog` 统一拥有，见 `3.9-config-compat.md`；model guidance 文件见 `3.7-prompt.md`。
...
2. 在 Config BC `ProviderCatalog`（见 `3.9-config-compat.md`）补默认 base URL、推荐 model、API key 环境变量名及有可靠证据时的官方 SDK User-Agent；**NEVER** 在 Provider adapter 复制这些默认值。
3. **SHOULD** 同步添加 model guidance 文件（见 `3.7-prompt.md`）。
4. **MUST** 只发布 provider-neutral raw usage，不计算或携带 Cost/Price；Runtime 负责 logical invocation 关联，Audit v0.1.0 只存 Usage。Future Pricing 不得回流 Provider。
