...
- `ToolRegistry` 仅是 Tools adapter 内部 backing；正式 Main/Sub Catalog 与 Execution 不发布或执行 Skill，并以 Skill-owned Catalog/Materialization 双端口接入 Context。`legacy-no-agent`、历史 `register_all_tools*`、内部 Profile/Registry 与 `SkillTool` 均为待退役兼容路径，**NEVER** 作为新机制入口。
...
- Skill materialization 与 Command Catalog/Router 已形成独立边界；旧 Registry/Profile/SkillTool 只允许退役，**NEVER** 扩展。
