...
**不适用**：feature 专属的业务规则在各自分片（`3.3-tui-cli.md` / `3.4-runtime.md` / …）。

## 3.2.1 编码规范

### 3.2.1.1 NEVER
- **NEVER** 在 core 库中使用 `println!` / `eprintln!`（`lib.rs` 已 deny）。
- **NEVER** 直接读取环境变量——使用配置分层（见 `3.9-config-compat.md`）。
...
- UnifiedLogger 驱动，从配置文件的 `logging.level` 读取全局日志级别；输出模式（File/Stderr）由 CLI typed bootstrap 输入决定（`-q -v` → Stderr，否则 File，），**NEVER** 经 env 旁路。
...
> **日志 level 等级选择**：见 `specs/3.15-3.15-logging.md` 的「日志级别策略」章节（5 级定义 + per-layer 细则）。选择 `trace/debug/info/warn/error` 时 **MUST** 对照该章节。
...
- **MUST** 所有跨层功能为 Domain、Application/Runtime、Adapter、SDK/Published Language、Consumer Adapter、Model/Reducer、UI/外部行为中的每一层建立相邻单元测试或契约测试；涉及持久化时还 **MUST** 覆盖落盘与 Resume。
- **MUST** 每层验证输入语义、字段完整性、无覆写或旁路、无重复或冲突事件，以及传给下一层的输出；目标是让链路在任一层断裂时由该层测试直接定位，**NEVER** 从最终表现逐层倒查作为常规验证方式。
- **MUST** 用最终场景测试证明组合正确，但 **NEVER** 用 L4/L5 或端到端测试替代中间层契约。
- **MUST** 状态与终态测试同时断言期望结果存在和冲突结果不存在；例如取消后不仅要出现取消终态，还必须证明没有成功终态或重复 terminal notice。
- **MUST** 实时路径和 Resume 路径对同一持久化事实产生等价业务语义；逐层测试必须覆盖终态原因、耗时等字段在编码、投影与消费中的完整性。
- **SHOULD** 对随机或枚举展示断言排除整个冲突类别，**NEVER** 仅排除一个当前样例值而让同类错误继续漏测。
