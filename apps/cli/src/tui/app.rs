...
impl App {
    ...
    fn draw(&mut self, terminal: &mut Terminal<CrosstermBackend<io::Stdout>>) -> io::Result<()> {
        ...
        // #520 workaround（待 ratatui 0.30 升级解除）：输出发生结构性变化（首帧 / 行数
        // 减少 / 滚动）时，先强制全屏清屏 + 全量重绘，清除 ratatui 0.29 在宽字符尾随
        // cell 上遗留的样式残影。流式追加（行数仅增长）不触发，避免闪屏。详见
        // `OutputArea::should_force_repaint`。
        {
            let doc = self.output_area.document();
            let total = doc.total_lines();
            let blocks = doc.blocks.len();
            let scroll = self.view_state.output.scroll_offset;
            if self.output_area.should_force_repaint(total, blocks, scroll) {
