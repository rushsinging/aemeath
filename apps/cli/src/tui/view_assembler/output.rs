...
impl OutputViewAssembler {
    ...
    pub(crate) fn assemble_shared_roots(
        conversation: &ConversationModel,
        workspace_root: Option<&std::path::Path>,
    ) -> Vec<std::sync::Arc<BlockNode>> {
        let lookup = super::output_tool_lookup::ConversationToolLookup::new(conversation);
        let mut roots = conversation
            .timeline
            .items()
            .iter()
            .filter_map(|item| {
                Self::assemble_item(item, &lookup, workspace_root).map(std::sync::Arc::new)
            })
            .collect::<Vec<_>>();
        if let Some(placeholder) = Self::assemble_placeholder(conversation) {
            roots.push(std::sync::Arc::new(placeholder));
        }
        roots
    }
