...
fn bench_refresh_cost_by_conversation_size() {
    ...
    fn build_conversation(n_runs: usize) -> ConversationModel {
        ...
        for i in 0..n_runs {
            ...
            let run_id = ChatRunId::new(format!("turn-{i}"));
    ...
    for n in [500usize, 1000, 2000, 4000] {
        ...
        println!(
            "runs={n:>5} blocks={blocks:>6} roots={roots:>6} | assemble={assemble_ms:>8.2}ms  render_cold={render_cold_ms:>8.2}ms  render_warm={render_warm_ms:>8.2}ms  | streaming_chunk≈assemble+warm={:>8.2}ms",
